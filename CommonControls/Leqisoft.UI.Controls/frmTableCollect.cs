...
#pragma warning restore CS0649
	...
	private void btnConfirm_Click(object sender, EventArgs e)
	{
		...
		{
			if (item.UserData is Leqisoft.Model.Column column2)
			{
	...
	private void InitializeMapping(CollectObjectEnum collectObject)
	{
		...
		foreach (Leqisoft.Model.Column column2 in Collector.Setting.Table.Columns)
		{
	...
	private void PopulateMapping(Dictionary<long, string> maps)
	{
		foreach (C1.Win.C1FlexGrid.Column item in (IEnumerable)grdMapping.Cols)
		{
			if (item.UserData is Leqisoft.Model.Column)
			{
		...
		foreach (KeyValuePair<long, string> map in maps)
		{
			...
			{
				if (item2.UserData is Leqisoft.Model.Column { Id: { Value: var value } } && value.Equals(map.Key))
				{
	...
	{
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(Leqisoft.UI.Controls.frmTableCollect));
