...
public class TableFilterContext : FilterContext
{
	...
	{
		Leqisoft.Model.Column column = Table.Columns[col];
		...
		object value2 = Leqisoft.Model.Cell.ChangeDataTypeImpl(value, dataType);
		return FilterValue.FromObject(value2, Leqisoft.Model.Cell.GetDisplayValueImpl(value2, column.GetFormat()));
	}

	public override List<FilterValue> GetColumnData(string columnId)
	{
		Leqisoft.Model.Column modelColumn = GetModelColumn(columnId);
		if (modelColumn == null)
		{
	...
	private Leqisoft.Model.Column GetModelColumn(string columnId)
	{
