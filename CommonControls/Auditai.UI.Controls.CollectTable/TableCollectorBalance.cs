...
public class TableCollectorBalance : TableCollectorAbstract
{
	...
	public TableCollectorBalance(Ledger ledger, Auditai.Model.Table table)
		: base(ledger, table)
	{
	...
	public override TableCollectResult Collect(int auditYear)
	{
		...
		Auditai.Model.Table table = base.Setting.Table;
		...
		foreach (KeyValuePair<long, string> map2 in base.Maps)
		{
			...
			if (!base.IsSaveCollectResultByColumnId && !table.Columns.Any((Auditai.Model.Column c) => c.Id == new Id64(map.Key)))
			{
				continue;
			}
