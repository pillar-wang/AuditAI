...
public class HelpViewerForm : C1RibbonForm
{
	...
	public bool LoadPdf(string file)
	{
		if (!File.Exists(file))
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "数据源不存在！", MessageBoxButtons.OK, "", false);
			return false;
		}
		try
		{
		...
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "打开文件失败！" + ex.Message, MessageBoxButtons.OK, "", false);
			return false;
		}
	}
	...
	private void cmdPreviousPage_Click(object sender, ClickEventArgs e)
	{
		...
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "当前文档为空！", MessageBoxButtons.OK, "", false);
		}
		else if (pdfViewer.Renderer.Page <= 0)
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "已到第一页！", MessageBoxButtons.OK, "", false);
		}
		else
		{
	...
	private void cmdNextPage_Click(object sender, ClickEventArgs e)
	{
		...
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "已到最后一页！", MessageBoxButtons.OK, "", false);
		}
		else
		{
			pdfViewer.Renderer.Page++;
	...
	private void Form_Find_NextKeyDown(object sender, FindReplaceEventArgs e)
	{
		...
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "未找到匹配项！", MessageBoxButtons.OK, "", false);
		}
		else if (_current >= _matches.Items.Count)
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "已到最后一个项！", MessageBoxButtons.OK, "", false);
			_current = 0;
		}
		else
		{
