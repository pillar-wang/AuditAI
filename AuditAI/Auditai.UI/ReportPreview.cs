...
public class ReportPreview
{
	protected class PagePrintSetting
	{
		...
		public double DataTableHeaderFinallyHeight;

		public double DataTableFinallyHeight;

		public double MainTitleFinallyHeight;

		public double SubTitleFinallyHeight;
	...
	private Auditai.Model.Image _image;
	...
	private Auditai.Model.Project CurrentProject
	{
		get
		{
			if (PreviewKind != 0)
	...
	public Auditai.Model.Image Image
	{
		get
		{
			return _image;
	...
	private RenderTable CreatePageHeader(Dictionary<string, string> tagDic)
	{
		...
		renderTable.Cells[num, 0].RenderObject = renderRichText;
		...
		renderTable.Cells[num, 1].RenderObject = renderRichText2;
		...
		renderTable.Cells[num, 2].RenderObject = renderRichText3;
		num++;
		renderTable.Cells[num, 0].SpanCols = 3;
	...
	private string OneColorDeal(string Rtf)
	{
		...
		return new RtfHelper().Load(Rtf).Monochrome().Save();
	}
	...
	private string HeaderFooterDeal(string str, Dictionary<string, string> tagDic, HorizontalAlignment align)
	{
		...
		return new RtfHelper().Load(rtf).ReplaceVariables(CurrentProject.DataReferenceManager, new DataReferenceEvaluationContext
		{
	...
	private void ResetCurrentPrintSetting()
	{
		...
		double num = Table.Title.TitleHeight;
		double num2 = Table.Title.Rows.Sum((TableTitleRow r) => r.Height);
		double num3 = Table.Columns.Sum((Auditai.Model.Column col) => col.Visible ? col.Width : 0);
		double num4 = Table.Rows.Sum((Auditai.Model.Row row) => row.Visible ? row.Height : 0);
		double num5 = Table.SumHeaderHeight(Table.GetNumCaptionRows());
		double num6 = ((Table.Foot != null) ? Table.Foot.Rows.Sum((TableTitleRow r) => r.Height) : 0);
		double num7 = UnitSubtract(_pd.PageLayout.PageSettings.Width, new Unit(PageSetup.LeftMargin + PageSetup.RightMargin, UnitTypeEnum.Mm)).ConvertUnit(UnitTypeEnum.Pixel, ScreenGraphics.DpiX);
		double num8 = UnitSubtract(_pd.PageLayout.PageSettings.Height, new Unit(Math.Max(PageSetup.HeaderMargin, PageSetup.TopMargin) + Math.Max(PageSetup.FooterMargin, PageSetup.BottomMargin), UnitTypeEnum.Mm)).ConvertUnit(UnitTypeEnum.Pixel, ScreenGraphics.DpiY);
		double num9 = num + num2 + num5 + num4 + num6;
		_currentPrintSetting.PageDataAreaMaxWidth = num7;
		_currentPrintSetting.PageDataAreaMaxHeight = num8;
		if (PageSetup.FitPageWidth || PageSetup.FitPageHeight)
		{
			...
			_currentPrintSetting.HScale = num10;
			_currentPrintSetting.VScale = num10;
			_currentPrintSetting.FinallyWidth = num3 * num10;
			_currentPrintSetting.DataTableHeaderFinallyHeight = num5 * num10;
			_currentPrintSetting.DataTableFinallyHeight = num4 * num10;
			_currentPrintSetting.MainTitleFinallyHeight = num * num10;
			_currentPrintSetting.SubTitleFinallyHeight = num2 * num10;
			_currentPrintSetting.FooterTableFinallyHeight = num6 * num10;
		}
		else
		{
			_currentPrintSetting.HScale = PageSetup.HorizontalZoom;
			_currentPrintSetting.VScale = PageSetup.VerticalZoom;
			_currentPrintSetting.FinallyWidth = num3 * _currentPrintSetting.HScale;
			_currentPrintSetting.DataTableHeaderFinallyHeight = num5 * _currentPrintSetting.VScale;
			_currentPrintSetting.DataTableFinallyHeight = num4 * _currentPrintSetting.VScale;
			_currentPrintSetting.MainTitleFinallyHeight = num * _currentPrintSetting.VScale;
			_currentPrintSetting.SubTitleFinallyHeight = num2 * _currentPrintSetting.VScale;
			_currentPrintSetting.FooterTableFinallyHeight = num6 * _currentPrintSetting.VScale;
		}
	}

	protected Unit UnitSubtract(Unit lhs, Unit rhs)
	{
		double num = _pd.ConvertUnit(lhs, _pd.ResolvedUnit);
		double num2 = _pd.ConvertUnit(rhs, _pd.ResolvedUnit);
		return new Unit(num - num2, _pd.ResolvedUnit);
	}

	protected Unit UnitAdd(Unit lhs, Unit rhs)
	{
		double num = _pd.ConvertUnit(lhs, _pd.ResolvedUnit);
		double num2 = _pd.ConvertUnit(rhs, _pd.ResolvedUnit);
		return new Unit(num + num2, _pd.ResolvedUnit);
	...
	private RenderObject CreateMainTitle()
	{
		...
		renderText.Text = Table.Title.TitleCell.GetDisplayValue();
		...
		renderText.Style.FlowAlign = FlowAlignEnum.Center;
		renderText.Width = new Unit(Math.Min(_currentPrintSetting.FinallyWidth, _currentPrintSetting.PageDataAreaMaxWidth), UnitTypeEnum.Pixel);
		renderText.Height = new Unit(_currentPrintSetting.MainTitleFinallyHeight, UnitTypeEnum.Pixel);
		return renderText;
	}
	...
	{
		RenderTable renderTable = new RenderTable();
		renderTable.CellStyle.TextIndent = new Unit(1.0, UnitTypeEnum.Point);
		float num = Table.Title.Columns.Sum((TableTitleColumn c) => c.Width);
		for (int i = 0; i < Table.Title.Columns.Count; i++)
		{
			renderTable.Cols[i].Width = new Unit(Table.Title.Columns[i].Width, UnitTypeEnum.Pixel);
		}
		for (int j = 0; j < Table.Title.Rows.Count; j++)
		{
			TableTitleRow tableTitleRow = Table.Title.Rows[j];
			renderTable.Rows[j].Height = new Unit(tableTitleRow.Height, UnitTypeEnum.Pixel);
			for (int k = 0; k < tableTitleRow.Cells.Count; k++)
			{
				TableTitleCell tableTitleCell = tableTitleRow.Cells[k];
				TableCell tableCell = renderTable.Cells[j, k];
				tableCell.Text = tableTitleCell.GetDisplayValue();
				UpdateStyleFromTitle(tableCell.Style, tableTitleCell);
				tableCell.CellStyle.Padding.Left = ConvertPixelNumberToUnit(tableTitleCell.Margin, UnitTypeEnum.Inch);
			}
		}
		foreach (TicketMerge merge in Table.Title.Merges)
		{
			TableCell tableCell2 = renderTable.Rows[merge.TopRow][merge.LeftColumn];
		...
		renderTable.Width = new Unit(Math.Min(_currentPrintSetting.FinallyWidth, _currentPrintSetting.PageDataAreaMaxWidth), UnitTypeEnum.Pixel);
		renderTable.Height = new Unit(_currentPrintSetting.SubTitleFinallyHeight, UnitTypeEnum.Pixel);
		return renderTable;
	}
	...
	private void CreateMainTable()
	{
		...
		MainTableSettupImpl();
		DataTable.Width = new Unit(_currentPrintSetting.FinallyWidth, UnitTypeEnum.Pixel);
		DataTable.Height = new Unit(_currentPrintSetting.DataTableHeaderFinallyHeight + _currentPrintSetting.DataTableFinallyHeight, UnitTypeEnum.Pixel);
	}
	...
	private RenderObject MainTableDataImpl(int printNum)
	{
		...
		void CreateCaption()
		{
			Dictionary<int, List<string>> dictionary = Table.Columns.ToDictionary((Auditai.Model.Column t) => t.Index, (Auditai.Model.Column t) => t.CaptionDisplay.Split('_').ToList());
			layers = ((dictionary.Count != 0) ? dictionary.Max((KeyValuePair<int, List<string>> t) => t.Value.Count) : 0);
			for (int l = 0; l < layers; l++)
			{
				DataTable.Rows[l].Height = new Unit(Table.GetHeaderHeight(l), UnitTypeEnum.Pixel);
				for (int m = 0; m < PrintNum; m++)
				{
					int num2 = (Table.Columns[m].Visible ? Table.Columns[m].Width : 0);
					DataTable.Cols[m].Width = new Unit(num2, UnitTypeEnum.Pixel);
		...
		void CreateContent()
		{
			for (int i = 0; i < Table.Rows.Count; i++)
			{
				...
				if (row.Visible)
				{
					DataTable.Rows[i + layers].Height = new Unit(row.Height, UnitTypeEnum.Pixel);
					for (int j = 0; j < PrintNum; j++)
					{
						Auditai.Model.Cell cell2 = Table.Cells.Get(i, j);
			...
			for (int k = 0; k < PrintNum; k++)
			{
				...
				DataTable.Cols[k].Width = new Unit(num, UnitTypeEnum.Pixel);
			}
		}
		RenderObject CreateRenderCell(Auditai.Model.Cell cell)
		{
	...
	private RenderObject TableHeaderImpl(Dictionary<string, string> tagDic)
	{
		...
		Unit lhs = new Unit(0.0, UnitTypeEnum.Pixel);
		RenderTable renderTable2 = CreatePageHeader(tagDic);
		lhs = UnitAdd(lhs, renderTable2.Rows[0].Height);
		renderTable.Cells[0, 0].SpanCols = 3;
		renderTable.Cells[0, 0].RenderObject = renderTable2;
		renderTable.Rows[0].Height = renderTable2.Rows[0].Height;
		RenderObject renderObject = CreateMainTitle();
		renderObject.Style.FlowAlign = FlowAlignEnum.Center;
		lhs = UnitAdd(lhs, renderObject.Height);
		renderTable.Cells[1, 1].RenderObject = renderObject;
		renderTable.Rows[1].Height = renderObject.Height;
		RenderObject renderObject2 = CreateSubTitle();
		renderObject2.Style.FlowAlign = FlowAlignEnum.Center;
		lhs = UnitAdd(lhs, renderObject2.Height);
		renderTable.Cells[2, 1].RenderObject = renderObject2;
		renderTable.Rows[2].Height = renderObject2.Height;
		renderTable.Cols[1].Width = new Unit(_currentPrintSetting.FinallyWidth, UnitTypeEnum.Pixel);
		renderTable.Height = lhs;
		renderTable.Style.FlowAlign = FlowAlignEnum.Center;
		return renderTable;
	}

	private LineDef ScaleLine(LineDef ld)
	{
		if (PageSetup.HorizontalZoom < 1.0)
		{
			return new LineDef(new Unit(ld.Width.Value * PageSetup.HorizontalZoom, ld.Width.Units), ld.Color);
		}
		return ld;
	}

	private void SetTableFoot()
	{
		if (Table.Foot == null)
		{
			return;
		...
		renderTable.CellStyle.TextIndent = new Unit(1.0, UnitTypeEnum.Point);
		renderTable.Style.GridLines.All = ScaleLine(new LineDef(PageSetup.HasNoteBorder ? Unit.LineWidth : Unit.Empty, Color.Black));
		renderTable.Style.GridLines.ResetTop();
		float num = Table.Foot.Columns.Sum((TableTitleColumn c) => c.Width);
		for (int i = 0; i < Table.Foot.Columns.Count; i++)
		{
			renderTable.Cols[i].Width = new Unit(Table.Foot.Columns[i].Width, UnitTypeEnum.Pixel);
		}
		for (int j = 0; j < Table.Foot.Rows.Count; j++)
		{
			TableTitleRow tableTitleRow = Table.Foot.Rows[j];
			renderTable.Rows[j].Height = new Unit(tableTitleRow.Height, UnitTypeEnum.Pixel);
			for (int k = 0; k < tableTitleRow.Cells.Count; k++)
			{
				TableTitleCell tableTitleCell = tableTitleRow.Cells[k];
				...
				{
					AlignHorzEnum alignHorzEnum = C1PrintDocumentEx.ToAlignHorz(tableTitleCell.Align);
					if (alignHorzEnum == AlignHorzEnum.Right)
					{
						tableCell.CellStyle.Spacing = new Offsets
						{
							Right = new Unit(0.5, UnitTypeEnum.Mm)
						};
					}
					if (alignHorzEnum == AlignHorzEnum.Left)
					{
						tableCell.CellStyle.Spacing = new Offsets
						{
		...
		foreach (TicketMerge merge in Table.Foot.Merges)
		{
			TableCell tableCell2 = renderTable.Rows[merge.TopRow][merge.LeftColumn];
			tableCell2.SpanCols = merge.RightColumn - merge.LeftColumn + 1;
			tableCell2.SpanRows = merge.BottomRow - merge.TopRow + 1;
		...
		renderTable.Height = new Unit(_currentPrintSetting.FooterTableFinallyHeight, UnitTypeEnum.Pixel);
	...
	private void TableConfigFitWidth()
	{
		if (PageSetup.FitPageWidth)
		{
			...
			double num4 = 1.0 + (double)(Table.Columns.Where((Auditai.Model.Column c) => c.Visible).Count() / 7 + 1) / 10.0;
	...
	{
		return new Font(Table.DefaultStyle.FontFamily, (float)((double)Table.DefaultStyle.FontSize.Value * Math.Min(_currentPrintSetting.HScale, _currentPrintSetting.VScale)));
	}

	private double TempHorzZoom()
	{
		double num = new Unit(Table.Columns.Sum((Auditai.Model.Column col) => col.Visible ? col.Width : 0), UnitTypeEnum.Pixel).ConvertUnit(ScreenGraphics.DpiX, UnitTypeEnum.Inch, 0f);
	...
	{
		return new Unit(Table.Columns.Sum((Auditai.Model.Column col) => col.Visible ? col.Width : 0), UnitTypeEnum.Pixel).ConvertUnit(ScreenGraphics.DpiX, UnitTypeEnum.Inch, 0f);
	}
	...
	private double MeasureTableBorderWidth(Auditai.Model.Table table)
	{
		double num = 0.0;
		double num2 = 1.3;
		...
		int num3 = table.Columns.Where((Auditai.Model.Column c) => c.Visible).Count() - 1;
		if (PageSetup.IsPrintIndex)
		{
			num3++;
		}
	...
	private double MeasureTableBorderHeight(Auditai.Model.Table table)
	{
		double num = 0.0;
		double num2 = 1.4;
		TableBorderStyle borderStyle = table.BorderStyle;
	...
	{
		AlignHorzEnum textAlignHorz = C1PrintDocumentEx.ToAlignHorz(titleCell.Align);
		renderStyle.TextAlignHorz = textAlignHorz;
		renderStyle.TextAlignVert = C1PrintDocumentEx.ToAlignVert(titleCell.Align);
		...
		renderStyle.BackColor = DisplayBackColor(titleCell.BackColor);
		renderStyle.FontSize = (float)((double)titleCell.FontSize * Math.Min(_currentPrintSetting.HScale, _currentPrintSetting.VScale));
		renderStyle.FontName = titleCell.FontFamily;
	}

	private void UpdateStyleFromCell(Style renderStyle, Auditai.Model.Cell cell)
	{
		AlignHorzEnum alignHorzEnum = C1PrintDocumentEx.ToAlignHorz(cell.DisplayAlign);
		renderStyle.BackColor = DisplayBackColor(cell.DisplayBackColor);
		renderStyle.TextAlignHorz = alignHorzEnum;
		renderStyle.TextAlignVert = C1PrintDocumentEx.ToAlignVert(cell.DisplayAlign);
		...
		renderStyle.FontSize = Convert.ToSingle((double)cell.DisplayFontSize * Math.Min(_currentPrintSetting.HScale, _currentPrintSetting.VScale));
		renderStyle.Padding.Left = ConvertPixelNumberToUnit(cell.DisplayMargin, UnitTypeEnum.Inch);
		if (alignHorzEnum == AlignHorzEnum.Right)
		{
	...
	private void UpdateStyleFromCellStyle(Style renderStyle, Auditai.Model.CellStyle cellStyle)
	{
		...
		renderStyle.TextAlignHorz = C1PrintDocumentEx.ToAlignHorz(cellStyle.Align.Value);
		renderStyle.TextAlignVert = C1PrintDocumentEx.ToAlignVert(cellStyle.Align.Value);
		renderStyle.FontSize = (float)((double?)cellStyle.FontSize * Math.Min(_currentPrintSetting.HScale, _currentPrintSetting.VScale)).Value;
	}
	...
	public void SetTableStyle(TableBorderStyle tableStyle, RenderTable RenTable, int CaptionRows)
	{
		if (tableStyle != null)
		{
			...
			{
			case LineStyle.Thick:
				RenTable.Style.GridLines.All = ScaleLine(new LineDef("2pt", Color.Black));
				break;
			case LineStyle.Thin:
				RenTable.Style.GridLines.All = ScaleLine(new LineDef("1pt", Color.Black));
				break;
			case LineStyle.Dash:
				RenTable.Style.GridLines.All = ScaleLine(new LineDef("1pt", Color.Black, DashStyle.Dot));
				break;
			case LineStyle.None:
			...
			{
			case LineStyle.Thick:
				style.Borders.Top = ScaleLine(new LineDef("2pt", Color.Black));
				style2.Borders.Bottom = ScaleLine(new LineDef("2pt", Color.Black));
				break;
			case LineStyle.Thin:
				style.Borders.Top = ScaleLine(new LineDef("1pt", Color.Black));
				style2.Borders.Bottom = ScaleLine(new LineDef("1pt", Color.Black));
				break;
			case LineStyle.Dash:
				style.Borders.Top = ScaleLine(new LineDef("1pt", Color.Black, DashStyle.Dot));
				style2.Borders.Bottom = ScaleLine(new LineDef("1pt", Color.Black, DashStyle.Dot));
				break;
			case LineStyle.None:
			...
			{
			case LineStyle.Thick:
				style3.Borders.Bottom = ScaleLine(new LineDef("2pt", Color.Black));
				break;
			case LineStyle.Thin:
				style3.Borders.Bottom = ScaleLine(new LineDef("1pt", Color.Black));
				break;
			case LineStyle.Dash:
				style3.Borders.Bottom = ScaleLine(new LineDef("1pt", Color.Black, DashStyle.Dot));
				break;
			case LineStyle.None:
			...
			{
			case LineStyle.Thick:
				style4.Borders.Left = ScaleLine(new LineDef("2pt", Color.Black));
				style5.Borders.Right = ScaleLine(new LineDef("2pt", Color.Black));
				break;
			case LineStyle.Thin:
				style4.Borders.Left = ScaleLine(new LineDef("1pt", Color.Black));
				style5.Borders.Right = ScaleLine(new LineDef("1pt", Color.Black));
				break;
			case LineStyle.Dash:
				style4.Borders.Left = ScaleLine(new LineDef("1pt", Color.Black, DashStyle.Dot));
				style5.Borders.Right = ScaleLine(new LineDef("1pt", Color.Black, DashStyle.Dot));
				break;
			case LineStyle.None:
	...
	private void ImageCreatePaperImpl()
	{
		...
		double pageWidth = new Unit(GetAvaiableWidth(), _pd.ResolvedUnit).ConvertUnit(ScreenGraphics.DpiX, UnitTypeEnum.Pixel, ScreenGraphics.DpiX);
		double pageHeight = new Unit(GetAvaiableHeight(), _pd.ResolvedUnit).ConvertUnit(ScreenGraphics.DpiY, UnitTypeEnum.Pixel, ScreenGraphics.DpiY);
