...
{
	internal DropCheckBox<Auditai.Model.Column> _dcbGroup;

	internal DropCheckBox<Auditai.Model.Column> _dcbAggregate;
	...
	public frmConsolidateSettings()
	{
		...
		_dcbGroup = new DropCheckBox<Auditai.Model.Column>
		{
			Left = lbl1.Left + lbl1.Width + 20,
			Top = lbl1.Top,
			ValueDisplay = (Auditai.Model.Column c) => c.CaptionDisplay,
			TextDisplay = (IEnumerable<Auditai.Model.Column> iter) => string.Join("|", iter.Select((Auditai.Model.Column c) => c.CaptionDisplay))
		};
		pnlBottom.Controls.Add(_dcbGroup);
		_dcbAggregate = new DropCheckBox<Auditai.Model.Column>
		{
	...
	{
		base.Icon = Auditai.UI.Controls.Theme.SelectedAuditaiTheme.GetThemedIcon(Resources.ConsolidateSettings);
	}
