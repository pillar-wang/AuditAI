...
public class frmLogin : Form
{
	...
	private Color _auditaiMainColor = Color.FromArgb(0, 195, 245);

	private Color _auditaiMainColorButton = Color.FromArgb(0, 195, 245);
	...
	private void SwitchLoginType(LoginType type)
	{
		...
		_loginType = type;
		labelSperator1.BackColor = _auditaiMainColor;
		labelSperator2.BackColor = _auditaiMainColor;
		switch (type)
		{
		...
		try
		{
			...
			ProgressForm<object> progressForm = new ProgressForm<object>(async delegate(IProgress<ProgressInfo> progress)
			{
				...
				if (_loginType == LoginType.LoginBySMS)
				{
					...
					{
						Auditai.DTO.User item2 = (await t).Item2;
				...
				{
					Auditai.DTO.User item = (await t).Item2;
					_loginedUser = new Auditai.Model.User
					{
						Id = item.Id,
						Name = item.Name,
	...
	private async void btnQQLogin_Click(object sender, EventArgs e)
	{
		...
		Tuple<UserToken, Auditai.DTO.User> tuple = qQLoginForm.ShowDialog();
		if (tuple == null)
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "QQ 授权失败");
		}
		else if (tuple.Item1 == null)
		{
			...
			if (frmRegister2.ShowDialog() == DialogResult.OK)
			{
				...
				try
				{
					...
					Tuple<UserToken, Auditai.DTO.User> tuple2 = await WebApiClient.QQRelogin(frmRegister2.UserId, frmRegister2.QQId);
					TokenTimer.Token = tuple2.Item1;
					Auditai.Model.User.Current.TelPhone = tuple2.Item2.Phone;
					if (await GetAndOpenTeam())
					{
						base.DialogResult = DialogResult.OK;
						Close();
			...
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "注册失败 暂不能使用QQ登录");
		}
		else if (tuple.Item1 != null)
		{
			SwitchStatusTo(Status.Logining);
			Auditai.DTO.User item = tuple.Item2;
			_loginedUser = new Auditai.Model.User
			{
				Id = item.Id,
				Name = item.Name,
	...
	private async void btnWechatLogin_Click(object sender, EventArgs e)
	{
		...
		Tuple<UserToken, Auditai.DTO.User> tuple = wechatLogin.ShowDialog();
		if (tuple == null)
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "微信登录授权失败");
		}
		else if (tuple.Item1 == null)
		{
			frmRegister frmRegister2 = new frmRegister(thirdLogin: true);
			...
			if (frmRegister2.ShowDialog() == DialogResult.OK)
			{
				...
				try
				{
					...
					TokenTimer.LoginInfo = loginInfo;
					Tuple<UserToken, Auditai.DTO.User> tuple2 = await WebApiClient.WechatRelogin(frmRegister2.UserId, frmRegister2.WechatId);
					TokenTimer.Token = tuple2.Item1;
					Auditai.Model.User.Current.TelPhone = tuple2.Item2.Phone;
					if (await GetAndOpenTeam())
					{
			...
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "注册失败 暂不能使用微信登录");
		}
		else if (tuple.Item1 != null)
		{
			SwitchStatusTo(Status.Logining);
	...
	private void lblHelpDoc_Click(object sender, EventArgs e)
	{
		...
		{
			string url = "";
			Auditai.UI.Controls.Util.ShellExecuteUrl(url);
		}
		catch (Exception ex)
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, ex.Message);
	...
	private void InitializeComponent()
	{
		...
		this.btnWechatLogin.BackgroundImage = Auditai.UI.Platform.Properties.Resources.wechat_img;
		...
		this.btnQQLogin.BackgroundImage = Auditai.UI.Platform.Properties.Resources.qq_img;
		...
		this.BackgroundImage = Auditai.UI.Platform.Properties.Resources.loginpicture1;
