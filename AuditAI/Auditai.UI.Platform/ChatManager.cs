...
public static class ChatManager
{
	...
	public static async Task Login()
	{
		...
		SignalRClient.UserState.TeamId = Auditai.Model.User.Current.TeamId.ToString();
		await SignalRClient.Login(Auditai.Model.User.Current.Id.ToString(), SignalRClient.UserState);
	...
	public static async Task UpdateTeamMember()
	{
		...
		try
		{
			IEnumerable<Auditai.DTO.User> enumerable;
			if (Auditai.LocalDataStore.StorageRouter.IsLocalMode)
			...
			lock (lock1)
			{
				...
				if (!enumerable.Any((Auditai.DTO.User u) => u.Id == Auditai.Model.User.Current.Id))
				{
					Auditai.Model.User.Current.TeamId = Guid.Empty;
					Auditai.Model.User.Current.IsTeamAdmin = false;
					Auditai.Model.User.Current.LicenseDate = DateTime.MaxValue;
					return;
				}
				foreach (Auditai.DTO.User item in enumerable)
				{
		...
		{
			Group group2 = memberManager.GetGroup(Auditai.Model.User.Current.TeamId.ToString());
			if (group2 == null)
			{
				group2 = new Group
				{
			...
			{
				group2.Id = Auditai.Model.User.Current.TeamId.ToString();
				group2.Clear();
				memberManager.AddGroup(group2);
			}
