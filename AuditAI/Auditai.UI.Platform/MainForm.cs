...
public class MainForm
{
	...
	{
		public Auditai.DTO.Project Dto { get; set; }
	...
	internal static Dictionary<Guid, Auditai.Model.Project> RecentProjects { get; } = new Dictionary<Guid, Auditai.Model.Project>();
	...
	public Dictionary<Auditai.Model.Document, DocumentEditor> DocumentEditors { get; } = new Dictionary<Auditai.Model.Document, DocumentEditor>();
	...
	public Auditai.Model.Project CurrentProject
	{
		get
		{
			return Auditai.Model.Project.Current;
		}
		set
		{
			...
			Auditai.Model.Project.Current = value;
	...
	public Auditai.Model.Table CurrentTable => TableEditor.Table;
	...
	public Dictionary<Auditai.Model.TreeGroup, int> GroupValidationResults { get; } = new Dictionary<Auditai.Model.TreeGroup, int>();
	...
	private void MainForm_MemberInfoChanged(object sender, string e)
	{
		...
		if (group != null)
		{
			CurrentProject.Users = (from m in @group.Members()
				select m.ToDto()).ToDictionary((Auditai.DTO.User u) => u, (Auditai.DTO.User u) => u.Role);
		}
	}
			...
			try
			{
				...
				progressForm.ShowDialog(progressRuntimeData, async delegate
				{
					...
					if (Auditai.LocalDataStore.StorageRouter.IsLocalMode)
					{
						...
						if (dto == null)
						{
							dto = await Auditai.LocalDataStore.StorageRouter.GetTemplateById(id);
						}
					}
					else
					{
						...
						{
							isReturnNull = true;
							Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "打开" + StringConstBase.Current.Project + "过程中发生异常，请重新尝试。");
						}
						else
						{
	...
	private async Task<Tuple<int, int>> OpenProjectImpl(Guid projectId)
	{
		try
		{
			return await Auditai.LocalDataStore.StorageRouter.OpenProject(projectId);
		}
		catch (Exception ex)
		{
	...
	public async Task<Auditai.Model.Project> OpenProjectDb_DownloadIfNotExist(Auditai.DTO.Project dto, ProgressForm2 progressForm = null)
	{
		if (progressForm == null)
		{
			...
			Auditai.Model.Project ret = null;
			progressForm.ShowDialog(progressRuntimeData, async delegate
			{
				await Task.Delay(1).ConfigureAwait(continueOnCapturedContext: false);
		...
		async Task<Auditai.Model.Project> LoadProjectFileImpl(TaskProgressValueUpdater progressUpdater)
		{
			...
			try
			{
				...
				if (Auditai.LocalDataStore.StorageRouter.IsLocalMode && dto.Type == ProjectType.Template)
				{
					string templatePath = Auditai.LocalDataStore.LocalDataStore.GetTemplateDbPath(dto.Id);
				...
				else
				{
					try
					{
						...
						if (Auditai.LocalDataStore.StorageRouter.IsLocalMode)
						{
							...
							ret2.Dal = new ProjectDAL(fileName);
							ret2.Dal.SaveProject(new Auditai.DTO.Project
							{
								Id = dto.Id,
								Name = dto.Name,
								ParentId = dto.Id,
			...
			{
				Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, ex6.Message);
				return null;
			}
		}
	}
	...
	{
		return Path.Combine("data", Auditai.Model.User.Current.Id.ToString(), $"{id}.db");
	}

	public void OpenTable()
	{
		Auditai.Model.Table table = (ProjectHierarchy.SelectedNode as TreeTableNode).Table;
	...
	public bool IsAllowToDesignTicket(Auditai.Model.Table table)
	{
		if (!table.TreeNode.HasWritePermission())
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "您没有表格的编辑权限，因此也不具备设计表单的权限!");
			return false;
		}
		if (!table.TreeNode.HasSchemaPermission())
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "您没有表格的结构调整权限，因此也不具备设计表单的权限!");
			return false;
		}
		if (table.RowOwnerExclusive || table.RowOwnerLoad)
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "增行独占保护模式下，禁止对表单样式进行修改!");
			return false;
		}
		return true;
	}
	...
	public void SwitchToTicketPrintView(Auditai.Model.Table table = null)
	{
		HideAllPnlContent();
		_isPreview = true;
		if (table == null)
	...
	public void ExportExcelDialog()
	{
		...
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "因文件被占用或其他原因，导出失败。");
		}
		catch (Exception ex2)
		{
	...
	public void TableExportPdf()
	{
		...
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "文件导出失败，失败原因: " + ex.Message);
		}
		catch (Exception ex2)
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "文件导出失败，失败原因: " + ex2.Message);
		}
	}
	...
	protected async Task ExportTableAttachmentsAsync(Auditai.Model.Table table, string tableFilePath)
	{
	...
	public void PopulateRecents()
	{
		...
		foreach (Auditai.Model.Project proj in RecentProjects.Values)
		{
			System.Drawing.Image largeImage;
			RibbonGroup ribbonGroup;
			if (proj.Kind == ProjectType.Project)
			{
				largeImage = ((Auditai.Model.Project.Current != proj) ? CurrentEdition.ProjectTileIcon : CurrentEdition.CurrentProjectIcon);
				ribbonGroup = AppCommandGroups.RecentProjects.RibbonGroup;
			}
			else
			{
				largeImage = (proj.SystemBuild ? ((Auditai.Model.Project.Current != proj) ? CurrentEdition.SystemTemplateTileIcon : CurrentEdition.CurrentSystemTemplateIcon) : ((Auditai.Model.Project.Current != proj) ? CurrentEdition.CustomTemplateTileIcon : CurrentEdition.CurrentCustomTemplateIcon));
	...
	public async Task CloseProject(Auditai.Model.Project p)
	{
	...
	public async void RemoveNodes()
	{
		...
		if (form.ShowRemoveNodes() != DialogResult.OK || Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.Exclamation, "您正在执行批量删除操作，会将您选定的文件夹、子文件夹、表格、文档等一并删除，请谨慎操作，确定要执行删除操作吗？", MessageBoxButtons.OKCancel) != DialogResult.OK)
		{
			return;
		}
	...
	public async Task SaveProjects()
	{
		...
		{
			await Task.Delay(1).ConfigureAwait(continueOnCapturedContext: false);
			foreach (Auditai.Model.Project value in RecentProjects.Values)
			{
	...
	public async Task SaveProject(Auditai.Model.Project proj)
	{
	...
	private async Task SaveProjectImpl(Auditai.Model.Project proj, TaskProgressValueUpdater progressUpdater = null)
	{
		if (progressUpdater == null)
		{
			progressUpdater = new TaskProgressValueUpdater(0f, 1f, null);
		}
		List<Auditai.Model.Table> tables = (from n in proj.GetAllTableNodes()
			select n.Table into t
			where t.NeedSave
			select t).ToList();
		int totalCount = tables.Count + DocumentEditors.Where((KeyValuePair<Auditai.Model.Document, DocumentEditor> d) => d.Value.NeedSave).Count() + (from u in proj.GetAllDocumentNodes()
		...
		{
			Auditai.Model.Document document = allDocumentNode.Document;
		...
		List<Auditai.Model.Image> list = (from n in proj.GetAllImageNodes()
			select n.Image into i
			where i.NeedSave
			select i).ToList();
		foreach (Auditai.Model.Image item in list)
		{
			item.Save();
			item.TreeNode.IsEntityDirty = true;
		}
		List<Auditai.Model.Pdf> list2 = (from n in proj.GetAllPdfNodes()
			select n.Pdf.LoadAndReturn()).ToList();
		foreach (Auditai.Model.Pdf item2 in list2)
		{
			item2.Save();
		}
		proj.Save();
	...
	public async Task SyncProjects()
	{
		...
		if (TicketInputEditor != null && CurrentView == MainFormView.TicketInput)
		{
			...
			{
				ex.Log();
				Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "表单内容保存失败，请重新保存表单内容, 失败原因描述: " + ex.Message);
				return;
			}
		}
		TreeNodeBase previousNode = ProjectHierarchy.SelectedNode;
		bool anyNodeUpdated = false;
		List<Auditai.Model.Project> projectList = new List<Auditai.Model.Project>();
		foreach (Auditai.Model.Project value in RecentProjects.Values)
		{
		...
		try
		{
			...
			{
				await Task.Delay(1).ConfigureAwait(continueOnCapturedContext: false);
				foreach (Auditai.Model.Project proj in projectList)
				{
					try
					{
		...
		catch (HttpRequestException ex2)
		{
			...
			{
				Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "因网络问题，可能数据未完全同步成功，请再同步一次！");
			}
			else
			{
	...
	private async Task<bool> SyncProjectImpl(Auditai.Model.Project proj, ProgressRuntimeData progressRuntimeData = null, ProgressForm2 progressForm = null)
	{
		// 本地模式下跳过同步
		if (Auditai.LocalDataStore.StorageRouter.IsLocalMode)
		{
		...
		{
			Auditai.Model.Image image = images[l];
			...
			if (image.TreeNode.IsEntityDirty)
			{
				...
				{
					Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "要上传的图片文件已在本地被删除，无法上传图片。图片节点 " + image.TreeNode.Name + " 已经无效，请删除此节点。");
				}
	...
	public async Task TicketCollectSet(TicketCollectFillTable ticket)
	{
		try
		{
			if (ticket.IsLocked)
			{
				Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "表格已锁定，不能执行采账填充");
				return;
			}
			if (IsLedgerEmpty())
			{
			...
			if (!Auditai.LocalDataStore.StorageRouter.IsLocalMode && DictionarySync.TableCollector.Version == 0)
			{
				...
				catch (WebException)
				{
					...
					{
						Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "因网络问题，字典更新失败！");
					}
				}
				catch (TimeoutException)
				{
					if (DictionarySync.TableCollector.Version == 0)
					{
						Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "更新字典失败！网络超时,请重试");
					}
				}
				catch (Exception ex3)
				{
					if (DictionarySync.TableCollector.Version == 0)
					{
						Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "更新字典失败！" + ex3.Message + ",请重试");
					}
				}
			}
			if (ticket == null)
			{
				Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "请先打开表格");
				return;
			}
		...
		catch (Exception ex7)
		{
			...
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, ex7.Message);
		}
	}
	...
	public async Task TableCollectSet()
	{
		...
		try
		{
			...
			{
				Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "表格已锁定，不能执行采账设置");
				return;
			}
			if (TableEditor.Table != null && TableEditor.Table.TreeNode != null)
			{
				...
				{
					Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "当前表格没有查看权限，不能执行采账设置");
					return;
				}
				if (!TableEditor.Table.TreeNode.HasWritePermission())
				{
					Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "当前表格没有编辑权限，不能执行采账设置");
					return;
				}
			}
			if (IsLedgerEmpty())
			...
			if (!Auditai.LocalDataStore.StorageRouter.IsLocalMode && DictionarySync.CellCollector.Version == 0)
			{
				...
				catch (Exception ex6)
				{
					if (DictionarySync.CellCollector.Version == 0)
					{
						Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "更新字典失败！" + ex6.Message + ",请重试");
					}
				}
			}
			Auditai.Model.Table table = TableEditor.Table;
			if (table == null)
			{
				Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "请先打开表格");
				return;
		...
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, ex8.Message);
		}
		catch (OperationCanceledException)
		{
		}
		catch (Exception ex10)
		{
			...
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, ex10.Message);
		}
	}
	...
	public void CellCollectSet()
	{
		try
		{
			...
			if (TableEditor.Table != null && TableEditor.Table.TreeNode != null)
			{
				if (!TableEditor.Table.TreeNode.HasReadPermission())
				{
					Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "没有表格的查看权限，不能执行采账设置");
					return;
				}
				if (!TableEditor.Table.TreeNode.HasWritePermission())
				{
					Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "没有表格的编辑权限，不能执行采账设置");
					return;
				}
			}
			if (IsLedgerEmpty())
	...
	public async Task OneClickCollect()
	{
		...
		ProgressForm<object> pf = new ProgressForm<object>(async delegate(IProgress<ProgressInfo> iProg)
		{
			for (int j = 0; j < count; j++)
			{
				...
				await Task.Delay(1);
				Auditai.Model.Table table = tableNodes[j].Table.LoadAndReturn();
	...
	public async Task AutoImport()
	{
		...
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, ex11.Message);
		}
	}
	...
	private void CellsCollect(Ledger ledger, Auditai.Model.Table table)
	{
		try
		{
	...
	public void FillEditorTable(TableCollectResult collectResult, Auditai.Model.Table table = null)
	{
		...
		Auditai.Model.Row row = table.Rows.FirstOrDefault((Auditai.Model.Row r) => r.Role == RowRole.Normal || r.Role == RowRole.Minus || r.Role == RowRole.Among);
		upperBoundRow = ((row == null) ? (table.Rows.Count - 1) : (row.Index - 1));
		lowerBoundRow = table.Rows.Skip(upperBoundRow + 1).FirstOrDefault((Auditai.Model.Row r) => r.Role == RowRole.Fixed || r.Role == RowRole.Header || r.Role == RowRole.Subtotal || r.Role == RowRole.Total)?.Index ?? table.Rows.Count;
		List<int> list = null;
		...
		List<Auditai.Model.Column> mapCols;
		List<Auditai.Model.Column> decimalCols;
		int j;
		if (tableCollectorBalance == null)
		{
			if (!(tableCollector is TableCollectorSubsidiary tableCollectorSubsidiary))
			{
				...
				if (tableCollectorSummary != null)
				{
					...
					Auditai.Model.Column byId = table.Columns.GetById(new Id64(key));
					if (byId != null)
					{
						...
						if (collectResult.Values.ContainsKey(byId))
						{
							...
							List<Auditai.Model.Cell> list3 = (from c in byId.GetCells()
								where c.Row.Index > upperBoundRow && c.Row.Index < lowerBoundRow
								select c).ToList();
							int num = list3.LastOrDefault((Auditai.Model.Cell c) => !string.IsNullOrWhiteSpace(c.GetDisplayValue()) && c.Row.Role == RowRole.Normal)?.Row.Index ?? upperBoundRow;
							for (int k = 0; k < list.Count; k++)
							{
								...
								Auditai.Model.Cell cell = list3.Find((Auditai.Model.Cell c) => data2.Equals(c.Value));
								if (cell == null)
								{
									num = (list[k] = num + 1);
								}
			...
			else
			{
				...
				Func<int> func = delegate
				{
					...
					{
						foreach (Auditai.Model.Column item in mapCols)
						{
		...
		else
		{
			...
			Auditai.Model.Column byId2 = table.Columns.GetById(new Id64(key2));
			if (byId2 != null)
			{
				list = Enumerable.Range(0, collectResult.Values.First().Value.Count).ToList();
				if (collectResult.Values.ContainsKey(byId2))
				{
					...
					List<Auditai.Model.Cell> list5 = (from c in byId2.GetCells()
						where c.Row.Index > upperBoundRow && c.Row.Index < lowerBoundRow
						select c).ToList();
					int num6 = list5.LastOrDefault((Auditai.Model.Cell c) => !string.IsNullOrWhiteSpace(c.GetDisplayValue()) && c.Row.Role == RowRole.Normal)?.Row.Index ?? upperBoundRow;
					for (int l = 0; l < list.Count; l++)
					{
						...
						Auditai.Model.Cell cell2 = list5.Find((Auditai.Model.Cell c) => data.Equals(c.Value));
						if (cell2 == null)
						{
							num6 = (list[l] = num6 + 1);
						}
		...
		try
		{
			...
			foreach (KeyValuePair<Auditai.Model.Column, List<object>> value2 in collectResult.Values)
			{
				...
				{
					int row2 = list[num9];
					Auditai.Model.Cell cell3 = table[row2, index];
		...
		bool CanFillBalance()
		{
			...
			{
				Auditai.Model.Column byId3 = table.Columns.GetById(new Id64(kv.Key));
		...
		bool CanFillSummary()
		{
			...
			{
				Auditai.Model.Column byId4 = table.Columns.GetById(new Id64(kv.Key));
		...
		{
			foreach (Auditai.Model.Column item2 in mapCols)
			{
	...
	public void ConfirmationSetting()
	{
		...
		instance.AfterSelected += delegate(object s, Auditai.Model.Column e)
		{
	...
	public async Task GenerateConfirmationFromDocument()
	{
		...
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "请先进行函证设置");
			return;
		}
		Auditai.Model.Table tableById = document.Project.GetTableById(new Id64(tableId));
		if (tableById == null)
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "未找到当前绑定表，请重新进行设置");
			return;
		}
		List<Dictionary<string, Auditai.Model.Cell>> dataTable = getDataTable(tableById);
		if (dataTable.Count >= 100)
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "一次最多生成一百张凭证！");
			return;
		}
		...
		static List<Dictionary<string, Auditai.Model.Cell>> getDataTable(Auditai.Model.Table ctable)
		{
			List<Dictionary<string, Auditai.Model.Cell>> list = new List<Dictionary<string, Auditai.Model.Cell>>();
			foreach (Auditai.Model.Row row in ctable.Rows)
			{
				if (row.Role == RowRole.Normal || row.Role == RowRole.Among || row.Role == RowRole.Minus)
				{
					Dictionary<string, Auditai.Model.Cell> dictionary = new Dictionary<string, Auditai.Model.Cell>();
					foreach (Auditai.Model.Column column in ctable.Columns)
					{
	...
	public async void AlterInfo()
	{
		...
		if (frmAlterInfo2.ShowDialog() == DialogResult.OK)
		{
			...
			{
				Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "因您修改了用户名，需要重新登录");
				await Exit();
			}
			else
			{
				Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "更新成功");
			}
		}
	}
	...
	public void AlterPwd()
	{
		if (Auditai.LocalDataStore.StorageRouter.IsLocalMode)
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "本地模式不支持修改密码");
			return;
		}
		LoginMode loginMode = TokenTimer.LoginInfo.LoginMode;
		if ((uint)(loginMode - 1) <= 1u)
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "第三方账号登录不支持修改密码操作！");
			return;
		}
	...
	{
		Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "程序更新已禁用");
	...
	public void LedgerPrint()
	{
		...
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, ex.ParamName);
		}
		catch (Exception ex2)
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, ex2.Message);
	...
	{
		string path = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "AuditAI采数器");
		FolderBrowserDialog folderBrowserDialog = new FolderBrowserDialog();
		folderBrowserDialog.Description = "请选择生成采数器的文件夹位置：";
		if (folderBrowserDialog.ShowDialog() == DialogResult.OK)
		{
			string text = Path.Combine(folderBrowserDialog.SelectedPath, "AuditAI采数器");
			try
			{
				...
				Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "采数器生成完毕");
				Process.Start(text);
			}
			catch (Exception ex)
			{
	...
	public async Task ValidateAll()
	{
		...
		ProgressForm<object> progressForm = new ProgressForm<object>(async delegate(IProgress<ProgressInfo> progress)
		{
			...
			{
				Auditai.Model.ValidationFormula vf = CurrentProject.ValidationManager.Formulas[i];
		...
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "恭喜您，校验已完成，未发现错误！");
		}
		else
		{
	...
	{
		if (!CurrentProject.ValidationManager.Formulas.Where((Auditai.Model.ValidationFormula u) => u.TableId == CurrentTable.Id).Any())
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "当前表格未设置校验公式！");
			return;
		}
		...
		try
		{
			foreach (Auditai.Model.ValidationFormula formula in CurrentProject.ValidationManager.Formulas)
			{
		...
		foreach (Id64 item2 in ValidationResultCache.Keys.Where((Id64 k) => !CurrentProject.ValidationManager.Formulas.Exists((Auditai.Model.ValidationFormula vf) => vf.Id == k)).ToList())
		{
			ValidationResultCache.Remove(item2);
		}
		foreach (Auditai.Model.ValidationFormula formula2 in CurrentProject.ValidationManager.Formulas)
		{
			try
			{
	...
	public void ValidateOne(Auditai.Model.ValidationFormula vf)
	{
	...
	public void ShowFunctionHint(string formulaText, int pos)
	{
		try
		{
			...
			AppCommands.FormulaTip1.Label.Text = "函数语法：" + functionInfo.Name + "(" + string.Join(", ", functionInfo.Parameters.Select((Auditai.Model.ParameterInfo p) => p.Name)) + ")";
			AppCommands.FormulaTip2.Label.Text = "函数功能：" + functionInfo.Description;
			AppCommands.FormulaTip3.Label.Text = "参数说明：" + string.Join("；", functionInfo.Parameters.Select((Auditai.Model.ParameterInfo p) => p.Name + "：" + p.Description));
		}
		catch (FormulaException)
		{
			DisplayEditingFormula();
	...
	public void ToggleSideToolbar()
	{
		if (_showSideToolbar)
		{
			...
			foreach (KeyValuePair<Auditai.Model.Document, DocumentEditor> documentEditor in DocumentEditors)
			{
		...
		else
		{
			...
			foreach (KeyValuePair<Auditai.Model.Document, DocumentEditor> documentEditor2 in DocumentEditors)
			{
	...
	public bool CanAccessControl()
	{
		...
		if (!CurrentProject.Users.Any((KeyValuePair<Auditai.DTO.User, UserRole> kv) => kv.Key.Id == Auditai.Model.User.Current.Id))
		{
			return false;
		}
		if (CurrentProject.Users.First((KeyValuePair<Auditai.DTO.User, UserRole> kv) => kv.Key.Id == Auditai.Model.User.Current.Id).Value == UserRole.Manager)
		{
			return true;
		}
		return false;
	...
	public async Task RevertTableDialog()
	{
		...
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "您没有该表格的编辑权限。");
			return;
		}
		int latestVersion = 0;
		Auditai.Model.Table clone;
		int version;
		if (TableEditor.Table.NeedSave || TableEditor.Table.TreeNode.IsEntityDirty)
		{
			clone = new Auditai.Model.Table
			{
		...
		try
		{
			PullTable pullTable = await Auditai.LocalDataStore.StorageRouter.PullTable(request);
		...
		catch (HttpRequestException ex)
		{
			...
			{
				Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "发生了未预期的异常:\r\n" + ex.ToString());
			}
			return;
		}
	...
	public async Task RevertDocumentDialog()
	{
		...
		if (!ProjectHierarchy.HasWritePermission())
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "您没有该文档的编辑权限。");
			return;
		}
		if (CurrentDocumentEditor.Document.TreeNode.IsEntityDirty)
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "当前文件未同步，请同步后再使用历史版本功能！");
			return;
		}
		FormTimelineViewer form = new FormTimelineViewer();
		Auditai.Model.Document clone = CurrentDocumentEditor.Document.TemporaryClone();
		JObject request = JObject.FromObject(new
		{
			Action = "PullDocument",
			Id = clone.Id.Value,
		...
		try
		{
			PullDocument pullDocument = await Auditai.LocalDataStore.StorageRouter.PullDocument(request);
	...
	public void CloseDocument(Auditai.Model.Document d)
	{
	...
	public void Print()
	{
		...
		case MainFormView.PdfPreview:
			...
			{
				Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "打印失败，pdf文件异常");
				break;
			}
		case MainFormView.Table:
		case MainFormView.TablePreview:
		...
		case MainFormView.ImagePreview:
		{
			Auditai.Model.Image image = (ProjectHierarchy.SelectedNode as TreeImageNode)?.Image;
			if (image != null)
			{
				Preview.Image = image;
				Preview.PrintDialog();
	...
	public async void ExportPdfDocumentDialog()
	{
		...
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "因文件被占用或其他原因，导出失败。错误信息：" + ex2.Message);
		}
	}
	...
	public void SetShowHelperTooltipAndRefresh(bool show)
	{
		...
		if (show)
		{
			foreach (Auditai.Model.TreeGroup treeGroup in CurrentProject.TreeGroups)
			{
				int num = 0;
	...
	{
		string exitMsg = Auditai.LocalDataStore.StorageRouter.IsLocalMode
		...
		switch (Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, exitMsg, MessageBoxButtons.YesNoCancel))
		{
		case DialogResult.Yes:
			await SyncProjects();
			break;
	...
	private async void MultiLedgerViewer_AfterCollect(object sender, LedgerCollectEventArgs e)
	{
		...
		try
		{
			...
			{
				Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "未能智能识别出填充的底稿的填充列，请检查该科目底稿列名设置是否正确。");
			}
		}
	...
	private void cmdDuplicateDocument_Click(object sender, ClickEventArgs e)
	{
		...
		if (!ProjectHierarchy.HasWritePermission())
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "您没有编辑权限，无法复制文档。");
			return;
		}
		try
		{
		...
		{
			ex.Log();
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.Hand, "复制文档失败:\r\n" + ex.Message, MessageBoxButtons.OK, "错误!");
		}
	}
					...
					if (treeImageNode == null)
					{
						...
						if (treePdfNode != null)
						{
							...
							if (!CurrentProject.FileCacheManager.Exists(treePdfNode.Pdf.FileId))
							{
								...
								{
									Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "下载过程中出错，请稍候重试");
									return;
								}
							}
	...
	public void OpendLedger_Click()
	{
		...
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "打开失败！文件损坏或其他异常");
		}
	}
	...
	public void ShareClick(C1CommandLink shareLink, string file)
	{
		...
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "账套文件不存在！");
			return;
		}
		...
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "项目中没有在线成员！");
			return;
		}
	...
	public C1CommandLink[] GetOnlineMemberContextMenu(string file)
	{
		...
		foreach (MemTab member in enumerable)
		{
			...
			c1Command.Click += async delegate
			{
				if (!File.Exists(file))
				{
					Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "该账套文件不存在！");
				}
				else
				{
		...
		static async Task PushFileImpl(MemTab toMember, string file2)
		{
			...
			try
			{
				...
				NotifyMessage notifyMessage = new NotifyMessage
				{
					...
					Text = MemberManager.GetInstance().GetMember(Auditai.Model.User.Current.Id.ToString()).Name + "向您发送来 " + Path.GetFileName(file2)
				};
	...
	public async Task BatchExport(string functionDescription)
	{
		...
		{
			if (Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "选择导出的文件需保存后方可导出，需要保存后导出吗？", MessageBoxButtons.OKCancel) != DialogResult.OK)
			{
				return;
			}
				...
				if (frm.Grid.GetCellCheck(node.Row.Index, 1) == CheckEnum.Checked)
				{
					...
					try
					{
						...
						if (!(key is frmNodeSelectorWithTicketRecord.TicketNavTreePageNode ticketNavTreePageNode))
						{
							...
							{
								if (!(key is Auditai.Model.TreeGroup treeGroup))
								{
					...
					catch (Exception ex2)
					{
						...
						Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, text + "导出失败！失败原因：" + ex2.Message);
					}
				}
				if (frm.SameDirExcelSaveToOneFile)
				{
					...
					if (source.Count() > 0)
					{
						...
						catch (Exception ex4)
						{
							...
							Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, text2 + "导出失败！失败原因：" + ex4.Message);
						}
					}
					...
					if (source2.Count() > 0)
					{
						_ = string.Empty;
						string text3;
						if (node.Key is Auditai.Model.TreeGroup treeGroup2)
						{
						...
						try
						{
							...
							List<Tuple<Auditai.Model.Table, PageSetup>> list = source2.Select((TreeTableNode t) => Tuple.Create(t.Table, t.Table.PageSetup)).ToList();
							...
							foreach (Tuple<Auditai.Model.Table, PageSetup> item2 in list)
							{
						...
						catch (Exception ex6)
						{
							...
							Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, text4 + "导出失败！失败原因：" + ex6.Message);
						}
					}
	...
	private void ThemeEditor_SelectedThemeChanged(object sender, AuditaiTheme theme)
	{
		if (theme == null)
		{
			return;
	...
	private void UpdateTableValidationCache()
	{
		...
		foreach (ValidationResult item in enumerable)
		{
			...
			foreach (Auditai.Model.Cell cellReference in item.Refs.CellReferences)
			{
				...
				bool flag = false;
				foreach (Tuple<Auditai.Model.Cell, ValidationResult> cell3 in tableValidationInfo.Cells)
				{
			...
			foreach (Auditai.Model.Column columnReference in item.Refs.ColumnReferences)
			{
				if (item.HasWildcard)
				{
					continue;
			...
			foreach (Auditai.Model.Column columnWildcardReference in item.Refs.ColumnWildcardReferences)
			{
				...
				{
					Auditai.Model.Cell cell = columnWildcardReference.Table[item.RowIndex, columnWildcardReference.Index];
			...
			foreach (Auditai.Model.Cell headerCellWildcardReference in item.Refs.HeaderCellWildcardReferences)
			{
				Auditai.Model.Table table = headerCellWildcardReference.Column.Table;
				...
				{
					Auditai.Model.Cell cell2 = table[item.RowIndex, headerCellWildcardReference.Column.Index];
			...
			foreach (Auditai.Model.Cell headerCellReference in item.Refs.HeaderCellReferences)
			{
				if (item.HasWildcard)
				{
					continue;
			...
			foreach (Tuple<Auditai.Model.Table, int, int> titleReference in item.Refs.TitleReferences)
			{
			...
			foreach (Tuple<Auditai.Model.Table, int, int> footReference in item.Refs.FootReferences)
			{
	...
	public void NodesIndexEdit()
	{
		...
		formNodesIndexEdit.Project = Auditai.Model.Project.Current;
	...
	private async Task ChangeProject()
	{
		...
		if (ChatManager.ChatForm.IsValueCreated)
		{
			...
			Group group = manager.GetGroup(Auditai.Model.User.Current.TeamId.ToString());
	...
	{
		if (!(errorRef is Auditai.Model.Cell cell))
		{
			if (!(errorRef is RangeOperand rangeOperand))
			{
				if (errorRef is Auditai.Model.Column column)
				{
	...
	private async Task SkipToTreeNode(Tuple<Guid, long> tp)
	{
		...
		Auditai.Model.Project project = await OpenOrSwitchToProject(tp.Item1);
		if (project == null)
		{
			return;
		}
		...
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "数据源文件不存在");
			return;
		}
		try
		{
	...
	private void ShowRelatedLedgerTip(Auditai.Model.Project project)
	{
	...
	public async Task ExecuteCustomFill()
	{
		...
		if (project?.Dal == null)
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "请先打开项目");
			return;
		}
		...
		// 按表格分组收集更新，便于批量执行和撤销
		var tableUpdates = new Dictionary<Auditai.Model.Table, List<Tuple<Auditai.Model.Cell, object>>>();
		...
		foreach (var rule in config.Rules)
		{
			try
			{
				...
				{
					updates = new List<Tuple<Auditai.Model.Cell, object>>();
		...
		Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, summary);
	}

	/// <summary>替换填充值中的占位符</summary>
	private static string ReplacePlaceholders(string value, Auditai.Model.Table table, int row, int col, int seq)
	{
