...
public class dlgProjectEditor : C1RibbonForm
{
	...
	private List<Auditai.DTO.Project> templates;
	...
	internal async Task PopulateProject()
	{
		...
		try
		{
			...
			if (Project.ParentId.HasValue)
			{
				...
				{
					if (item2.Tag is Auditai.DTO.Project project && project.Id == Project.ParentId.Value)
					{
	...
	private async Task<bool> ValidateProject()
	{
		...
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, StringConstBase.Current.Project + "编号为必填项，请填写！");
			return false;
		}
		if (string.IsNullOrWhiteSpace(txtName.Text))
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, StringConstBase.Current.Project + "名称为必填项，请填写！");
			return false;
		}
		if (string.IsNullOrWhiteSpace(txtAuditee.Text))
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, StringConstBase.Current.Auditee + "为必填项，请填写！");
			return false;
		}
		IEnumerable<Auditai.DTO.User> users = (IEnumerable<Auditai.DTO.User>)((_ListOrTile == ListTileViewMode.Tile) ? projectUsersTileSelector.ValidateAndGetUsers() : projectUsersListSelector.ValidateAndGetUsers());
		if (users == null)
		{
		...
		if (num < 0)
		{
			...
			{
				if (Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.Exclamation, "您未选择使用" + StringConstBase.Current.Template + "，这将会为您创建一个无任何文件的空" + StringConstBase.Current.Project + "，确定要创建一个空" + StringConstBase.Current.Project + "吗？", MessageBoxButtons.OKCancel) != DialogResult.OK)
				{
					return false;
				}
		...
		{
			Project.ParentId = ((Auditai.DTO.Project)((InputComponent)cboParent.SelectedItem).Tag).Id;
		}
		else
		{
	...
	private async Task PopulateCreate()
	{
		...
		try
		{
			...
			foreach (Auditai.DTO.Project template in templates)
				{
