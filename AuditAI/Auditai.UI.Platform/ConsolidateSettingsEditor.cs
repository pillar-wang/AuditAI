...
public class ConsolidateSettingsEditor
{
	private class ProjectTreeColumnEditor : Control, IC1EmbeddedEditor
	{
		...
		public void SetProject(Auditai.Model.Project p)
		{
			_editor.Project = p;
			_editor.Populate();
			SetHeight();
	...
	private class MultiColumnEditor : CheckedListBox, IC1EmbeddedEditor
	{
		...
		public object C1EditorGetValue()
		{
			List<KeyValuePair<Auditai.DTO.Column, bool>> list = new List<KeyValuePair<Auditai.DTO.Column, bool>>();
			for (int i = 0; i < base.Items.Count; i++)
			{
				list.Add(new KeyValuePair<Auditai.DTO.Column, bool>((Auditai.DTO.Column)base.Items[i], GetItemChecked(i)));
			}
			return list;
		}
		...
		{
			if (!(value is IEnumerable<KeyValuePair<Auditai.DTO.Column, bool>> enumerable))
			{
				return;
			}
			base.Items.Clear();
			foreach (KeyValuePair<Auditai.DTO.Column, bool> item in enumerable)
			{
	...
	private class ProjectSelectorEditor : C1FlexGrid, IC1EmbeddedEditor
	{
		...
		public void Initialize()
		{
			...
			void AddProject(Auditai.Model.Project p, Node parentNode)
			{
				Node node = ((parentNode != null) ? parentNode.AddNode(NodeTypeEnum.LastChild, null) : base.Rows.AddNode(0));
				node.Key = p;
				node.Data = p.Name;
				node.Row.UserData = p;
				foreach (Auditai.Model.Project item in _owner._projects.Where((Auditai.Model.Project c) => c.ParentId == p.Id))
				{
	...
	private List<Auditai.Model.Project> _projects;

	private C1FlexGrid _grid => _form._grid;

	private DropCheckBox<Auditai.Model.Column> _dcbGroup => _form._dcbGroup;

	private DropCheckBox<Auditai.Model.Column> _dcbAggregate => _form._dcbAggregate;
	...
	public static async Task<List<Auditai.DTO.Column>> GetTableColumns(Auditai.Model.Table table)
	{
		try
		{
	...
	public async Task Populate()
	{
		...
		{
			Auditai.Model.Project project2 = _projects.FirstOrDefault((Auditai.Model.Project p) => p.Id == source.ProjectId);
			if (project2 == null)
			{
				continue;
			}
			...
			Auditai.Model.Table table = (Auditai.Model.Table)(row["Table"] = project2.GetTableById(source.TableId));
			if (table != null)
			{
				List<Auditai.DTO.Column> source2 = await GetTableColumns(table);
				row["Group"] = source2.ToDictionary((Auditai.DTO.Column c) => c, (Auditai.DTO.Column c) => source.GroupSrcId.Contains(c.Id));
				row["Data"] = source2.ToDictionary((Auditai.DTO.Column c) => c, (Auditai.DTO.Column c) => source.DataSrcId.Contains(c.Id));
				...
				row["IntercompanyCols"] = source2.Where(c => intercompanyModelCols.Contains(c.Id)).ToDictionary((Auditai.DTO.Column c) => c, (Auditai.DTO.Column c) => source.IntercompanyCols.Contains(c.Id));
			}
		}
		foreach (Auditai.Model.Project project in _projects)
		{
		...
		_dcbGroup.Value = Table.Columns.Where((Auditai.Model.Column c) => c.ConsolidateAttributes == null || c.ConsolidateAttributes.Role == ConsolidateRole.None || c.ConsolidateAttributes.Role == ConsolidateRole.GroupBy).ToDictionary((Auditai.Model.Column c) => c, (Auditai.Model.Column c) => Table.ConsolidateSettings.GroupDestId.Contains(c.Id));
		_dcbAggregate.Value = Table.Columns.Where((Auditai.Model.Column c) => c.ConsolidateAttributes == null || c.ConsolidateAttributes.Role == ConsolidateRole.None || c.ConsolidateAttributes.Role == ConsolidateRole.Aggregate).ToDictionary((Auditai.Model.Column c) => c, (Auditai.Model.Column c) => Table.ConsolidateSettings.AggregateDestId.Contains(c.Id));
	...
	private async void _grid_AfterEdit(object sender, RowColEventArgs e)
	{
		if (_grid.Cols[e.Col].Name == "Selected")
		{
			...
			Auditai.Model.Table previousRowTable = (Auditai.Model.Table)_grid[previousRow, "Table"];
			if (previousRowTable == null)
			{
				return;
			}
			Auditai.Model.Project project = (Auditai.Model.Project)_grid[e.Row, "Project"];
			...
			if (treeTableNode != null)
			{
				...
				List<Auditai.DTO.Column> list = await GetTableColumns(treeTableNode.Table);
				IEnumerable<KeyValuePair<Auditai.DTO.Column, bool>> source = (IEnumerable<KeyValuePair<Auditai.DTO.Column, bool>>)_grid[previousRow, "Group"];
				List<string> previousGroupCaptions = (from kv in source
					where kv.Value
					select kv.Key.Caption).ToList();
				Dictionary<Auditai.DTO.Column, bool> value = list?.ToDictionary((Auditai.DTO.Column c) => c, (Auditai.DTO.Column c) => previousGroupCaptions.Contains(c.Caption));
				_grid[e.Row, "Group"] = value;
				IEnumerable<KeyValuePair<Auditai.DTO.Column, bool>> source2 = (IEnumerable<KeyValuePair<Auditai.DTO.Column, bool>>)_grid[previousRow, "Data"];
				List<string> previousDataCaptions = (from kv in source2
					where kv.Value
					select kv.Key.Caption).ToList();
				Dictionary<Auditai.DTO.Column, bool> value2 = list?.ToDictionary((Auditai.DTO.Column c) => c, (Auditai.DTO.Column c) => previousDataCaptions.Contains(c.Caption));
				_grid[e.Row, "Data"] = value2;
			}
		}
		else if (_grid.Cols[e.Col].Name == "Table" && _grid[e.Row, e.Col] is Auditai.Model.Table table)
		{
			Dictionary<Auditai.DTO.Column, bool> value3 = (await GetTableColumns(table))?.ToDictionary((Auditai.DTO.Column c) => c, (Auditai.DTO.Column c) => false);
	...
	private void _grid_OwnerDrawCell(object sender, OwnerDrawCellEventArgs e)
	{
		...
		if (e.Col == _grid.Cols["Project"].Index && _grid[e.Row, e.Col] is Auditai.Model.Project project)
		{
			e.Text = project.Name;
		}
		if (e.Col == _grid.Cols["Table"].Index && _grid[e.Row, e.Col] is Auditai.Model.Table table)
		{
			e.Text = table.TreeNode.Name;
		}
		if ((e.Col == _grid.Cols["Group"].Index || e.Col == _grid.Cols["Data"].Index || e.Col == _grid.Cols["IntercompanyCols"].Index) && _grid[e.Row, e.Col] is IEnumerable<KeyValuePair<Auditai.DTO.Column, bool>> source)
		{
	...
	private void _grid_SetupEditor(object sender, RowColEventArgs e)
	{
		...
		if (e.Col == _grid.Cols["Table"].Index)
		{
			...
			Auditai.Model.Project p = obj as Auditai.Model.Project;
			if (p != null)
			{
		...
		if ((e.Col != _grid.Cols["Group"].Index && e.Col != _grid.Cols["Data"].Index && e.Col != _grid.Cols["IntercompanyCols"].Index) || !(_grid[e.Row, "Project"] is Auditai.Model.Project))
		{
			return;
		}
		obj = _grid[e.Row, "Table"];
		Auditai.Model.Table t = obj as Auditai.Model.Table;
		if (t == null)
		{
			return;
		}
			...
			else if (e.Col == _grid.Cols["IntercompanyCols"].Index)
			{
				...
				else
				{
					var intercompanyCols = t.Columns.Where(c => c.ConsolidateAttributes != null && c.ConsolidateAttributes.Role == ConsolidateRole.Intercompany)
						.Select(c => new Auditai.DTO.Column { Id = c.Id, Caption = c.Caption })
						.ToDictionary(c => (Auditai.DTO.Column)c, c => false);
	...
	private async Task<List<Auditai.Model.Project>> GetProjects()
	{
		List<Auditai.Model.Project> ret = new List<Auditai.Model.Project>();
		try
		{
			if (StorageRouter.IsLocalMode)
			{
				...
				foreach (var dto in dtos)
				{
					if (dto.Id == Program.MainForm.CurrentProject.Id)
						continue;
					ret.Add(new Auditai.Model.Project
					{
						Id = dto.Id,
						Name = dto.Name,
			...
			else
			{
				...
				foreach (Auditai.DTO.Project item in await WebApiClient.GetProjectDescendants(Program.MainForm.CurrentProject.Id))
				{
					Auditai.Model.Project proj = new Auditai.Model.Project
					{
						Id = item.Id,
						Name = item.Name,
	...
	private bool Validate()
	{
		...
		HashSet<Tuple<Auditai.Model.Project, Auditai.Model.Table>> hashSet = new HashSet<Tuple<Auditai.Model.Project, Auditai.Model.Table>>();
		foreach (int item2 in list)
		{
			if (_grid[item2, "Project"] == null)
			{
				Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "来源单体不能为空");
				return false;
			}
			if (_grid[item2, "Table"] == null)
			{
				Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "来源报表不能为空");
				return false;
			}
			if (_grid[item2, "Table"] == Table)
			{
				Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "来源报表不能为当前表");
				return false;
			}
			Tuple<Auditai.Model.Project, Auditai.Model.Table> item = Tuple.Create(_grid[item2, "Project"] as Auditai.Model.Project, _grid[item2, "Table"] as Auditai.Model.Table);
			if (hashSet.Contains(item))
			{
				Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "来源报表不能有重复");
				return false;
			}
			hashSet.Add(item);
			if (!(_grid[item2, "Group"] is IEnumerable<KeyValuePair<Auditai.DTO.Column, bool>> source) || !source.Any((KeyValuePair<Auditai.DTO.Column, bool> kv) => kv.Value))
			{
				Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "合并维度不能为空");
				return false;
			}
			if (!(_grid[item2, "Data"] is IEnumerable<KeyValuePair<Auditai.DTO.Column, bool>> source2) || !source2.Any((KeyValuePair<Auditai.DTO.Column, bool> kv) => kv.Value))
			{
				Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "合并金额不能为空");
				return false;
			}
			...
			{
				Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "合并维度和合并金额不能重复");
				return false;
			}
			decimal ownershipRatio = (_grid[item2, "OwnershipRatio"] as decimal?) ?? 100m;
			if (ownershipRatio < 0m || ownershipRatio > 100m)
			{
				Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "持股比例必须在0~100之间");
				return false;
			}
			int level = (_grid[item2, "Level"] as int?) ?? 1;
			if (level < 1)
			{
				Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "合并层级必须为正整数");
				return false;
			}
		}
		int num = (_grid[list[0], "Group"] as IEnumerable<KeyValuePair<Auditai.DTO.Column, bool>>).Where((KeyValuePair<Auditai.DTO.Column, bool> kv) => kv.Value).Count();
		foreach (int item3 in list.Skip(1))
		{
			if ((_grid[item3, "Group"] as IEnumerable<KeyValuePair<Auditai.DTO.Column, bool>>).Where((KeyValuePair<Auditai.DTO.Column, bool> kv) => kv.Value).Count() != num)
			{
				Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "各来源报表的合并维度数量必须相同");
				return false;
			}
		}
		if (num != _dcbGroup.SelectedValue.Count)
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "来源报表合并维度数量与目标合并维度数量必须相同");
			return false;
		}
		int num2 = (_grid[list[0], "Data"] as IEnumerable<KeyValuePair<Auditai.DTO.Column, bool>>).Where((KeyValuePair<Auditai.DTO.Column, bool> kv) => kv.Value).Count();
		foreach (int item4 in list.Skip(1))
		{
			if ((_grid[item4, "Data"] as IEnumerable<KeyValuePair<Auditai.DTO.Column, bool>>).Where((KeyValuePair<Auditai.DTO.Column, bool> kv) => kv.Value).Count() != num2)
			{
				Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "各来源报表的合并金额数量必须相同");
				return false;
			}
		}
		if (num2 != _dcbAggregate.SelectedValue.Count)
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "来源报表合并金额数量与目标合并金额数量必须相同");
			return false;
		}
		if (_dcbAggregate.SelectedValue.Intersect(_dcbGroup.SelectedValue).Any())
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "目标合并维度与目标合并金额不得重复");
			return false;
		}
		return true;
	}
	...
	private void Apply()
	{
		...
		for (int i = _grid.Rows.Fixed; i < _grid.Rows.Count; i++)
		{
			Auditai.Model.Project project = _grid[i, "Project"] as Auditai.Model.Project;
			Auditai.Model.Table table = _grid[i, "Table"] as Auditai.Model.Table;
			IEnumerable<KeyValuePair<Auditai.DTO.Column, bool>> enumerable = _grid[i, "Group"] as IEnumerable<KeyValuePair<Auditai.DTO.Column, bool>>;
			IEnumerable<KeyValuePair<Auditai.DTO.Column, bool>> enumerable2 = _grid[i, "Data"] as IEnumerable<KeyValuePair<Auditai.DTO.Column, bool>>;
				...
				ConsolidateEntry item = new ConsolidateEntry
			{
				...
				IntercompanyCols = (_grid[i, "IntercompanyCols"] is IEnumerable<KeyValuePair<Auditai.DTO.Column, bool>> intercompanyEnum
		...
		if (Table.ConsolidateSettings.Sources.Count != 0)
		{
			...
			Table.ConsolidateSettings.GroupDestId = Table.ConsolidateSettings.GroupDest.Select((Auditai.Model.Column c) => c.Id).ToList();
			Table.ConsolidateSettings.AggregateDestId = Table.ConsolidateSettings.AggregateDest.Select((Auditai.Model.Column c) => c.Id).ToList();
