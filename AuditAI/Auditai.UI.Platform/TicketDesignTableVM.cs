...
public class TicketDesignTableVM
{
		...
		if (ticket2.Kind == TicketKind.DynamicRow)
		{
			...
			for (int l = 0; l < ticket2.DataRowCount; l++)
			{
				...
				for (int m = 0; m < ticket2.Columns.Count; m++)
				{
					...
					{
						Auditai.Model.Column byId = _ticket.Table.Columns.GetById(ticketColumn.Field);
						if (byId != null)
						{
		...
		for (int i = 0; i < rowsCount; i++)
		{
			for (int j = 0; j < columnsCount; j++)
			{
				...
				if (cell.HasFormula())
				{
					...
					{
						return "表体区单元格[" + Auditai.Model.Column.GetExcelColumnName(j) + ":" + (i + 1) + "]的单元格公式不正确!";
					}
				}
			}
		}
	...
	public TicketDesignValidation Validate()
	{
		...
		Dictionary<string, Auditai.Model.Column> dictionary = _ticket.Table.Columns.ToDictionary((Auditai.Model.Column c) => c.GetUniqueFormulaName() ?? "", (Auditai.Model.Column c) => c);
		Dictionary<Id64, Auditai.Model.Column> dictionary2 = new Dictionary<Id64, Auditai.Model.Column>();
		foreach (TicketDesignCellVM cell5 in Cells)
		{
		...
		bool GetTitleFooterInputField(Dictionary<string, Auditai.Model.Column> columnDic, Dictionary<Id64, Auditai.Model.Column> dataAreaUsedField)
		{
			Dictionary<Id64, Auditai.Model.Column> dictionary3 = new Dictionary<Id64, Auditai.Model.Column>();
			Dictionary<Id64, Auditai.Model.Column> dictionary4 = new Dictionary<Id64, Auditai.Model.Column>();
		...
		bool GetTitleOrFooterInputField(Dictionary<string, Auditai.Model.Column> columnDic, TicketDesignTitleFooterVM target, bool isTitleArea, Dictionary<Id64, Auditai.Model.Column> dataAreaUsedField, Dictionary<Id64, Auditai.Model.Column> outUsedFieldMap, Dictionary<object, TicketDesignValidation.TempField> outDic)
		{
	...
	public TicketDesignValidation Save()
	{
		...
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, TicketDesignFailureReasonMessage(ret.FailureReason));
		}
		return ret;
	...
	private TicketDesignCellVM GetCellVM(TicketCell tc)
	{
		...
		if (tc.HasField())
		{
			Auditai.Model.Column byId = _ticket.Table.Columns.GetById(tc.Field);
			if (byId != null)
			{
