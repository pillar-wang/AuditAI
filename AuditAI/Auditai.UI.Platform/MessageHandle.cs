...
public class MessageHandle
{
	...
	{
		Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "文件接收超时，任务取消");
	}

	public static async void SignalRClient_MessageReceived(object sender, MessageReceivedEventArgs e)
	{
		string text = Auditai.Model.User.Current.Id.ToString();
		try
		{
			...
			case MessageKind.MessageFromUser:
			{
				...
				if (mpg.Kind == "mergemessage")
				{
					if (mpg.Value != null)
					{
						...
						Auditai.DTO.User user = await WebApiClient.GetUserById(id);
						Member member13;
			...
			case MessageKind.TeamBroadcast:
			{
				...
				{
					string text4 = Auditai.Model.User.Current.TeamId.ToString();
