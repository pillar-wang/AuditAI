...
public frmCrossProjectRefStatus(Auditai.Model.Project currentProject)
    {
        ...
        _cache = new CrossProjectRefCache(Auditai.Model.User.Current?.Id ?? 1);
    ...
    private async System.Threading.Tasks.Task LoadData()
    {
        ...
        {
            Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.Error, $"加载状态数据失败：{ex.Message}");
        }
    }
    ...
    private async System.Threading.Tasks.Task RunHealthCheck()
    {
        try
        {
            ...
            _lblErrorRefs.Text = errorCount.ToString();
            Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.Information, $"健康检查完成！共 {allRefs.Count} 个引用，{errorCount} 个异常。");
        }
        catch (Exception ex)
        {
            Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.Error, $"健康检查失败：{ex.Message}");
        }
    }
    ...
    /// </summary>
    public static DialogResult ShowDashboard(Auditai.Model.Project project)
    {
