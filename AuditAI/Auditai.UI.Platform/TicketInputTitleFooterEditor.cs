...
public class TicketInputTitleFooterEditor : ISetTheme
{
	...
	public Auditai.Model.Table Table => _owner.Table;
	...
	private async Task AddAttachment()
	{
		...
		if (!SoftwareLicenseManager.IsTableAttachmentOutOfLicenseLimit(fileInfo.Length))
		{
			...
			{
				cell.Attachments = new Auditai.Model.CellAttachments();
			}
	...
	private void ShowTooltip()
	{
		...
		if (Program.MainForm.TableValidationResults.TryGetValue(Table.TreeNode, out var value) && cellVM.IsTableExistCell)
		{
			...
			IEnumerable<Tuple<Auditai.Model.Cell, ValidationResult>> enumerable = value.Cells.Cast<Tuple<Auditai.Model.Cell, ValidationResult>>().Where((Tuple<Auditai.Model.Cell, ValidationResult> t) => t.Item1.Id == tableCell.Id);
			...
			IEnumerable<Tuple<Auditai.Model.Column, ValidationResult>> enumerable3 = value.Columns.Cast<Tuple<Auditai.Model.Column, ValidationResult>>().Where((Tuple<Auditai.Model.Column, ValidationResult> t) => t.Item1.Index == cellColIndex);
			IEnumerable<Tuple<Auditai.Model.Cell, ValidationResult>> enumerable4 = value.HeaderCells.Cast<Tuple<Auditai.Model.Cell, ValidationResult>>().Where((Tuple<Auditai.Model.Cell, ValidationResult> t) => t.Item1.Column.Index == cellColIndex && t.Item1.Row.Index < cellRowIndex && t.Item1.GetHeaderLastRow() >= cellRowIndex);
	...
	private void _grid_BodyOwnerDrawCell(object sender, OwnerDrawCellEventArgs e)
	{
		...
		try
		{
			...
			if (cellVM.IsTableExistCell && Program.MainForm.ShowHelperTooltip && Program.MainForm.TableValidationResults.TryGetValue(Table.TreeNode, out var value))
			{
				...
				int cellRowIndex = tableCell.Row.Index;
				int cellColIndex = tableCell.Column.Index;
				IEnumerable<Tuple<Auditai.Model.Cell, ValidationResult>> source = value.Cells.Cast<Tuple<Auditai.Model.Cell, ValidationResult>>().Where((Tuple<Auditai.Model.Cell, ValidationResult> t) => t.Item1.Id == tableCell.Id);
				...
				IEnumerable<Tuple<Auditai.Model.Column, ValidationResult>> source3 = value.Columns.Cast<Tuple<Auditai.Model.Column, ValidationResult>>().Where((Tuple<Auditai.Model.Column, ValidationResult> t) => t.Item1.Index == cellColIndex);
				IEnumerable<Tuple<Auditai.Model.Cell, ValidationResult>> source4 = value.HeaderCells.Cast<Tuple<Auditai.Model.Cell, ValidationResult>>().Where((Tuple<Auditai.Model.Cell, ValidationResult> t) => t.Item1.Column.Index == cellColIndex && t.Item1.Row.Index < cellRowIndex && t.Item1.GetHeaderLastRow() >= cellRowIndex);
	...
	private void SendBodySelectionChanged_SignalR(int rowIndex, int colIndex)
	{
		...
		async Task BodySelectionChanged_SignalR(int bodyRowIndex, int bodyColIndex)
		{
			try
			{
				...
				{
					Auditai.Model.Cell tableCell = cellVM.TableCell;
					if (tableCell != null)
					{
						long value = tableCell.Id.Value;
						await SignalRClient.UpLoadTableCellId(Auditai.Model.User.Current.Id.ToString(), value.ToString());
					}
				}
			}
			catch
