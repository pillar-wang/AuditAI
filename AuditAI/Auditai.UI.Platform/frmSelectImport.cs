...
public class frmSelectImport : C1RibbonForm
{
	...
	private Auditai.Model.Project _project;
	...
	public frmSelectImport(Auditai.Model.Project project)
	{
		_project = project;
		InitializeComponent();
	...
	private void PopulateTree(LedgerCollectEventArgs e)
	{
		...
		try
		{
			...
			foreach (Auditai.Model.TreeGroup treeGroup in _project.TreeGroups)
			{
	...
	private void btnCertain_Click(object sender, EventArgs e)
	{
		...
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "请选中要填充的表格后再进行该操作!");
			return;
		}
		Auditai.Model.Table table = selectedNodes[0].Table;
		if (!table._loaded)
		{
		...
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "表格 " + table.TreeNode.Name + " 损坏了，无法打开表格!");
			return;
		}
