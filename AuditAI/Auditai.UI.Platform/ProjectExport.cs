...
public class ProjectExport
{
	...
	{
		Auditai.Model.Table table = tableNode.Table;
		table.LoadAndReturn();
	...
	public async Task ExportTableAttachments(Auditai.Model.Table table, string tableFilePath)
	{
		table.LoadAndReturn();
	...
	{
		Auditai.Model.Document document = docNode.Document;
	...
	private async Task SaveProjectImpl(string path)
	{
		...
		Dictionary<Auditai.Model.TreeGroup, List<TreeNodeBase>> groupList = GetGroupList(Project);
		totalProgress = groupList.SelectMany((KeyValuePair<Auditai.Model.TreeGroup, List<TreeNodeBase>> t) => t.Value).Count() + (from n in groupList.SelectMany((KeyValuePair<Auditai.Model.TreeGroup, List<TreeNodeBase>> t) => t.Value)
		...
		foreach (KeyValuePair<Auditai.Model.TreeGroup, List<TreeNodeBase>> item in groupList)
		{
			string tail2;
	...
	private Dictionary<Auditai.Model.TreeGroup, List<TreeNodeBase>> GetGroupList(Auditai.Model.Project project)
	{
		Dictionary<Auditai.Model.TreeGroup, List<TreeNodeBase>> dictionary = new Dictionary<Auditai.Model.TreeGroup, List<TreeNodeBase>>();
