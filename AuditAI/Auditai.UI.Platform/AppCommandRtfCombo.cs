...
public abstract class AppCommandRtfCombo : AppCommandComboBox
{
	...
	{
		Auditai.Model.HeaderFooter obj = (Auditai.Model.HeaderFooter)typeof(PageSetup).GetProperty(PropName1).GetValue(ps);
		typeof(Auditai.Model.HeaderFooter).GetProperty(PropName2).SetValue(obj, rtf);
	}

	private string GetProp(PageSetup ps)
	{
		Auditai.Model.HeaderFooter obj = (Auditai.Model.HeaderFooter)typeof(PageSetup).GetProperty(PropName1).GetValue(ps);
		return (string)typeof(Auditai.Model.HeaderFooter).GetProperty(PropName2).GetValue(obj);
	}
}
