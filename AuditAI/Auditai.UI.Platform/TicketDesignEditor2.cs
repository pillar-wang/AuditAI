...
public class TicketDesignEditor2
{
	...
	public void SetColumnWidth()
	{
		...
		if (num2 < 0)
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "列宽不能小于0");
			return;
		}
		if (num2 > 9999)
		{
	...
	public void SetRowHeight()
	{
		...
		if (num2 < 0)
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "行高不能小于0");
			return;
		}
		if (num2 > 9999)
		{
	...
	public string GetFormulaContextCellLabel()
	{
		...
		return $"[{Auditai.Model.Column.GetExcelColumnName(_grid.BodyCol)},{_grid.BodyRow + 1}]";
	}
	...
	public void Copy()
	{
		...
		if (cell.HasFormula())
		{
			...
			dataObject.SetData("AuditaiClipboardTicketDesignCell", data);
		}
		StringBuilder stringBuilder = new StringBuilder();
		for (int i = bodySelection.TopRow; i <= bodySelection.BottomRow; i++)
		{
	...
	public void Paste()
	{
		...
		if (Clipboard.ContainsData("AuditaiClipboardTicketDesignCell"))
		{
			ClipboardTicketDesignCell clipboardTicketDesignCell = (ClipboardTicketDesignCell)Clipboard.GetData("AuditaiClipboardTicketDesignCell");
			C1.Win.C1FlexGrid.CellRange bodySelection = _grid.BodySelection;
			for (int i = bodySelection.TopRow; i <= bodySelection.BottomRow; i++)
			{
				for (int j = bodySelection.LeftCol; j <= bodySelection.RightCol; j++)
	...
	public void ImportTable()
	{
		...
		formSelectNode.Project = Auditai.Model.Project.Current;
		if (formSelectNode.ShowImportTicket() != DialogResult.OK)
		{
			return;
		}
		Auditai.Model.Table table = formSelectNode.SelectedTableNode.Table.LoadAndReturn();
		if (table.Ticket.IsEmpty())
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "表格没有" + Ticket.GetLevelString() + "定义。");
			return;
		}
	...
	private void _grid_DragDrop(object sender, DragEventArgs e)
	{
		...
		{
			Auditai.Model.Column column = Table.Columns[_dragRow];
	...
	private void _grid_OwnerDrawCell(object sender, OwnerDrawCellEventArgs e)
	{
		if (_vm.ColumnHeaderRowsCount > 0 && _columnHeaderRowBackgroundBrush == null)
		{
			...
			_columnHeaderRowBackgroundBrush = new SolidBrush(Auditai.UI.Controls.Util.DarkenColor(color, 0.01));
		}
		if (e.Col == 0)
		{
		...
		{
			string excelColumnName = Auditai.Model.Column.GetExcelColumnName(e.Col - 1);
	...
	private void _grid_MouseUp(object sender, MouseEventArgs e)
	{
		...
		if (formulaEditor.IsEditing)
		{
			...
			{
				C1.Win.C1FlexGrid.CellRange bodySelection = _grid.BodySelection;
				string text = ((!bodySelection.IsSingleCell) ? $"[{Auditai.Model.Column.GetExcelColumnName(bodySelection.LeftCol)},{bodySelection.TopRow + 1}:{Auditai.Model.Column.GetExcelColumnName(bodySelection.RightCol)},{bodySelection.BottomRow + 1}]" : $"[{Auditai.Model.Column.GetExcelColumnName(bodySelection.LeftCol)},{bodySelection.TopRow + 1}]");
