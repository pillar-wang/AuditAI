...
public class frmCrossProjectRefConfigWizard : Form  // 与 frmTableCollect2 一致
{
    ...
    private Auditai.Model.Table _targetTable;
    private Auditai.Model.Table _sourceTable;
    ...
    private List<Auditai.DTO.Project> _allProjects = new List<Auditai.DTO.Project>();
    ...
    private void ShowStep1()
    {
        ...
        _tileProjects.CustomBorderColor = Theme.SelectedAuditaiTheme.ThemeContext.DarkColor;
        ...
        try
        {
            ...
            gridTarget.SelectionChanged += (s, args) =>
            {
                ...
                if (_targetTable != null)
                {
                    ...
                    if (hasFormula)
                    {
                        Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.Warning, "选中的单元格包含公式，不能作为跨项目引用的目标。\n请选择不含公式的单元格。");
                        _tgtCol = -1;
            ...
            _stepValidate = new Func<bool>(() =>
            {
                if (_tgtCol < 0)
                {
                    Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.Warning, "请先在表格中选择目标位置");
                    return false;
                }
                return true;
            });
        ...
        try
        {
            ...
            if (_selectedRefMode == RefMode.FormulaCompute)
            {
                ...
                _stepValidate = new Func<bool>(() =>
                {
                    ...
                    if (_formulaCheckedIndices.Count == 0)
                    {
                        Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.Warning, "请勾选参与运算的来源列");
                        return false;
                    }
                    if (string.IsNullOrEmpty(_formulaExpr))
                    {
                        Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.Warning, "请输入公式表达式");
                        return false;
                    }
                    // 保存配置
            ...
            _stepValidate = new Func<bool>(() =>
            {
                if (_srcCol < 0)
                {
                    Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.Warning, "请先选择数据源范围");
                    return false;
                }
                ...
                switch (_selectedRefMode)
                {
                    case RefMode.CellRef:
                        ...
                        {
                            Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.Warning, "无法获取目标单元格信息，请重试");
                            return false;
                        }
                        if (srcCell == null || srcCell.Id.Value <= 0)
                        {
                            Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.Warning, "无法获取来源单元格信息，请重试");
                            return false;
                        }
                        _refConfigJson = Newtonsoft.Json.JsonConvert.SerializeObject(new
                        {
    ...
    private void LoadSourceTableData(LightweightTableEditor editor)
    {
        try
        {
            ...
            var project = new Auditai.Model.Project { Id = dto.Id, Name = dto.Name };
            project.Dal = dal;
            project.PopulateFieldsFromDto(dto);
            project.Load();
    ...
    /// </summary>
    private static void LoadTableToEditor(LightweightTableEditor editor, Auditai.Model.Table table, int maxRows)
    {
        table.LoadAndReturn();
        editor.HideError();
    ...
    /// </summary>
    public static DialogResult ShowWizard(Auditai.Model.Project project, Id64 targetTableId, CrossProjectDataRef editingRef = null)
    {
