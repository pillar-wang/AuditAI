	...
	public SendFileMessage(string sendId, string sendName, NotifyMessage mpg)
	{
		...
		ActionDeal = async delegate(object content)
		{
			if (content is Tuple<string, string, NotifyMessage> { Item1: var item, Item2: var item2 })
			{
				...
				if (DialogResult.OK == Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, item2 + "向您发送来账套文件：" + fileInfo.Name + "，是否现在接收？", MessageBoxButtons.OKCancel))
				{
					if (FileTranferManager.GetInstance().CanceledFileSet.Contains(fileInfo.Id))
					{
						Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "文件发送已被发送者取消！");
					}
					else
					{
						FileTranferManager.GetInstance().RecieveFileInfo(sendId, Auditai.Model.User.Current.Id.ToString(), fileInfo);
				...
				else
				{
					...
					await SignalRClient.SendToUser(item, new NotifyMessage
					{
						Kind = "sendfileresponse",
						Bullet = true,
						Text = Auditai.Model.User.Current.Name + "拒绝了发送的文件",
