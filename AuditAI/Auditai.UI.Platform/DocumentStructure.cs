...
public class DocumentStructure : ISetTheme
{
	...
	/// <returns>范围内的表格书签列表（位置, AuditaiBookmark）</returns>
	public List<(int Position, AuditaiBookmark Bookmark)> GetTablesInRange(int startPos, int endPos)
	{
		var result = new List<(int, AuditaiBookmark)>();
		...
		foreach (TXTextControl.DocumentTarget dt in tx.DocumentTargets)
		{
			try
			{
				...
				if (AuditaiBookmark.TryParse(targetName, out var bookmark))
				{
