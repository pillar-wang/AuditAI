...
using LqParagraph = Auditai.Model.Paragraph;
...
public class DocumentEditor : UserControl
{
	...
	public void PopulateDocument(params object[] args)
	{
		...
		if (doc.IsCorrupted)
		{
			...
			try
			{
				...
				if (project != null && Auditai.LocalDataStore.StorageRouter.IsLocalMode)
				{
					var defaultDto = new Auditai.DTO.Document
					{
						Id = doc.Id,
						Version = doc.Version,
						Locker = 0,
	...
	private void LoadDocumentFromModel()
	{
			...
			if (dts != null && dts.Count > 0)
			{
				foreach (TXTextControl.DocumentTarget dt in dts)
				{
					if (dt.TargetName != null && dt.TargetName.StartsWith("lsbm@"))
					{
						...
						if (AuditaiBookmark.TryParse(dt.TargetName, out var lsbm))
						{
		...
		try
		{
			var formulas = Auditai.Model.Project.Current?.ValidationManager?.Formulas;
			if (formulas != null && formulas.Count > 0)
			{
				var appFields = _textControl.ApplicationFields;
				if (appFields != null && appFields.Count > 0)
				{
					var fieldIds = new HashSet<Auditai.DTO.Id64>();
					foreach (TXTextControl.ApplicationField f in appFields)
					{
						if (f != null && f.Parameters != null && f.Parameters.Length >= 3)
						{
							...
							if (long.TryParse(f.Parameters[2], out fid) && fid > 0)
								fieldIds.Add(new Auditai.DTO.Id64(fid));
						}
					}
			...
			if (doc.MergeTable == null || doc.MergeTable.IsZero())
			{
				try
				{
					...
					for (int i = 1; i <= count; i++)
					{
						...
						if (dt != null && AuditaiBookmark.TryParse(dt.TargetName, out var lsbm) && lsbm.TableId != null)
						{
							doc.MergeTable = Auditai.DTO.Id64.ParseBase64(lsbm.TableId);
							break;
						}
					}
				}
	...
	private void ParseDocxToParagraphs(Auditai.Model.Document doc, byte[] docxBytes)
	{
		// 将 docx 保存为临时文件
		...
		try
		{
			...
			using (var package = System.IO.Packaging.Package.Open(tempFile, FileMode.Open, FileAccess.Read))
			{
				...
				using (var reader = new StreamReader(stream))
				{
					...
					var ns_w = Auditai.Model.Document.xmlns_w;
					var ns_r = Auditai.Model.Document.xmlns_r;
					var ns_a = Auditai.Model.Document.xmlns_a;
	...
	public void RibbonSetTableBorder(params object[] args)
	{
		var table = _textControl.Tables.GetItem();
		if (table == null) return;
		if (args?.Length > 0 && args[0] is Auditai.Model.TableBorderStyle style)
		{
			int thinWidth = 15;
			int thickWidth = 30;
			int upDownWidth = style.UpDownLine == Auditai.Model.LineStyle.Thick ? thickWidth : (style.UpDownLine == Auditai.Model.LineStyle.Thin ? thinWidth : 0);
			int leftRightWidth = style.LeftRightLine == Auditai.Model.LineStyle.Thick ? thickWidth : (style.LeftRightLine == Auditai.Model.LineStyle.Thin ? thinWidth : 0);
			int bodyWidth = style.BodyLine == Auditai.Model.LineStyle.Thick ? thickWidth : (style.BodyLine == Auditai.Model.LineStyle.Thin ? thinWidth : 0);
			int secondWidth = style.SecondLine == Auditai.Model.LineStyle.Thick ? thickWidth : (style.SecondLine == Auditai.Model.LineStyle.Thin ? thinWidth : 0);
	...
	{
		this.PageSettingTarget = Auditai.UI.Platform.PageSettingTarget.Document;
		AppCommands.ApplySelection.IsPressed = false;
	}

	public void PageSettingTargetSelectionClicked(params object[] args)
	{
		this.PageSettingTarget = Auditai.UI.Platform.PageSettingTarget.Selection;
	...
	public void SetSidaStyle(params object[] args) { RibbonSetTableBorder(new object[] { Auditai.Model.TableBorderStyles.ThickBorderThinBody }); }
	...
	public void AddValidationPoint()
	{
		...
		using (var dlg = new frmDocValidationRuleEditor())
		{
			...
			Auditai.DTO.Id64 fieldId = Project.Current.GetNextId();
	...
	public void RemoveValidationPoint()
	{
		...
		if (fieldId > 0)
		{
			try
			{
				...
				var rulesToRemove = Auditai.Model.Project.Current.ValidationManager.Formulas
					.Where(f => f.DocumentFieldId == new Auditai.DTO.Id64(fieldId))
					.ToList();
				foreach (var rule in rulesToRemove)
				{
					Auditai.Model.Project.Current.ValidationManager.RemoveOne(rule);
				}
			}
	...
	public void AddValidationRuleToFormulaField()
	{
		...
		using (var dlg = new frmDocValidationRuleEditor())
		{
			...
			try
			{
				...
				var vf = new Auditai.Model.ValidationFormula
				{
					Id = Auditai.Model.Project.Current.GetNextId(),
					LeftExpr = formula,
					RightExpr = dlg.RightExpression,
					Note = dlg.Note,
					DocumentFieldId = new Auditai.DTO.Id64(fieldId),
					IsDirty = true,
					Status = Auditai.Model.SyncStatus.New
				};
				vf.Operator = dlg.GetOperator();

				// 移除同一文档域的旧规则，避免重复
				var existing = Auditai.Model.Project.Current.ValidationManager.Formulas
					.FindAll(f => f.DocumentFieldId == vf.DocumentFieldId);
				foreach (var old in existing)
				{
					Auditai.Model.Project.Current.ValidationManager.Formulas.Remove(old);
				}

				Auditai.Model.Project.Current.ValidationManager.Formulas.Add(vf);
			}
	...
	private void InsertModelTable(Auditai.Model.Table table)
	{
		if (!_textControl.Tables.CanAdd)
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "当前插入点不允许插入表格。");
			return;
		}
		...
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "表格为空！");
			return;
		}
		...
		try
		{
			...
			{
				var borderStyle = AppCommands.TableStyle.SelectedStyle ?? Auditai.Model.TableBorderStyles.Grid;
			...
			else if (AppCommands.PendingCustomStyle != null && AppCommands.PendingCustomStyle.IsCustomStyle)
			{
				...
				lsbm.TableStyle = Auditai.Model.TableBorderStyles.Custom;
			...
			{
				lsbm.TableStyle = (AppCommands.TableStyle.SelectedStyle ?? Auditai.Model.TableBorderStyles.Grid).InternalNumber;
			}
		...
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.Error, "发生错误：" + ex.Message);
		}
		finally
		{
			_textControl.ClearUndo();
	...
	private int getCaptionRows(Auditai.Model.Table table)
	{
		int result = 0;
		if (table.HeaderMode != (Auditai.Model.TableHeaderMode)1)
			result = table.GetNumVisibleCaptionRows();
		return result;
	}

	private void AddNewBookmark(TXTextControl.Table table, AuditaiBookmark lsbm)
	{
		try
		{
	...
	/// </summary>
	private Auditai.Model.Table FindTableByParaId(string paraIdBase64)
	{
		if (string.IsNullOrEmpty(paraIdBase64)) return null;
		try
		{
			var paraId = Auditai.DTO.Id64.ParseBase64(paraIdBase64);
			var doc = Document as Auditai.Model.Document;
			...
			if (_bookmarkTableIdCache.TryGetValue(paraIdBase64, out var cachedTableId))
			{
				if (project != null)
				{
					var cachedId = Auditai.DTO.Id64.ParseBase64(cachedTableId);
			...
			if (project != null)
			{
				...
				if (nodeById is Auditai.Model.TreeTableNode tableNode)
				{
			...
			// 从 Paragraph 的 Stream 中解析书签，获取完整的 AuditaiBookmark（可能包含 TableId）
			if (!string.IsNullOrEmpty(paragraph.Stream))
			{
				try
				{
					...
					AuditaiBookmark bm = null;
						if (!string.IsNullOrEmpty(name) && AuditaiBookmark.TryParse(name, out bm) && bm.TableId != null)
						{
							var id = Auditai.DTO.Id64.ParseBase64(bm.TableId);
			...
			{
				var mergeTable = project?.GetTableById(new Auditai.DTO.Id64(doc.MergeTable.Value));
				if (mergeTable != null)
				{
					return mergeTable;
				}
	...
	private bool GetRefTable(TXTextControl.Table txTable, out AuditaiBookmark bookmark, out Auditai.Model.Table refTable)
	{
		refTable = null;
		bookmark = null;
		try
		{
			...
			if (item != null && !string.IsNullOrEmpty(item.TargetName) && AuditaiBookmark.TryParse(item.TargetName, out bookmark))
			{
				// 1a：书签有 TableId，直接查找
				if (bookmark.TableId != null)
					{
						...
						Auditai.DTO.Id64 id = Auditai.DTO.Id64.ParseBase64(bookmark.TableId);
						var project = Program.MainForm.CurrentProject;
					...
					if (bookmark.ParaIdBase64 != null)
					{
						if (_bookmarkTableIdCache.TryGetValue(bookmark.ParaIdBase64, out var cachedTableId))
						{
							...
							Auditai.DTO.Id64 id = Auditai.DTO.Id64.ParseBase64(cachedTableId);
							var project = Program.MainForm.CurrentProject;
							refTable = project?.GetTableById(id);
				...
				if (allTargets != null)
				{
					...
					AuditaiBookmark bestBookmark = null;
					...
					if (bestBookmark != null)
					{
						...
						if (bestBookmark.TableId != null)
						{
							...
							Auditai.DTO.Id64 id = Auditai.DTO.Id64.ParseBase64(bestBookmark.TableId);
							var project = Program.MainForm.CurrentProject;
							refTable = project?.GetTableById(id);
						}
	...
	private void SetColumnWidth(TXTextControl.Table txTable, Auditai.Model.Table modelTable)
	{
	...
	private void SetRowHeight(TXTextControl.Table txTable, Auditai.Model.Table modelTable)
	{
	...
	private void SetDefaultStyleWithFormat(TXTextControl.Table txTable, Auditai.Model.CellStyle style)
{
	...
	private void SetColumnStyleWithFormat(TXTextControl.Table txTable, Auditai.Model.Table modelTable)
	{
		foreach (TXTextControl.TableColumn col in txTable.Columns)
		{
			var modelCol = modelTable.Columns.GetVisibleColumnAt(col.Column - 1);
	...
	private void SetCaptionStyleWithFormat(TXTextControl.Table txTable, Auditai.Model.Table modelTable)
	{
		int capRows = getCaptionRows(modelTable);
		foreach (TXTextControl.TableColumn col in txTable.Columns)
		{
	...
	private void SetCaptionContentsWithFormat(TXTextControl.Table txTable, Auditai.Model.Table modelTable)
	{
		int capRows = getCaptionRows(modelTable);
	...
	private void MergeCaptionCells(Auditai.Model.Table modelTable, TXTextControl.Table txTable)
	{
	...
	private void SetBodyContentsWithFormat(TXTextControl.Table txTable, Auditai.Model.Table modelTable)
	{
		int capRows = getCaptionRows(modelTable);
	...
	private void MergeBodyCells(TXTextControl.Table txTable, Auditai.Model.Table modelTable)
	{
		int capRows = getCaptionRows(modelTable);
	...
	private void SetSelectionStyleWithFormat(Auditai.Model.CellStyle style)
	{
		if (style == null) return;
	...
	private void SetSidaStyle(TXTextControl.Table txTable)
	{
		...
		{
			txTable.Select();
			SetTableBorder(txTable, Auditai.Model.TableBorderStyles.Grid, 0);
		}
		catch { }
	}
	...
	public void NextError(params object[] args)
	{
		try
		{
			...
			for (int i = 0; i < totalFields; i++)
			{
				...
				var key = new Auditai.DTO.Id64(fid);
			...
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "没有更多错误。");
		}
		catch { }
	}
	...
	public void StartValidate(params object[] args)
	{
		try
		{
			...
			var results = new Dictionary<Auditai.DTO.Id64, List<ValidationResult>>();

			// === 第一步：格式合规校验 ===
			try
			{
				var project = Auditai.Model.Project.Current;
				if (project?.FormatComplianceChecker != null && Document is Auditai.Model.Document doc)
				{
			...
			try
			{
				...
				if (project != null && project.CrossDocumentValidationRules != null)
				{
					crossTotalCount = project.CrossDocumentValidationRules
						.Count(r => r.SourceDocumentId == ((Document as Auditai.Model.Document)?.Id ?? Auditai.DTO.Id64.Zero) ||
						            r.TargetDocumentId == ((Document as Auditai.Model.Document)?.Id ?? Auditai.DTO.Id64.Zero));
			...
			{
				Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "文档中没有校验点或格式合规规则。");
			}
			else
			{
				string msg = "文档校验结果：\n\n";
				...
				Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, msg);
			}
		}
	...
	public int ValidateCrossDocument()
	{
		...
		// 获取当前文档 Id
		var currentDoc = Document as Auditai.Model.Document;
		if (currentDoc == null || currentDoc.Id == null)
			return 0;

		Auditai.DTO.Id64 currentDocId = currentDoc.Id;
		...
		var currentFieldValues = new Dictionary<Auditai.DTO.Id64, string>();
		foreach (TXTextControl.ApplicationField f in _textControl.ApplicationFields)
		{
			...
			{
				currentFieldValues[new Auditai.DTO.Id64(fid)] = f.Text ?? "";
			}
		}
	...
	/// 对当前文档中指定的域进行着色。
	/// </summary>
	private void HighlightCrossField(Auditai.DTO.Id64 fieldId, bool passed, bool canCompare)
	{
		foreach (TXTextControl.ApplicationField f in _textControl.ApplicationFields)
		{
			if (f == null || f.TypeName != "MERGEFIELD" || f.Parameters == null || f.Parameters.Length < 3)
				continue;
			long fid;
			if (long.TryParse(f.Parameters[2], out fid) && new Auditai.DTO.Id64(fid) == fieldId)
			{
				f.HighlightMode = TXTextControl.HighlightMode.Always;
				f.DoubledInputPosition = true;
	...
	public void GenerateConfirmation(params object[] args)
	{
		...
		try
		{
			...
			var data = args[0] as System.Collections.Generic.List<System.Collections.Generic.Dictionary<string, Auditai.Model.Cell>>;
		...
		try
		{
			...
			try
			{
				while (enumerator.MoveNext())
				{
					...
					{
						if (AuditaiBookmark.TryParse(dt.TargetName, out AuditaiBookmark _))
						{
	...
	public void RefreshTableWithoutFormat()
	{
		try
		{
			...
			if (GetRefTable(txTable, out var bookmark, out var refTable))
			{
				if (refTable == null)
				{
					Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "表格不存在。");
					return;
				}
	...
	private Auditai.Model.Table RefreshModelTableWithFormatImpl(TXTextControl.Table txTable)
	{
		...
		try
		{
			...
			if (refTable == null)
			{
				Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.Exclamation, "表格不存在", MessageBoxButtons.OK, "", false);
				return null;
			}
			...
			{
				Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.Exclamation, "表格为空！", MessageBoxButtons.OK, "", false);
				return refTable;
			}
		...
		if (bookmark?.TableStyle.HasValue == true && bookmark.TableStyle.Value == 5)
			...
			else if (bookmark?.TableStyle.HasValue == true && bookmark.TableStyle.Value == Auditai.Model.TableBorderStyles.Custom)
			{
				// 自定义样式：从书签的 CustomBorderStyleJson 反序列化
				if (!string.IsNullOrEmpty(bookmark.CustomBorderStyleJson))
				{
					var customStyle = Auditai.Model.TableBorderStyle.FromJson(bookmark.CustomBorderStyleJson);
					if (customStyle != null)
					{
						txTable.Select();
					...
					else
					{
						// 反序列化失败，降级为默认样式
						txTable.Select();
						SetTableBorder(txTable, Auditai.Model.TableBorderStyles.Grid, getCaptionRows(refTable));
					}
				}
				else
				{
			...
			else
			{
				var borderStyle = bookmark?.TableStyle.HasValue == true
					? Auditai.Model.TableBorderStyles.FromNumber(bookmark.TableStyle.Value)
	...
	private void RefreshTableContentsImpl(TXTextControl.Table txTable, Auditai.Model.Table refTable)
	{
		try
		{
	...
	private void SetCaptionStyleWithoutFormat(TXTextControl.Table txTable, Auditai.Model.Table modelTable)
	{
		int capRows = getCaptionRows(modelTable);
	...
	private void SetBodyContentsWithoutFormat(TXTextControl.Table txTable, Auditai.Model.Table modelTable)
	{
		int capRows = getCaptionRows(modelTable);
	...
	private void ModifyBookmark(TXTextControl.Table table, AuditaiBookmark bookmark)
	{
	...
	public void Tx_InputPositionChanged(object sender, EventArgs e)
	{
		...
		PopulateToolbar();
		Program.MainForm.FormulaEditor.Context.Kind = Auditai.Model.FormulaContextKind.Document;
			...
			if (table != null && IsReferencedTable(table))
			{
				...
				try
				{
					...
					if (doc != null && doc.MergeTable != null && doc.MergeTable.Value != 0)
					{
						...
						var node = project?.GetNodeById(new Auditai.DTO.Id64(doc.MergeTable.Value)) as TreeTableNode;
						if (node != null)
						{
				...
				if (dt != null && AuditaiBookmark.TryParse(dt.TargetName, out var lsbm) && lsbm.TableId != null)
				{
					Auditai.DTO.Id64 id = Auditai.DTO.Id64.ParseBase64(lsbm.TableId);
					var project = Project.Current;
		...
		try
		{
			...
			try
			{
				new ProgressForm<object>(async delegate(IProgress<Auditai.DTO.ProgressInfo> iProg)
				{
					...
					{
						iProg.Report(new Auditai.DTO.ProgressInfo
						{
	...
	private void HandleTraceNavigation(string href, object tag)
	{
		try
		{
			if (href.StartsWith("tracecell_") && tag is Auditai.Model.Cell cell)
			{
				NavigateToCell(cell);
			}
			else if (href.StartsWith("tracecol_") && tag is Auditai.Model.Column column)
			{
	...
	/// 导航到单元格
	/// </summary>
	private void NavigateToCell(Auditai.Model.Cell cell)
	{
	...
	/// 导航到列
	/// </summary>
	private void NavigateToColumn(Auditai.Model.Column column)
	{
	...
	private bool IsReferencedTable(TXTextControl.Table table)
	{
		...
		try
		{
			...
			if (dt != null && !string.IsNullOrEmpty(dt.TargetName) && AuditaiBookmark.TryParse(dt.TargetName, out _))
				return true;
	...
	/// </summary>
	private Auditai.DTO.Id64? GetSourceTableId(TXTextControl.Table table = null)
	{
		try
		{
			if (table == null)
			...
			if (table != null)
			{
				...
				if (firstCell != null)
				{
					...
					_tx.Select(targetPos, 0);
					DocumentTarget dt = _tx.DocumentTargets.GetItem();
					if (dt != null && !string.IsNullOrEmpty(dt.TargetName) && AuditaiBookmark.TryParse(dt.TargetName, out var lsbm))
					{
						// 有 TableId，直接返回
						if (lsbm.TableId != null)
							{
								...
								return Auditai.DTO.Id64.ParseBase64(lsbm.TableId);
							}
							// 没有 TableId，先从缓存恢复
							if (lsbm.ParaIdBase64 != null)
							{
								...
								{
									return Auditai.DTO.Id64.ParseBase64(cachedTableId);
								}
						...
						// 方法1b：枚举所有 DocumentTarget，查找位置在表格前面最近的 AuditaiBookmark
						var allTargets = _textControl.DocumentTargets;
						if (allTargets != null)
						{
							int tableStart = firstCell.Start;
							...
							foreach (DocumentTarget target in allTargets)
							{
								...
								{
									if (string.IsNullOrEmpty(target.TargetName)) continue;
									if (!AuditaiBookmark.TryParse(target.TargetName, out var bm)) continue;
									if (bm.TableId == null && bm.ParaIdBase64 == null) continue;
							...
							if (bestBookmark != null)
							{
								if (bestBookmark.TableId != null)
								{
									...
									if (bestBookmark.ParaIdBase64 != null)
									_bookmarkTableIdCache[bestBookmark.ParaIdBase64] = bestBookmark.TableId;
								return Auditai.DTO.Id64.ParseBase64(bestBookmark.TableId);
							}
							if (bestBookmark.ParaIdBase64 != null)
							{
								// 先检查缓存
				...
				try
				{
					...
					if (doc != null && doc.MergeTable != null && !doc.MergeTable.IsZero())
					{
						...
						if (project != null)
						{
							...
							if (mergeTable != null)
							{
								return new Auditai.DTO.Id64(doc.MergeTable.Value);
							}
						}
					}
				}
	...
	private void DeleteTable(TXTextControl.Table table)
	{
		try
		{
			if (Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.Question, "确定要删除此表格吗？") == DialogResult.Yes)
			{
				HideTableContextMenu();
				DetachEvents();
				try
	...
	private void TrackTable(TXTextControl.Table table)
	{
		try
		{
			...
			if (fields != null && fields.Count > 0)
			{
				...
				var referredTableIds = new HashSet<Auditai.DTO.Id64>();
	...
	void TagCurrentPOrTableModified(int position)
	{
		PreserveSelection(delegate
		{
			...
			if (dt != null && AuditaiBookmark.TryParse(dt.TargetName, out var bookmark))
			{
				bookmark.TagModifiedIfNotNew();
	...
	private void SetTableBorder(TXTextControl.Table txTable, Auditai.Model.TableBorderStyle tableStyle, int captionLayers = 1)
	{
			...
			try
			{
				...
				foreach (TXTextControl.TableCell cell in txTable.Cells)
				{
					try
					{
						...
						int upDownWidth = tableStyle.UpDownLine == Auditai.Model.LineStyle.Thick ? thickWidth
							: (tableStyle.UpDownLine == Auditai.Model.LineStyle.Thin ? thinWidth : 0);
						// Left/Right border
						int leftRightWidth = tableStyle.LeftRightLine == Auditai.Model.LineStyle.Thick ? thickWidth
							: (tableStyle.LeftRightLine == Auditai.Model.LineStyle.Thin ? thinWidth : 0);
						// Body line
						int bodyWidth = tableStyle.BodyLine == Auditai.Model.LineStyle.Thick ? thickWidth
							: (tableStyle.BodyLine == Auditai.Model.LineStyle.Thin ? thinWidth : 0);
						// Second line (between caption and body)
						int secondWidth = tableStyle.SecondLine == Auditai.Model.LineStyle.Thick ? thickWidth
							: (tableStyle.SecondLine == Auditai.Model.LineStyle.Thin ? thinWidth : 0);
	...
	private void SetCustomTableBorder(TXTextControl.Table txTable, Auditai.Model.TableBorderStyle style, int captionLayers = 1)
	{
	...
	/// 批量应用表格样式到多个表格
	/// </summary>
	/// <param name="tablesInRange">范围内的表格书签列表 (Position, AuditaiBookmark)</param>
	/// <param name="style">要应用的边框样式</param>
	/// <param name="progressCallback">进度回调 (current, total)，可为 null</param>
	/// <returns>成功应用的表格数量</returns>
	public int BatchApplyTableStyle(List<(int Position, AuditaiBookmark Bookmark)> tablesInRange, Auditai.Model.TableBorderStyle style, Action<int, int> progressCallback = null)
	{
			...
			try
			{
				for (int i = 0; i < tablesInRange.Count; i++)
				{
					...
					try
					{
						...
						var updatedBookmark = new AuditaiBookmark
						{
							...
							TableStyle = Auditai.Model.TableBorderStyles.Custom, // 6
