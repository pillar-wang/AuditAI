...
namespace Auditai.LocalDataStore
{
    ...
    public static class LocalDataStore
    {
        ...
        public static async Task<Auditai.DTO.User> GetUserById(long userId)
        {
            using var conn = CreateConnection();
            using var cmd = new SQLiteCommand(
            ...
            if (!reader.Read())
            {
                return new Auditai.DTO.User { Id = userId, Name = "管理员", UserName = "admin" };
            }
            return ReadUserFromReader(reader);
        }

        public static async Task UpdateUserInfo(Auditai.DTO.User userInfo)
        {
            using var conn = CreateConnection();
            new SQLiteCommand(@"
                UPDATE Users SET
            ...
            // 同步到内存模型
            if (Auditai.Model.User.Current != null && Auditai.Model.User.Current.Id == userInfo.Id)
            {
                Auditai.Model.User.Current.Name = userInfo.Name;
                Auditai.Model.User.Current.UserName = userInfo.UserName;
            }
        }

        public static async Task UpdatePhoneInfo(Auditai.DTO.User userInfo, string validateCode)
        {
            // 本地模式不校验验证码，直接更新手机号
            await UpdateUserInfo(userInfo);
        }

        private static Auditai.DTO.User ReadUserFromReader(System.Data.SQLite.SQLiteDataReader reader)
        {
            return new Auditai.DTO.User
            {
        ...
        {
            var projects = new List<Auditai.DTO.Project>();
        ...
        private static List<Auditai.DTO.User> GetProjectMembers(Guid projectId)
        {
            var users = new List<Auditai.DTO.User>();
            try
            {
                using var conn = CreateConnection();
                using var cmd = new SQLiteCommand(@"
        ...
        {
            var templates = new List<Auditai.DTO.Project>();
            ...
            foreach (var dbFile in Directory.GetFiles(_templatesPath, "*.db"))
            {
                ...
                try
                {
                    ...
                    if (reader.Read())
                    {
                        var template = new Auditai.DTO.Project
                        {
                ...
                foreach (var dbFile in Directory.GetFiles(_templatesPath, "*.db"))
                {
                    try
                    {
                        ...
                        if (reader.Read())
                        {
                            ...
                            if (id == templateId)
                            {
                                return new Auditai.DTO.Project
                                {
                                    Id = id,
                                    Name = reader["Name"].ToString(),
                                    Number = reader["Number"]?.ToString() ?? "",
        ...
        /// </summary>
        public static void RegisterImportedProject(Auditai.DTO.Project project)
        {
            using var conn = CreateConnection();
            EnsureProjectColumns(conn);
        ...
        /// </summary>
        private static void CreateTemplateDbFile(Auditai.DTO.Project project)
        {
            Directory.CreateDirectory(_templatesPath);
        ...
        /// </summary>
        private static void CreateProjectDbFile(Auditai.DTO.Project project)
        {
            // 确定项目数据库文件路径：与 GetDbPathByGuid 保持一致，使用 data/{userId}/{projectId}.db
            // GetDbPathByGuid 使用 Auditai.Model.User.Current.Id 作为子目录
            long userId = Auditai.Model.User.Current?.Id ?? 1;
        ...
        public static async Task<PullTable> PullTable(JObject request)
        {
            ...
            {
                var pushTable = Auditai.DTO.PushTable.Parser.ParseFrom(tableData);
        ...
        public static async Task<PullDocument> PullDocument(JObject request)
        {
            ...
            {
                var pushDoc = Auditai.DTO.PushDocument.Parser.ParseFrom(data);
