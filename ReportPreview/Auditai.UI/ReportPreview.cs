...
public class ReportPreview
{
	...
	private void TableCreatePaperImpl()
	{
		...
		if (PageSetup.FitPageHeight)
		{
			...
			double num4 = new Unit(Table.Rows.Sum((Auditai.Model.Row row) => row.Height) + Table.SumHeaderHeight(Table.GetNumCaptionRows()), UnitTypeEnum.Pixel).ConvertUnit(ScreenGraphics.DpiY, UnitTypeEnum.Inch, 0f);
