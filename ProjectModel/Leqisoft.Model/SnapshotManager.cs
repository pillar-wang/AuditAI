...
public class SnapshotManager
{
	...
	public void SaveSnapshot(Table table, bool isDeleting)
	{
		...
		Leqisoft.DTO.Table dto = table.ToDto();
	...
	public void SaveSnapshot(Document document, bool isDeleting)
	{
		...
		Leqisoft.DTO.Document dto = document.ToDto();
	...
	public void SaveSnapshot(Image image, bool isDeleting)
	{
		...
		Leqisoft.DTO.Image dto = image.ToDto();
	...
	public void SaveSnapshot(Pdf pdf, bool isDeleting)
	{
		...
		Leqisoft.DTO.Pdf dto = pdf.ToDto();
		pdfDAL.SavePdf(dto);
		if (isDeleting)
		{
	...
	public TreeTableNode GetSnapshotTable(SnapshotInfo si)
	{
		...
		Leqisoft.DTO.Table table = tableDAL.GetTable();
		...
		foreach (Leqisoft.DTO.CellStyle cellStyle2 in tableDAL.GetCellStyles())
		{
		...
		foreach (Leqisoft.DTO.Column column2 in tableDAL.GetColumns())
		{
		...
		foreach (Leqisoft.DTO.Row row2 in tableDAL.GetRows())
		{
			Row row = new Row
			{
				Table = treeTableNode.Table,
		...
		List<Leqisoft.DTO.Cell> list = tableDAL.GetCells().ToList();
		...
		for (int i = 0; i < treeTableNode.Table.Rows.Count; i++)
		{
			...
			{
				Leqisoft.DTO.Cell cell = list[i * treeTableNode.Table.Columns.Count + j];
				Cell cell2 = new Cell
				{
	...
	public TreeDocumentNode GetSnapshotDocument(SnapshotInfo si)
	{
		...
		Leqisoft.DTO.Document document = documentDAL.GetDocument();
		treeDocumentNode.Document.SectPr = document.SectPr;
		treeDocumentNode.Document.MergeTable = document.MergeTable;
		foreach (Leqisoft.DTO.Paragraph paragraph2 in documentDAL.GetParagraphs())
		{
	...
	public TreeImageNode GetSnapshotImage(SnapshotInfo si)
	{
		...
		Leqisoft.DTO.Image image = imageDAL.GetImage();
	...
	public TreePdfNode GetSnapshotPdf(SnapshotInfo si)
	{
		...
		Leqisoft.DTO.Pdf pdf = pdfDAL.GetPdf();
