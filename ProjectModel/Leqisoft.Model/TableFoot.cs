...
{
	private const int MAX_FOOT_ROWS = 10;
	...
	internal TableFoot(Table table)
	{
		Table = table;
		Version = 1;
	}
	...
	public void InsertRow(int index, bool useNextRowStyle)
	{
		if (!CanAddRow)
		{
			throw new InvalidOperationException($"表底行不能多于{10}行");
		}
	...
	private void Upgrade_ValueFormula(JObject cell)
	{
		...
		else
		{
			try
			{
				...
				string text3 = formulaEvaluator.EvaluateToOperand().ToString();
				text2 = text;
				cell["Value"] = text3;
			}
	...
	{
		TableTitleRow tableTitleRow = Rows[row];
