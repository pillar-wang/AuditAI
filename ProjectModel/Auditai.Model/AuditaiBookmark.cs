...
public class AuditaiBookmark
{
	public string ParaIdBase64 { get; set; }

	public AuditaiBookmarkStatus Status { get; set; }
	...
	/// </summary>
	public static bool TryParse(string s, out AuditaiBookmark lsbm)
	{
		lsbm = null;
		if (string.IsNullOrEmpty(s) || !s.StartsWith("lsbm"))
			return false;

		lsbm = new AuditaiBookmark();
		int pos = 4; // 跳过 "lsbm"
			...
			switch (key)
			{
				...
				case '1':
				{
					...
					if (int.TryParse(statusStr, out var st))
						lsbm.Status = (AuditaiBookmarkStatus)st;
					break;
				}
				case '2':
	...
	public void TagModifiedIfNotNew()
	{
		if (Status != 0)
		{
			Status = AuditaiBookmarkStatus.Modified;
		}
	}
}