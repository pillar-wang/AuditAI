...
public class CrossProjectFormulaStore
{
    ...
    /// <summary>
    /// 延迟计算的项目数据库路径
    /// 不在构造函数中缓存，因为构造时 project.Id 可能还是 Guid.Empty
    /// </summary>
    private string _dbPath => GetProjectDbPath(_project);

    public CrossProjectFormulaStore(Project project)
    {
        _project = project;
        // 不在构造函数中调用 EnsureTable()，因为此时 project.Id 可能还是 Guid.Empty
        // EnsureTable() 改为在 Load()/Save() 时延迟调用
    }
