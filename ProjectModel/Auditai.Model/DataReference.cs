...
public class DataReference
{
	...
	internal Auditai.DTO.DataReference ToDto()
	{
		return new Auditai.DTO.DataReference
		{
			Dirty = Dirty,
			Id = Id,
			Key = Key,
	...
	private string EvaluateBuiltIn(DataReferenceEvaluationContext context)
	{
		...
		case "全体成员姓名":
			return context.Project?.Users != null
				? string.Join("|", context.Project.Users.Select((KeyValuePair<Auditai.DTO.User, UserRole> u) => u.Key.Name))
				: string.Empty;
		case "当前日期":
		...
		case "当前用户角色":
			if (context.Project?.Users == null || User.Current == null)
				return string.Empty;
			var roleKv = context.Project.Users.FirstOrDefault((KeyValuePair<Auditai.DTO.User, UserRole> kv) => kv.Key.Id == User.Current.Id);
			return roleKv.Equals(default(KeyValuePair<Auditai.DTO.User, UserRole>)) ? string.Empty : GetUserRoleString(roleKv.Value);
		case "换行":
			return "\n";
		case "当前组织名称":
		...
		case "全体成员账号":
			return context.Project?.Users != null
				? string.Join("|", context.Project.Users.Select((KeyValuePair<Auditai.DTO.User, UserRole> u) => u.Key.UserName))
				: string.Empty;
		default:
