...
internal class VoucherExporter : LedgerExporter
{
	...
	private const int CN_DEBIT = 6;

	private const int CN_CREDIT = 7;

	private const int CN_MAKER = 8;

	private const int CN_CHECKER = 9;

	private const int CN_BOOKER = 10;

	private Ledger _ledger;

	private List<Voucher> _vouchers;

	public VoucherExporter(Ledger ledger, IEnumerable<Voucher> vouchers)
	{
		_ledger = ledger;
	...
	public override void Build()
	{
		...
		xLSheet[num, 6].SetValue("借方金额", styleHCenter);
		xLSheet.Columns[6].Width = C1XLBook.PixelsToTwips(110.0);
		xLSheet[num, 7].SetValue("贷方金额", styleHCenter);
		xLSheet.Columns[7].Width = C1XLBook.PixelsToTwips(110.0);
		xLSheet[num, 8].SetValue("制单人", styleHCenter);
		xLSheet[num, 9].SetValue("审核人", styleHCenter);
		xLSheet[num, 10].SetValue("记账人", styleHCenter);
		foreach (Voucher voucher in _vouchers)
		{
			...
			xLSheet[num, 6].SetValue(voucher.IsDebit ? LedgerExporter.EmptyIf0(voucher.Amount) : string.Empty, styleMoney);
			xLSheet[num, 7].SetValue(voucher.IsDebit ? string.Empty : LedgerExporter.EmptyIf0(voucher.Amount), styleMoney);
			xLSheet[num, 8].SetValue(voucher.Maker, styleBorder);
			xLSheet[num, 9].SetValue(voucher.Checker, styleBorder);
			xLSheet[num, 10].SetValue(voucher.Booker, styleBorder);
		}
