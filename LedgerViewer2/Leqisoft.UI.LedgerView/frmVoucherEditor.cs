...
#pragma warning restore CS0649
	...
	{
		base.Icon = Leqisoft.UI.Controls.Theme.SelectedLeqiTheme.GetThemedIcon(Resources.largeModifyVoucher);
	}
	...
	private bool ValidateAndRead()
	{
		...
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "凭证字不能为空");
		...
		if (DateTime.TryParse(txtVoucherDate.Text.Trim(), out var result))
		{
			...
			{
				Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "该凭证号当月已存在");
				return false;
			}
			...
			for (int j = grdVoucher.Rows.Fixed; j < grdVoucher.Rows.Count; j++)
			{
				...
				{
					Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "科目代码必须为末级科目");
					grdVoucher.Row = j;
					grdVoucher.Col = grdVoucher.Cols["Code"].Index;
					return false;
				}
				...
				if (accountBalanceWithCache.ClassBalances.Count > 0)
				{
					...
					if (item == null)
					{
						Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "科目（" + tuple.Item1.Code + "）" + tuple.Item1.Name + "为带辅助核算科目，必须为凭证挂接辅助核算项");
						grdVoucher.Row = j;
						grdVoucher.Col = grdVoucher.Cols["Code"].Index;
						return false;
					}
					foreach (AuxiliaryClass auxClass in accountBalanceWithCache.ClassBalances.Keys)
					{
						...
						{
							Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "该凭证未为辅助核算类（" + auxClass.Code + "）" + auxClass.Name + "挂接项目");
							grdVoucher.Row = j;
							grdVoucher.Col = grdVoucher.Cols["Code"].Index;
							return false;
						}
			...
			if (list.Count == 0)
			{
				Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "凭证为空");
				return false;
			}
			if (list.Sum((Voucher v) => (!v.IsDebit) ? (-v.Amount) : v.Amount) != 0m)
			{
				Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "会计凭证借贷发生额不平衡");
				return false;
			}
		...
		Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "日期格式不正确");
	...
	private void PreviousVoucherGroup()
	{
		...
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "当前账套没有凭证数据");
			return;
		}
	...
	{
		if (DialogResult.OK != Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "凭证删除后无法恢复，确定要删除该凭证吗？", MessageBoxButtons.OKCancel))
		{
			return;
		}
		foreach (Voucher item in currentVoucherGroup)
	...
	private void InitializeComponent()
	{
		...
		this.cmdAddVoucher.Image = Leqisoft.UI.LedgerView.Properties.Resources.largeCreateVoucher;
		...
		this.cmdModifyVoucher.Image = Leqisoft.UI.LedgerView.Properties.Resources.largeModifyVoucher;
		...
		this.cmdDeleteVoucher.Image = Leqisoft.UI.LedgerView.Properties.Resources.largeRemoveVoucher;
		...
		this.cmdCancelSave.Image = Leqisoft.UI.LedgerView.Properties.Resources.largeCancelSave;
		...
		this.cmdSaveVoucher.Image = Leqisoft.UI.LedgerView.Properties.Resources.saveData;
		...
		this.cmdPreviousVoucher.Image = Leqisoft.UI.LedgerView.Properties.Resources.largePreviousVoucher;
		...
		this.cmdNextVoucher.Image = Leqisoft.UI.LedgerView.Properties.Resources.largeNextVoucher;
