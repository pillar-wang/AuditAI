...
#pragma warning restore CS0649
	...
	{
		base.Icon = Leqisoft.UI.Controls.Theme.SelectedLeqiTheme.GetThemedIcon(Resources.OpenExcelLedger);
	}
	...
	{
		if (e.CloseReason == CloseReason.UserClosing && !_hasGenerated && DialogResult.OK != Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "关闭后当前窗体数据将被放弃，确定要关闭吗？", MessageBoxButtons.OKCancel))
		{
			e.Cancel = true;
		}
	}
	...
	private void GrdBalance_AppendTotalRow()
	{
		if (grdBalance.Rows.Count <= 20000)
		{
			...
			CrawlerModelAlias::Leqisoft.Model.Ledger accounts = ledgerBuilder.GetAccounts("人民币");
			decimal num = accounts.Accounts.Where((CrawlerModelAlias::Leqisoft.Model.Account a) => a.Children.Count == 0 && a.IsDebit).Sum((CrawlerModelAlias::Leqisoft.Model.Account a) => a.Balance);
			decimal num2 = accounts.Accounts.Where((CrawlerModelAlias::Leqisoft.Model.Account a) => a.Children.Count == 0 && !a.IsDebit).Sum((CrawlerModelAlias::Leqisoft.Model.Account a) => a.Balance);
	...
	private async Task CmdPaste_Click(C1FlexGridEx grid)
	{
		...
		{
			exception.Log();
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "复制粘贴的数据存在异常，请尝试重新复制。");
		}
	}
	...
	private void Frf_FindNextHandler(object sender, FindNextEventArgs e)
	{
		...
		if (selection.IsSingleCell)
		{
			...
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "已查找到表格结尾");
			return;
		}
		...
		try
		{
			...
			if (e.IsReplaceAll)
			{
				if (selection.IsSingleCell)
				{
					...
					Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, $"共替换 {num} 处。");
					return;
				}
				int num2 = 0;
				for (int k = selection.TopRow; k <= selection.BottomRow; k++)
				...
				Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, $"共替换 {num2} 处。");
				return;
			}
			if (selection.IsSingleCell)
			{
	...
	private async Task<bool> ValidateFill(bool save)
	{
		try
		{
			...
			CrawlerModelAlias::Leqisoft.Model.Ledger ledger = LedgerBuilder3.EMPTY_LEDGER;
		...
		catch (ImportException2 importException)
		{
			...
			case FailureReasonEnum.AccountLevelCannotCreate:
			{
				...
				if (importException.FailureContext.Table == TableEnum.BALANCE)
				{
					...
					if (flag2)
					{
						Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "科目代码或科目名称不规范，无法生成科目级次");
					}
				}
				else
				{
			...
			case FailureReasonEnum.SpecifyVoucherDebitNotEqualsCredit:
			{
				...
				if (flag4)
				{
					Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "凭证" + tuple2.Item2 + "借贷金额不平");
				}
				break;
			}
			case FailureReasonEnum.AuxiliaryBeginBalanceNotEqualsAccountBegin:
			{
				...
				if (!flag)
				{
					Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, text3);
				}
				break;
			}
			...
			case FailureReasonEnum.SpecificMessage:
				Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, importException.FailureContext.UserData?.ToString());
				break;
			}
		}
	...
	private async void cmdValidate_Click(object sender, ClickEventArgs e)
	{
		try
		{
			...
			{
				Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "校验通过！");
			}
		}
		catch (Exception ex)
		{
	...
	private async void cmdGenerate_Click(object sender, ClickEventArgs e)
	{
		try
		{
			...
			{
				Hide();
				Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "已成功生成账套，生成后的账套文件存储在" + SavePath);
	...
	{
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(Leqisoft.UI.LedgerView.frmImport));
		...
		this.grdBalance = new Leqisoft.UI.Controls.C1FlexGridEx();
		this.tabAuxiliary = new C1.Win.C1Command.C1DockingTabPage();
		this.grdAuxiliary = new Leqisoft.UI.Controls.C1FlexGridEx();
		this.tabVoucher = new C1.Win.C1Command.C1DockingTabPage();
		this.grdVoucher = new Leqisoft.UI.Controls.C1FlexGridEx();
		...
		this.cmdHelpDoc.Image = Leqisoft.UI.LedgerView.Properties.Resources.HelpCenter;
		...
		this.cmdReplace.Image = Leqisoft.UI.LedgerView.Properties.Resources.Replace;
