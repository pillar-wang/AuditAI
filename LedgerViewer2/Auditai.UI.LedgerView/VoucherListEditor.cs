...
public class VoucherListEditor : ISetTheme
{
	...
	private void CmdSidebarCancelMarkVoucher_Click(object sender, ClickEventArgs e)
	{
		...
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "请选择取消关注的凭证行");
		}
	}
	...
	private void BindTreeContexMenu()
	{
		try
		{
			...
			{
				if (Tree.Row < Tree.Rows.Fixed || Tree.Row >= Tree.Rows.Count || DialogResult.OK != Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "凭证删除后无法恢复，确定要删除该凭证吗？", MessageBoxButtons.OKCancel))
				{
					return;
				}
				try
