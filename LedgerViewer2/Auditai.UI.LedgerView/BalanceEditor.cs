...
internal class BalanceEditor : ISetTheme
{
	...
	{
		Auditai.UI.Controls.Theme.SetCurrentObject(dteStart);
		dteStart.OpenDropDown();
	}

	private void DatEnd_DoubleClick(object sender, EventArgs e)
	{
		Auditai.UI.Controls.Theme.SetCurrentObject(dteEnd);
		dteEnd.OpenDropDown();
	}
		...
		cmdModifyStartDate.Click += delegate
		{
			...
			if (dateTime.HasValue)
			{
				...
				if (Ledger.Vouchers.Count > 0)
				{
					...
					{
						Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "账套起始日期的设置必须小于凭证的最小凭证日期");
						return;
					}
				}
