...
public class LedgerViewer
{
	...
	public LedgerViewer(MultiLedgerViewer owner)
	{
		...
		lazyAccountTreeEditor = new Lazy<AccountTreeEditor>(delegate
		{
			...
			Auditai.UI.Controls.Theme.SetCurrentTree(pnlTree);
		...
		lazySubsidiaryEditor = new Lazy<SubsidiaryEditor>(delegate
		{
			...
			Auditai.UI.Controls.Theme.SetCurrentTree(subsidiaryEditor.View);
		...
		lazySummaryEditor = new Lazy<SummaryEditor>(delegate
		{
			...
			Auditai.UI.Controls.Theme.SetCurrentTree(summaryEditor.View);
		...
		lazyLedgerAgingEditor = new Lazy<LedgerAgingEditor>(delegate
		{
			...
			Auditai.UI.Controls.Theme.SetCurrentTree(ledgerAgingEditor.View);
		...
		lazyVoucherListEditor = new Lazy<VoucherListEditor>(delegate
		{
			...
			Auditai.UI.Controls.Theme.SetCurrentObject(voucherListEditor.Tree);
			Auditai.UI.Controls.Theme.SetCurrentTree(voucherListEditor.View);
		...
		lazyTrendencyEditor = new Lazy<TrendencyEditor>(delegate
		{
			...
			Auditai.UI.Controls.Theme.SetCurrentTree(trendencyEditor.View);
		...
		lazyStructureEditor = new Lazy<StructureEditor>(delegate
		{
			...
			Auditai.UI.Controls.Theme.SetCurrentTree(structureEditor.View);
		...
		lazyValidateEditor = new Lazy<ValidateEditor>(delegate
		{
			...
			Auditai.UI.Controls.Theme.SetCurrentTree(validateEditor.View);
		...
		Auditai.UI.Controls.Theme.SetCurrentTree(ctnView);
		SetTheme();
	...
	private void OpenExcelTemplate()
	{
		...
		{
			ex.Log();
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "生成序时账模板错误，请先关闭打开的序时账模板文件。错误信息：" + ex.Message);
		}
		catch (Exception ex2)
		{
			ex2.Log();
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "打开序时账模板错误！" + ex2.Message);
		}
	}
	...
	public string MergeLedger()
	{
		try
		{
			...
			if (openFileDialog.ShowDialog() == DialogResult.OK)
			{
				...
				if (text != null)
				{
					Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "账套合并完成，合并后生成的账套文件为：" + text);
					return text;
				}
			}
			return null;
		...
		{
			ex2.Log();
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "合并错误！" + ex2.Message);
			return null;
		}
		catch (Exception ex3)
		{
	...
	public void ShowLedgerAgeAnalazy()
	{
		...
		try
		{
			...
			{
				Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "账期小于1年，无法进行账龄分析！");
				return;
			}
	...
	public void FillToTable()
	{
		try
		{
			...
			case ActiveView.VoucherList:
				...
				break;
			default:
				Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "当前页面不支持采账填充");
				break;
			}
		}
		catch (Exception ex)
	...
	public void PrintPreview(bool toPreview)
	{
		try
		{
			if (toPreview)
			{
				...
				{
					Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, previewNotSupport.Message);
					throw;
				}
			}
	...
	public async Task ModifyVoucher(Voucher voucher)
	{
		try
		{
			...
			{
				Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "会计凭证修改成功，即将重新加载账套");
...
#pragma warning restore SCS0005
		...
		{
			if (Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "当前路径包含同名文件是否覆盖？", MessageBoxButtons.OKCancel) == DialogResult.OK)
			{
