...
#pragma warning restore CS0649
	...
	{
		base.Icon = Theme.SelectedAuditaiTheme.GetThemedIcon(Resources.largeModifyVoucher);
	}
	...
	private bool ValidateAuxiliary()
	{
		...
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "辅助核算代码不能为空");
			return false;
		}
		if (string.IsNullOrEmpty(txtAccountName.Text.Trim()))
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "辅助核算名称不能为空");
			return false;
		}
		if (isAdd)
		{
			...
			{
				Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "该辅助核算项代码已存在，辅助核算项代码不能重复。");
				return false;
			}
		}
		else if (ledger.AuxiliaryClasses.Any((AuxiliaryClass c) => c != _operatingAuxClass && c.Code == txtAccountCode.Text.Trim()))
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "该辅助核算类代码已存在，辅助核算类代码不能重复。");
			return false;
		}
		return true;
	}
	...
	private bool ValidateAccount()
	{
		...
		if (list.Count > 0)
		{
			...
			{
				Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "科目代码不符合编码规则，请修改！");
				return false;
			}
		...
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "该科目的科目代码已存在，科目代码不能重复。");
			return false;
		}
		if (ParentAccount != null && ParentAccount.Children.Count == 0 && !_viewer.CacheManager.IsEmptyAccountWithCache(ParentAccount))
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "上级科目\u00a0(" + ParentAccount.Code + ")" + ParentAccount.Name + "\u00a0存在数据，不能在其下增加子科目。");
			return false;
		}
		return true;
	}
