...
public class LedgerDefaultPanel : ISetTheme
{
	...
	private async void CmdCancelRecieve_Click(object sender, ClickEventArgs e)
	{
		...
		if (DialogResult.OK == Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "确定要取消接收吗？", MessageBoxButtons.OKCancel))
		{
	...
	{
		Tile tileAt = _tileControl.GetTileAt(contextMenuLocation);
		if (DialogResult.OK == Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "确定要取消发送吗？", MessageBoxButtons.OKCancel))
		{
	...
	private void PopulateRecievingFile()
	{
		try
		{
			...
			if (group == null)
			{
				...
				}, new string[0], Auditai.UI.LedgerView.Properties.Resources.ledger2);
			...
			foreach (KeyValuePair<string, FileCache> item4 in dictionary)
			{
				...
				Tile tile = CreateFileTile(value.FileInfo.Name, Auditai.UI.LedgerView.Properties.Resources.ledger2, new TileInfo
				{
	...
	private void OpenLocation()
	{
		if (!(_selectedTile?.Tag is TileInfo tileInfo) || !tileInfo.TileFlag.HasFlag(TileFlag.LocalFile))
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "请先选择账套！");
			return;
		}
	...
	private void DeleteLedger()
	{
		...
		try
		{
			...
			{
				Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "当前账套在打开状态，无法删除");
			}
			else if (DialogResult.OK == Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "确定要删除选定的账套文件吗？", MessageBoxButtons.OKCancel))
			{
	...
	private void RenameLedger()
	{
		...
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "该文件已存在，请使用其他名称");
			return;
		}
		try
		{
		...
		{
			Auditai.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "文件重命名失败！失败原因：" + ex.Message);
		}
	}
	...
	public void Populate(string selected = null)
	{
		...
		try
		{
			string text = Auditai.Model.Project.Current?.Id.ToString();
			if (text == null)
			{
				appendDemoGroup();
				appendReceivedDir();
		...
		if (Directory.Exists(folder))
		{
			...
			if (files.Length != 0)
			{
				...
				_tileControl.Groups.Add(CreateFilesGroup(new TileInfo
				{
					TileFlag = TileFlag.Group,
					LocalFile = fullPath
				}, files, Auditai.UI.LedgerView.Properties.Resources.ledger2));
			}
		}
	}
