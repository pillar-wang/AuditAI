...
internal class FlexGridDecorator
{
	...
	public void SetTable(Leqisoft.Model.Table table)
	{
		_table = table;
	}
	...
	public void Prepare()
	{
		...
		{
			if (!long.TryParse(item.UserState.TableCellId, out var result) || item.Id == Leqisoft.Model.User.Current.Id.ToString())
			{
				continue;
			}
			...
			try
			{
				Leqisoft.Model.Cell cellById = _table.GetCellById(new Id64(result));
				if (cellById != null)
				{
	...
	private void Trigger_Elapsed(object sender, EventArgs e)
	{
		...
		else
		{
			...
			try
			{
				...
				foreach (Leqisoft.Model.Cell key in DrawCollection.Values.Keys)
				{
	...
	private void Grid_OwnerDrawCell(object sender, OwnerDrawCellEventArgs e)
	{
		try
		{
			...
			Leqisoft.Model.Cell cell = _table.Cells.Get(num, num2);
