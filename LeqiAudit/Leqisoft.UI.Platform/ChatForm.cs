...
public class ChatForm : C1RibbonForm, ISetTheme
{
	...
	public ChatForm()
	{
		...
		string arg = Path.Combine(directoryName, "config", "record", $"{Leqisoft.Model.User.Current.Id}.db");
	...
	{
		base.Icon = Leqisoft.UI.Controls.Theme.SelectedLeqiTheme.GetThemedIcon(Resources.chat);
	}
	...
	private void ChatForm_RePopulate(object sender, EventArgs e)
	{
		...
		Group group2 = instance.GetGroup(Leqisoft.Model.User.Current.TeamId.ToString());
	...
	public void SetTheme()
	{
		...
		C1Theme c1Theme = Leqisoft.UI.Controls.Theme.SelectedLeqiTheme.GetC1Theme();
	...
	private async void CmdRefreshProjectMembers_Click(object sender, ClickEventArgs e)
	{
		...
		TeamMemberViewEditor.Populate(instance.GetGroup(Leqisoft.Model.User.Current.TeamId.ToString())?.GetSelfAndMembers());
	...
	private async Task ChangeHeaderImpl()
	{
		...
		try
		{
			...
			if (openFileDialog.ShowDialog() == DialogResult.OK)
			{
				...
				Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "头像更新成功");
				ChatManager.OnAfterHeadChanged(saveImage);
				await SignalRClient.ChangeMemberInfo(Leqisoft.Model.User.Current.Id.ToString());
			}
		}
		catch (Exception ex)
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "修改头像失败，失败原因：" + ex.Message);
		}
	}
	...
	private async Task PushTreeNodeImpl()
	{
		...
		if (memTab == null)
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "请先选择用户沟通！");
			return;
		}
		if (memTab is Group)
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "群聊不支持推送文件");
			return;
		}
		...
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "用户当前不在本" + StringConstBase.Current.Project + "，无法推送文件");
			return;
		}
		TreeNodeBase selectedNode = Program.MainForm.ProjectHierarchy.SelectedNode;
		if (selectedNode == null)
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "您当前未选择任何节点，无法推送文件");
			return;
		}
		JObject jObject = new JObject();
		jObject.Add("nodeId", selectedNode.Id.ToString());
		jObject.Add("nodeName", selectedNode.Name);
		jObject.Add("name", Leqisoft.Model.User.Current.Name);
		NotifyMessage notifyMessage = new NotifyMessage
		{
			Bullet = true,
			Value = jObject.ToString(),
			Text = Leqisoft.Model.User.Current.Name + "向您推送来 " + selectedNode.Name
		};
	...
	private async Task SendMessageImpl()
	{
		...
		try
		{
			...
			{
				Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "发送消息不能为空！");
				return;
			}
			txtMessage.Text = string.Empty;
			ChatRecord record = new ChatRecord(memTab.Id, Leqisoft.Model.User.Current.Id.ToString(), text);
			recordPersistence.AddRecord(record);
			...
			if (memTab is Group)
			{
				if (memTab.Id == Leqisoft.Model.User.Current.TeamId.ToString())
				{
	...
	{
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(Leqisoft.UI.Platform.ChatForm));
		...
		this.tabProjectMembers.Image = Leqisoft.UI.Platform.Properties.Resources.member;
		...
		this.tabTeamMembers.Image = Leqisoft.UI.Platform.Properties.Resources.group;
