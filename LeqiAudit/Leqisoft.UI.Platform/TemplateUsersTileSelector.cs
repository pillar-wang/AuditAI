...
public class TemplateUsersTileSelector
{
	...
	public TemplateUsersTileSelector()
	{
	...
	private void _tileControl_TileClicked(object sender, TileEventArgs e)
	{
		...
		c1Command.Text = "可编辑的用户";
		c1Command.UserData = Tuple.Create(tile, UserRole.Editor);
		...
		c1Command2.Text = "可使用的用户";
		c1Command2.UserData = Tuple.Create(tile, UserRole.User);
		...
		contextMenu.ShowContextMenu(_tileControl, new Point(e.Tile.Group.X + e.Tile.X, e.Tile.Group.Y + e.Tile.Y + tile.Height + _tileControl.ScrollOffset));
	}
	...
	private bool IsUserVisible(User u)
	{
		return Context.UserViewStates.Find((Tuple<User, bool> tup) => tup.Item1.Id == u.Id).Item2;
	}
	...
	public void PopulateUsers()
	{
		...
		void AppendGroup(UserGroup ug)
		{
			...
			foreach (User user3 in ug.Users)
			{
	...
	public void SetTheme()
	{
		...
		if (_tileControl.AllowMultiSelect)
		{
			_tileControl.HotBorderColor = Color.Transparent;
		}
		else
		{
	...
	public IEnumerable<User> ValidateAndGetUsers()
	{
		...
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "成员至少要包含一名可编辑的用户");
			return null;
		}
		return list;
	}
