...
{
	private class ValidateResultEqualityComparer : IEqualityComparer<Tuple<Leqisoft.Model.Cell, ValidationResult>>
	{
		public bool Equals(Tuple<Leqisoft.Model.Cell, ValidationResult> x, Tuple<Leqisoft.Model.Cell, ValidationResult> y)
		{
		...
		public int GetHashCode(Tuple<Leqisoft.Model.Cell, ValidationResult> obj)
		{
			return 0;
		}
	}
	...
	{
		Text = "采数公式",
		Image = Leqisoft.UI.Controls.Properties.Resources.TableCollect16
	};
	...
	{
		Image = Leqisoft.UI.Platform.Properties.Resources.HelpCenter,
		Text = "帮助中心",
	...
	{
		Image = Leqisoft.UI.Platform.Properties.Resources.back32,
		Text = "后退"
	};
	...
	{
		Image = Leqisoft.UI.Platform.Properties.Resources.forward32,
		Text = "前进"
	};
	...
	private readonly C1CommandMenu mnuRowOwnerShare = new C1CommandMenu
	{
		...
		Image = Leqisoft.UI.Platform.Properties.Resources.RowOwnerShare16
	};
	...
	{
		Text = "插入单元格附件",
		Image = Leqisoft.UI.Platform.Properties.Resources.ctxAttachment
	};
	...
	{
		Text = "删除单元格附件",
		Image = Leqisoft.UI.Platform.Properties.Resources.ctxRemoveAttachment
	};
	...
	public Tuple<Leqisoft.Model.Cell, Leqisoft.Model.Cell> Clipboard { get; set; }

	private Leqisoft.Model.Column CopiedColumn { get; set; }

	private List<Leqisoft.Model.Column> CopiedColumns { get; } = new List<Leqisoft.Model.Column>();


	private Leqisoft.Model.Cell CopiedHeaderCell { get; set; }
	...
	private void InitializeContextMenu()
	{
		...
		AddCommandMenu("数值格式", Leqisoft.UI.Platform.Properties.Resources.Numeric, SetDataFormatNumeric, Tuple.Create("1234.56", DataFormatType.Number), Tuple.Create("1,234.56", DataFormatType.Comma), Tuple.Create("$1,234.56", DataFormatType.NumDollar), Tuple.Create("￥1,234.56", DataFormatType.NumRmb), Tuple.Create("123,456.78%", DataFormatType.Percentage));
		C1CommandMenu menu = AddCommandMenu("日期格式", Leqisoft.UI.Platform.Properties.Resources.Date, SetDataFormatDate, Tuple.Create("2017年12月31日", DataFormatType.DateChinese), Tuple.Create("2017-12-31", DataFormatType.DateDash), Tuple.Create("2017/12/31", DataFormatType.DateSlash), Tuple.Create("2017.12.31", DataFormatType.DateDot));
		...
		AddCommandMenu("判断格式", Leqisoft.UI.Platform.Properties.Resources.Boolean, SetDataFormatBoolean, Tuple.Create("复选框", DataFormatType.BoolCheckBox), Tuple.Create("开关钮", DataFormatType.BoolOnOff));
	...
	private void CmdForbiddenManualInputValue_CommandStateQuery(object sender, CommandStateQueryEventArgs e)
	{
		...
		{
			Leqisoft.Model.Column column = Table.Columns[i];
			Leqisoft.Model.CellStyle style = column.Style;
	...
	private void AddRowRoleCommand(RowRole rr, string caption)
	{
		...
		cmd.Click += delegate
		{
			if (rr == RowRole.Header && Table.Columns.Any(delegate(Leqisoft.Model.Column c)
			{
				if (!c.HasFormula)
				{
					return false;
			...
			{
				Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "表格应用了CrossTable函数，不能设置列头类行。");
			}
			else
			{
	...
	private void CmdRemoveRows_CommandStateQuery(object sender, CommandStateQueryEventArgs e)
	{
		...
		if (!Table.Rows.Where((Leqisoft.Model.Row r) => r.Index >= start && r.Index <= end).All((Leqisoft.Model.Row r) => CanEditRow(r)))
		{
			e.Enabled = false;
		}
		else
	...
	private void CmdCancelSubtotal_CommandStateQuery(object sender, CommandStateQueryEventArgs e)
	{
		...
		e.Visible = Table.Rows.Any((Leqisoft.Model.Row r) => r.Role == RowRole.Subtotal || r.Role == RowRole.Total);
	...
	private void CmdSubtotalTable_Click(object sender, ClickEventArgs e)
	{
		...
		Leqisoft.Model.Column column = Table.Columns[_grid.BodyCol];
		treeTableNode.Name = Table.TreeNode.Name + "分类汇总表（按" + column.CaptionDisplay + "）";
		List<Leqisoft.Model.Column> list = Table.Columns.Where((Leqisoft.Model.Column c) => c.Style?.DataType == typeof(double)).ToList();
		Leqisoft.Model.Table table = treeTableNode.Table;
		...
		Leqisoft.Model.Column column2 = table.Columns[0];
		column2.UpdateFormula($"Distinct([2:{Table.Id}:{column.Id}])");
		column2.UpdateCaption(column.Caption);
		Leqisoft.Model.CellStyle oldStyle = column.Style;
		if (oldStyle != null)
		{
			table.Columns[0].UpdateStyle(table.CellStyles.MutateAndGet(table.DefaultStyle, delegate(Leqisoft.Model.CellStyle cs)
			{
		...
		for (int i = 0; i < list.Count; i++)
		{
			...
			if (oldStyle != null)
			{
				column2.UpdateStyle(table.CellStyles.MutateAndGet(table.DefaultStyle, delegate(Leqisoft.Model.CellStyle cs)
				{
					cs.DataType = oldStyle.DataType;
					cs.Format = oldStyle.Format;
		...
		currentGroup.Grid.Row = currentGroup.Grid.Rows[currentGroup.Grid.Row].Node.AddNode(NodeTypeEnum.NextSibling, treeTableNode.Name, treeTableNode, Leqisoft.UI.Platform.Properties.Resources.TreeTable).Row.Index;
	}
	...
	private void CmdRemoveFormula_Click(object sender, ClickEventArgs e)
	{
		...
		for (int i = bodySelection.TopRow; i <= bodySelection.BottomRow; i++)
		{
			...
			{
				Leqisoft.Model.Cell cell = _table[i, j];
	...
	{
		bool flag = SoftwareLicenseManager.IsAllowModifyTableStruct();
		Leqisoft.Model.Column column = Table.Columns[_grid.BodySelection.LeftCol];
	...
	private void CmdCancelSumColumns_CommandStateQuery(object sender, CommandStateQueryEventArgs e)
	{
		...
		e.Visible = Table.Rows.Any((Leqisoft.Model.Row r) => r.Role == RowRole.Total);
		if ((Table.RowOwnerLoad || Table.RowOwnerExclusive) && !Table.IsManager())
		{
			e.Enabled = false;
		}
	...
	private void CmdShowColumns_CommandStateQuery(object sender, CommandStateQueryEventArgs e)
	{
		...
		if (Table.Columns.Any((Leqisoft.Model.Column c) => !c.Visible))
		{
			e.Visible = true;
		}
		else
	...
	private void CmdFreezeColumn_CommandStateQuery(object sender, CommandStateQueryEventArgs e)
	{
		...
		cmdFreezeColumn.Image = Leqisoft.UI.Platform.Properties.Resources.ctxFreezeCol;
	...
	private void CmdLedgerCollectFormulaEdit_CommandStateQuery(object sender, CommandStateQueryEventArgs e)
	{
		...
		if (Program.ClientPlatformType == PlatformType.AuditPlatform && Leqisoft.Model.User.Current.IsSystemSupporter)
		{
			flag = true;
		}
		if (flag && _grid.Selection.RightCol - _grid.Selection.LeftCol >= 1)
	...
	private void CmdMakerSign_CommandStateQuery(object sender, CommandStateQueryEventArgs e)
	{
		...
		cmdMakerSign.Image = Leqisoft.UI.Platform.Properties.Resources.MakerSign;
	}
	...
	private void CmdCheckerSign_CommandStateQuery(object sender, CommandStateQueryEventArgs e)
	{
		...
		cmdCheckerSign.Image = Leqisoft.UI.Platform.Properties.Resources.CheckerSign;
	}
	...
	private void CmdRowAccessAll_Click(object sender, ClickEventArgs e)
	{
		...
		{
			if (commandLink.Command.Checked && commandLink.Command.UserData is Leqisoft.DTO.User user)
			{
		...
		{
			Leqisoft.Model.Row row2 = Table.Rows[j];
	...
	private void CmdRowAccessAll_CommandStateQuery(object sender, CommandStateQueryEventArgs e)
	{
		...
		mnuRowAccess.Image = Leqisoft.UI.Platform.Properties.Resources.AccessControl16;
	}
	...
	private void CmdPasteHeaderCellFormula_CommandStateQuery(object sender, CommandStateQueryEventArgs e)
	{
		...
		Leqisoft.Model.Cell cell = Table[_grid.BodyRow, _grid.BodyCol];
	...
	private void MnuRowOwnerShare_Popup(object sender, EventArgs e)
	{
		...
		foreach (Leqisoft.DTO.User item in from kv in Table.Project.Users
		...
		Leqisoft.Model.Row row = Table.Rows[_grid.BodyRow];
		...
		Leqisoft.Model.User current2 = Leqisoft.Model.User.Current;
	...
	private void MnuRowAccess_Popup(object sender, EventArgs e)
	{
		...
		foreach (KeyValuePair<Leqisoft.DTO.User, UserRole> item in _owner.CurrentProject.Users.Where((KeyValuePair<Leqisoft.DTO.User, UserRole> u) => u.Value == UserRole.Assistant || u.Value == UserRole.Checker))
		{
			C1Command c1Command = new C1Command
			{
				Text = item.Key.Name,
			...
			c1Command.Click += delegate
			{
				...
				for (int i = _grid.BodyRow; i <= _grid.BodyRowSel; i++)
				{
					Leqisoft.Model.Row row2 = Table.Rows[i];
					row2.Permissions.Write = permission;
					row2.TagPermissionsDirty();
				}
			};
	...
	private void CmdColumnAccessAll_Click(object sender, ClickEventArgs e)
	{
		...
		{
			Leqisoft.Model.Column column2 = Table.Columns[j];
	...
	private void MnuColumnAccess_Popup(object sender, EventArgs e)
	{
		...
		foreach (KeyValuePair<Leqisoft.DTO.User, UserRole> item in _owner.CurrentProject.Users.Where((KeyValuePair<Leqisoft.DTO.User, UserRole> u) => u.Value == UserRole.Assistant || u.Value == UserRole.Checker))
		{
			...
			c1Command.Click += delegate
			{
				...
				for (int i = _grid.BodyCol; i <= _grid.BodyColSel; i++)
				{
					Leqisoft.Model.Column column2 = Table.Columns[i];
					column2.Permissions.Write = permission;
					column2.TagPermissionsDirty();
				}
			};
	...
	private void MnuColumnAccess_CommandStateQuery(object sender, CommandStateQueryEventArgs e)
	{
		...
		mnuColumnAccess.Image = Leqisoft.UI.Platform.Properties.Resources.AccessControl16;
	...
	{
		e.Visible = Table.Cells.Any((Leqisoft.Model.Cell c) => !string.IsNullOrEmpty(c.CollectSource));
	}
	...
	private void CmdRowConvertToColHeader_Click(object sender, ClickEventArgs e)
	{
		...
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "因您没有当前表格的【" + Table.TreeNode.GetDontHavePermissionString() + "】权限，因此，无法将选定行次内容转换为表格列头。");
			return;
		}
		...
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, $"此操作最多选定{10}行。");
			return;
		}
		...
		for (int i = 0; i < Table.Columns.Count; i++)
		{
			...
			{
				Leqisoft.Model.Cell cell = Table[j, i];
				Leqisoft.Model.Cell mergeTopLeftCell = cell.GetMergeTopLeftCell();
			...
			Leqisoft.Model.CellStyle captionStyle = column.CaptionStyle;
			Leqisoft.Model.Cell cell2 = Table[topRow, i];
	...
	private void CmdRowConvertToSubtitle_Click(object sender, ClickEventArgs e)
	{
		if (!HasRowConvertPermission())
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "因您没有当前表格的【" + Table.TreeNode.GetDontHavePermissionString() + "】权限，因此，无法将选定行次内容转换为表格副标题。");
			return;
		}
		int topRow = _grid.BodySelection.TopRow;
		int bottomRow = _grid.BodySelection.BottomRow;
		...
		for (int i = topRow; i <= bottomRow; i++)
		{
			...
			List<Leqisoft.Model.Cell> list = (from c in Table.Rows[i].GetCells()
		...
		for (int i = selection.TopRow; i <= selection.BottomRow; i++)
		{
			...
			for (int j = selection.LeftCol; j <= selection.RightCol; j++)
			{
				...
				if (!_grid.IsIndexOutOfRange(i, j))
				{
					...
					Leqisoft.Model.Cell cell = Table[row, col];
	...
	private async void CmdExportAttachment_Click(object sender, ClickEventArgs e)
	{
		...
		if (list.Count == 0)
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "选中区域内没有任何附件！");
			return;
		}
		try
		{
		...
		{
			ex.Log();
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.Hand, "附件导出失败:\r\n" + ex.ToString(), MessageBoxButtons.OK, "错误!");
		}
	}
	...
	{
		if (Program.MainForm.CurrentProject.Creator.Id == Leqisoft.Model.User.Current.Id)
		{
			return true;
		}
		if (!Program.MainForm.CurrentProject.Users.Any((KeyValuePair<Leqisoft.DTO.User, UserRole> kv) => kv.Key.Id == Leqisoft.Model.User.Current.Id))
		{
			return false;
		}
		if (Program.MainForm.CurrentProject.Users.First((KeyValuePair<Leqisoft.DTO.User, UserRole> kv) => kv.Key.Id == Leqisoft.Model.User.Current.Id).Value == UserRole.Manager)
		{
			return true;
		}
	...
	private void CmdRowConvertToTitle_Click(object sender, ClickEventArgs e)
	{
		if (!HasRowConvertPermission())
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "因您没有当前表格的【" + Table.TreeNode.GetDontHavePermissionString() + "】权限，因此，无法将选定行次内容转换为表格主标题。");
			return;
		}
		int topRow = _grid.BodySelection.TopRow;
		int bottomRow = _grid.BodySelection.BottomRow;
	...
	private void CmdRowConvertToFoot_Click(object sender, ClickEventArgs e)
	{
		if (!HasRowConvertPermission())
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "因您没有当前表格的【" + Table.TreeNode.GetDontHavePermissionString() + "】权限，因此，无法将选定行次内容转换为表底签名。");
			return;
		}
		int topRow = _grid.BodySelection.TopRow;
		int bottomRow = _grid.BodySelection.BottomRow;
	...
	{
		Leqisoft.Model.Column dstCol = Table.Columns[_grid.BodyCol];
	...
	{
		e.Visible = CopiedColumns.Count > 0 && CopiedColumns[0].Table.Project.Id == Leqisoft.Model.Project.Current.Id;
	}
	...
	{
		Leqisoft.Model.Column srcCol = CopiedColumns[0];
		Leqisoft.Model.Column dstCol = Table.Columns[_grid.BodyCol];
	...
	{
		Leqisoft.Model.Column column = CopiedColumns[0];
	...
	private void CopyToTitleCell(Leqisoft.Model.Cell c, TableTitleCell tc)
	{
	...
	public void AppendRows()
	{
		...
		if (num2 > 50000)
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, $"一次新增行数不得超过 {50000}");
		}
		else
		{
	...
	public void CopyFill()
	{
		...
		List<Tuple<Leqisoft.Model.Cell, object>> list = new List<Tuple<Leqisoft.Model.Cell, object>>();
		for (int j = i; j <= _grid.BodyRowSel; j++)
		{
			...
			{
				Leqisoft.Model.Cell cell = Table[j, k];
	...
	public void SequenceFill()
	{
		...
		Leqisoft.Model.Cell cell = Table[i, leftCol];
		if (cell == null) return;
		...
		bool isTableExistFillFormula = IsTableExistFillFormula();
		Leqisoft.Model.Cell cell2 = Table[j, leftCol];
		...
		if (value is double num && value2 is double num2)
		{
			...
			for (int k = j + 1; k <= bodySelection.BottomRow; k++)
			{
				...
				num3 += num4;
				Leqisoft.Model.Cell cell3 = Table[k, leftCol];
		...
		else if (!(value2 is DateTime) && !(value2 is DateYearMonth))
		{
			...
			if (sequenceInfo != null)
			{
				for (int l = j; l <= bodySelection.BottomRow; l++)
				{
					...
					Leqisoft.Model.Cell cell4 = Table[l, leftCol];
		...
		for (int m = num8; m <= bodySelection.BottomRow; m++)
		{
			...
			Leqisoft.Model.Cell cell5 = Table[m, leftCol];
	...
	public static bool CheckIsInputValidInt_BigThanZero(string strExp)
	{
		...
		Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.Hand, "不是有效的数值！", MessageBoxButtons.OK, "错误");
		return false;
	}
	...
	public void RemoveRows()
	{
		...
		int start = _grid.BodyRow;
		int end = _grid.BodyRowSel;
		if (!Table.Rows.Where((Leqisoft.Model.Row r) => r.Index >= start && r.Index <= end).All((Leqisoft.Model.Row r) => CanEditRow(r) && !r.IsLocked && !Table.ControlLockRows.Contains(r) && Table.IsControlFormulaAllowEditRow(r)))
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "要删除的行包含您无权删除的行");
			return;
		}
		IEnumerable<Leqisoft.Model.Cell> source = Table.Cells.Where((Leqisoft.Model.Cell c) => c.Row.Index >= start && c.Row.Index <= end);
		List<FormulaDependency> list = Table.Project.FormulaManager.GetCellsReferrer(Table.Id, source.Select((Leqisoft.Model.Cell c) => c.Id)).ToList();
		if (list.Count > 0)
		{
			FormulaDependency formulaDependency = list[0];
			StringBuilder stringBuilder = new StringBuilder("要删除的行被");
			Leqisoft.Model.Table table = null;
			...
			{
				table = Leqisoft.Model.Project.Current.GetTableById(formulaDependency.HostTable);
				if (table == null)
				{
			...
			{
				Leqisoft.Model.Cell cellById = table.GetCellById(formulaDependency.HostObject);
				if (cellById == null)
				{
			...
			{
				Leqisoft.Model.Column byId = table.Columns.GetById(formulaDependency.HostObject);
				if (byId == null)
				{
			...
			if (Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, stringBuilder.ToString(), MessageBoxButtons.OKCancel) != DialogResult.OK)
			{
				return;
			}
		}
	...
	public void RecoverHistoryTable(Leqisoft.Model.Table historyTable)
	{
	...
	public async void PopulateTable()
	{
		...
		{
			SetCorruptedView();
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "表格 " + _table.TreeNode.Name + " 未同步到本地，请重新同步。\n若同步后仍无数据，则表格还未同步过，请联系该表格的创建人执行同步。");
			return;
		}
		if (_table.IsCorrupted)
		{
			SetCorruptedView();
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "表格 " + _table.TreeNode.Name + " 数据已损坏，请尝试删除此表格重新编辑。");
			return;
		}
		PopulateToolbar();
	...
	private void PopulateRerredTables()
	{
		...
		{
			Leqisoft.Model.Table referredTable = Table.Project.GetTableById(item2);
			if (referredTable != null)
			{
				C1Command c1Command = new C1Command
				{
					Image = Leqisoft.UI.Platform.Properties.Resources.TreeTable,
	...
	public void AppendColumns()
	{
		...
		if (num.Value > 50m)
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, $"一次新增列数不得超过 {50}");
		}
		else
		{
	...
	public void RemoveColumns()
	{
		...
		if (!Table.Columns.Where((Leqisoft.Model.Column c) => c.Index >= start && c.Index <= end).All((Leqisoft.Model.Column c) => CanEditColumn(c) && !c.IsLocked))
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "要删除的列包含您无权删除的列");
			return;
		}
		IEnumerable<Leqisoft.Model.Cell> source = Table.Cells.Where((Leqisoft.Model.Cell c) => c.Column.Index >= start && c.Column.Index <= end);
		List<FormulaDependency> list = Table.Project.FormulaManager.GetCellsReferrer(Table.Id, source.Select((Leqisoft.Model.Cell c) => c.Id)).Concat(Table.Project.FormulaManager.GetColumnsReferrer(Table.Id, from c in Table.Columns
		...
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "由于删除的列中包含有冻结列，冻结列已重置。");
			Table.UpdateFrozenCols(0);
			_grid.Cols.Frozen = 0;
		}
		pnlGrid.SuspendDrawing();
	...
	public void ShowColumnsDialog()
	{
		...
		pnlGrid.SuspendDrawing();
		foreach (Leqisoft.Model.Column item in showColumnsSelector.Selected)
		{
	...
	public void SetDataType<T>()
	{
		...
		C1.Win.C1FlexGrid.CellRange bodySelection = _grid.BodySelection;
		if (Table.Rows.Skip(bodySelection.TopRow).Take(bodySelection.BottomRow - bodySelection.TopRow + 1).All((Leqisoft.Model.Row r) => !Table[r.Index, 0].ShouldApplyColumnFormula()))
		{
			foreach (Leqisoft.Model.Cell item in Table.EnumerateCellRange(bodySelection.TopRow, bodySelection.LeftCol, bodySelection.BottomRow, bodySelection.RightCol))
			{
		...
		{
			foreach (Leqisoft.Model.Cell item2 in Table.EnumerateCellRange(bodySelection.TopRow, bodySelection.LeftCol, bodySelection.BottomRow, bodySelection.RightCol))
			{
		...
		MutateCellStyle((Leqisoft.Model.CellStyle cs) => cs.DataType, typeof(T), updateTableDefaultIfSelectAll: false);
	}
	...
	public void SetDataFormatText()
	{
		if (_grid.BodyRow >= 0 && _grid.BodyCol >= 0 && Table.Rows.Count != 0 && !IsTableLocked)
		{
			...
			SetDataType<string>();
			MutateCellStyle((Leqisoft.Model.CellStyle cs) => cs.Format, value);
			ForceRefresh();
		}
	}
	...
	public void ClearColumnAllowManualInputFlag(int colIndex, bool isReCalcTable)
	{
		...
		bool flag = false;
		Leqisoft.Model.Column column = Table.Columns[colIndex];
		if (CanEditColumn(column))
		{
			Leqisoft.Model.CellStyle style = column.Style;
			if (style != null && (style.Format?.IsAllowEditOnExistFormula).GetValueOrDefault())
			{
				column.UpdateStyle(Table.CellStyles.MutateAndGet(column.Style, delegate(Leqisoft.Model.CellStyle cs)
				{
		...
		{
			Leqisoft.Model.Cell cell = Table[i, column.Index];
	...
	{
		Lazy<List<Leqisoft.Model.Column>> lazy = new Lazy<List<Leqisoft.Model.Column>>();
		...
		bool flag = false;
		foreach (Leqisoft.Model.Column item in lazy.Value)
		{
			for (int j = range.TopRow; j <= range.BottomRow; j++)
			{
				Leqisoft.Model.Cell cell = Table[j, item.Index];
				if (cell.IsExistManualInputValue)
				{
					cell.IsExistManualInputValue = false;
					flag = true;
	...
	public void SetDataFormatNumeric(DataFormatType dft)
	{
		if (_grid.BodyRow >= 0 && _grid.BodyCol >= 0 && Table.Rows.Count != 0 && !IsTableLocked)
		{
			...
			MutateCellStyle((Leqisoft.Model.CellStyle cs) => cs.Format, value);
			MutateCellStyle((Leqisoft.Model.CellStyle cs) => cs.Align, CellTextAlign.MiddleRight);
			ForceRefresh();
	...
	public void SetDataFormatBoolean(DataFormatType dft)
	{
		...
		{
			SetDataType<bool>();
			MutateCellStyle((Leqisoft.Model.CellStyle cs) => cs.Format, new DataFormat(dft));
			MutateCellStyle((Leqisoft.Model.CellStyle cs) => cs.Align, CellTextAlign.MiddleCenter);
			ForceRefresh();
		}
	}
	...
	public void SetForeColor(Color color)
	{
		...
		{
			MutateCellStyle((Leqisoft.Model.CellStyle cs) => cs.ForeColor, color);
		}
	}
	...
	public void SetBackColor(Color color)
	{
		...
		{
			MutateCellStyle((Leqisoft.Model.CellStyle cs) => cs.BackColor, color);
		}
	}
	...
	public void SetAlign(CellTextAlign align)
	{
		...
		{
			MutateCellStyle((Leqisoft.Model.CellStyle cs) => cs.Align, align);
		}
	}
	...
	public void SetFontSize(float fontSize)
	{
		...
		{
			MutateCellStyle((Leqisoft.Model.CellStyle cs) => cs.FontSize, fontSize);
			AppCommands.TableFontSize.FontSizeSelector.SelectFontSize(fontSize);
		}
	}
	...
	public void GrowFont()
	{
		...
		else
		{
			...
			if (flag)
			{
				...
				MutateCellStyle((Leqisoft.Model.CellStyle cs) => cs.FontSize, displayFontSize);
	...
	public void SetFontFamily(string ff)
	{
		...
		{
			MutateCellStyle((Leqisoft.Model.CellStyle cs) => cs.FontFamily, ff);
		}
	}
	...
	public void MorePrecision()
	{
		...
		if (flag)
		{
			...
			MutateCellStyle((Leqisoft.Model.CellStyle cs) => cs.Format, value, updateTableDefaultIfSelectAll: false);
			ForceRefresh();
		}
	}
	...
	public void SetEditCommentDialog()
	{
		...
		else
		{
			...
			Leqisoft.Model.Cell cell = Table[num, _grid.BodyCol];
			...
			{
				MutateCellStyle((Leqisoft.Model.CellStyle cs) => cs.Comment, AuxEditor.View.CommentValue, updateTableDefaultIfSelectAll: false, clearCellStyleIfEntireColumn: false);
			}
		}
	}
	...
	private void AuxEditor_Closed(object sender, EventArgs e)
	{
		...
		try
		{
			...
			if (AuxEditor.View.QueryValueChanged)
			{
				...
				MutateCellStyle((Leqisoft.Model.CellStyle cs) => cs.Format, value2, updateTableDefaultIfSelectAll: false);
			}
			ForceRefresh();
			Invalidate();
		}
	...
	private void LedgerCollectFormulaTryRunHandle(string formula)
	{
		...
		Leqisoft.Model.Operand operand = formulaEvaluator.EvaluateOnLedgerVirtualTable(tec, BalanceVirtualTableBuilder.BalanceVirtualTableId, VoucherVirtualTableBuilder.VoucherVirtualTableId);
	...
	private void LedgerCollectFormulaEditor_Closed(object sender, EventArgs e)
	{
		...
		try
		{
			...
			{
				Dictionary<Leqisoft.Model.Column, string> changedFormula = LedgerCollectFormulaEditor.View.ChangedFormula;
				try
				{
					foreach (Leqisoft.Model.Column key in changedFormula.Keys)
					{
						...
						MutateTableColumnCellStyle((Leqisoft.Model.CellStyle cs) => cs.Format, value, key);
					}
				}
				catch (Exception exception)
				{
	...
	public void SetBold(bool v)
	{
		...
		{
			MutateCellStyle((Leqisoft.Model.CellStyle cs) => cs.Bold, v);
		}
	}
	...
	public void SetItalic(bool v)
	{
		...
		{
			MutateCellStyle((Leqisoft.Model.CellStyle cs) => cs.Italic, v);
		}
	}
	...
	public void MergeEveryHorizontalCells()
	{
		...
		C1.Win.C1FlexGrid.CellRange bodySelection = _grid.BodySelection;
		if (!_table.EnumerateCellRange(bodySelection.TopRow, bodySelection.LeftCol, bodySelection.BottomRow, bodySelection.RightCol).All((Leqisoft.Model.Cell c) => CanEditRow(c.Row) && CanEditColumn(c.Column)))
		{
			return;
		}
		for (int i = bodySelection.TopRow; i <= bodySelection.BottomRow; i++)
	...
	public void UnmergeCells()
	{
		...
		C1.Win.C1FlexGrid.CellRange bodySelection = _grid.BodySelection;
		foreach (Leqisoft.Model.Cell item in _table.EnumerateCellRange(bodySelection.TopRow, bodySelection.LeftCol, bodySelection.BottomRow, bodySelection.RightCol))
		{
			try
			{
	...
	public async Task TableStyleBrush()
	{
		...
		IEnumerable<Leqisoft.Model.Table> tables = from n in frmNodeSelector2.Selected.OfType<TreeTableNode>()
			select n.Table;
		...
		progressForm.ShowDialog(progressRuntimeData, delegate
		{
			...
			foreach (Leqisoft.Model.Table item in tables)
			{
				num++;
	...
	public void BatchColumnDuplicate()
	{
		if (!_grid.IsEntireColumnSelected)
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "请先选择要复制的列");
			return;
		}
		frmNodeSelector frmNodeSelector2 = new frmNodeSelector();
		frmNodeSelector2.Project = Leqisoft.Model.Project.Current;
		...
		foreach (Leqisoft.Model.Table item in from n in frmNodeSelector2.Selected.OfType<TreeTableNode>().Except(new TreeTableNode[1] { Table.TreeNode })
			...
			select t)
		{
			...
			if (!flag)
			{
				Leqisoft.Model.Column byCaption = item.Columns.GetByCaption(caption);
				if (byCaption == null)
				{
					flag = true;
				}
		...
		static void CopyColumn(Leqisoft.Model.Column src, Leqisoft.Model.Column dst)
		{
			dst.Caption = src.Caption;
			dst.Width = src.Width;
			if (src.Style != null)
			{
				Leqisoft.Model.CellStyle style = dst.Table.CellStyles.MutateAndGet(dst.Style, delegate(Leqisoft.Model.CellStyle cs)
				{
	...
	public void BatchColumnRemove()
	{
		if (!_grid.IsEntireColumnSelected)
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "请先选择要删除的列");
			return;
		}
		frmNodeSelector frmNodeSelector2 = new frmNodeSelector();
		frmNodeSelector2.Project = Leqisoft.Model.Project.Current;
		...
		foreach (Leqisoft.Model.Table item in from n in frmNodeSelector2.Selected.OfType<TreeTableNode>()
			select n.Table.LoadAndReturn() into t
			where t.HasSchemaPermission()
			select t)
		{
	...
	public void BatchColumnRename()
	{
		if (!_grid.IsEntireColumnSelected)
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "请先选择要重命名的列");
			return;
		}
	...
	public void GenerateBatchFormula()
	{
		if (!_grid.IsEntireColumnSelected)
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "请先选择列");
			return;
		}
		if (Table == null)
		{
		...
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "当前表格没有查看权限，无法进行编辑");
			return;
		}
		if (Table.TreeNode.HasWritePermission())
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "当前表格没有编辑权限，无法进行编辑");
			return;
		}
		int leftCol = _grid.BodySelection.LeftCol;
		Leqisoft.Model.Column column = Table.Columns[leftCol];
		if (!column.HasFormula)
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "列公式为空");
			return;
		}
		FormulaEvaluator formulaEvaluator = new FormulaEvaluator(column.Formula);
		if (!formulaEvaluator.CanPatternMatchBatch())
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "智能扩充跨表公式功能仅支持含有Distinct、Distinct、DistinctUp、DistinctDown、 Vlookup、Sumif函数的列公式的智能扩充，当前选中列的列公式中未包含这些函数，因此不支持使用该功能。");
			return;
		}
		FormulaReferenceModelResolver resolver = new FormulaReferenceModelResolver(Table.Project);
		...
		List<Leqisoft.Model.Table> list = (from n in frmNodeSelector2.Selected.OfType<TreeTableNode>()
			select n.Table.LoadAndReturn() into n
			where n.TreeNode.HasReadPermission()
			select n).ToList();
		if (list.Count == 0)
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "选中的表格没有查看权限");
			return;
		}
		...
		if (text == null)
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "智能填充跨表公式操作失败。");
			return;
		}
	...
	public void Cut()
	{
		...
		for (int i = bodySelection.TopRow; i <= bodySelection.BottomRow; i++)
		{
			...
			for (int j = bodySelection.LeftCol; j <= bodySelection.RightCol; j++)
			{
				...
				Leqisoft.Model.Cell cell = Table[i, j];
				if (!CanEditCell(cell, isTableExistFillFormula))
				{
					continue;
				}
	...
	public void Copy()
	{
		...
		try
		{
			dataObject.SetData("LeqiClipboardCell", new ClipboardCell
			{
	...
	public async Task PasteValue()
	{
		...
		List<Tuple<Leqisoft.Model.Cell, object>> newValues = new List<Tuple<Leqisoft.Model.Cell, object>>();
		try
		{
			if (_isInPasting)
			{
			...
			await progressForm.ShowDialog(progressRuntimeData, async delegate
			{
				...
				{
					Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "复制粘贴的数据存在异常，请尝试重新复制粘贴。\r\n异常信息描述: " + ex3.Message);
				...
				if (ret != null && ret.Count > 0)
				{
					...
					if (num > 0 || num2 > 0)
					{
						...
						if (!HasSchemaPermission())
						{
							Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "您没有表格的结构调整权限，无法通过粘贴生成新的列");
							return;
						}
					}
					if (num2 > 0)
								...
								else
								{
									...
									if (CanEditCell(cell, isTableExistFillFormula) && !Table.MergedCells.Any((CellMerge m) => m.ContainsAndNotTopLeft(cell)) && (!cell.HasCellFormulaOrColumnFormula || isOnlySelectFormulaArea))
									{
										try
										{
											...
											double result;
											object item = ((obj is string text && text.EndsWith("%") && double.TryParse(text.TrimEnd('%'), out result) && cell.DisplayDataType == typeof(double)) ? ((object)(result / 100.0)) : ((!(cell.DisplayDataType == typeof(bool))) ? Leqisoft.Model.Cell.ChangeDataTypeImpl(obj, cell.DisplayDataType) : ((object)"√".Equals(obj))));
	...
	public void PasteFormat()
	{
		...
		Leqisoft.Model.Table table = Clipboard.Item1.Row.Table;
		...
		for (int i = 0; i < num3; i++)
		{
			...
			{
				Leqisoft.Model.Cell srcCell = table[index + i % num, index2 + j % num2];
				Leqisoft.Model.Cell cell = Table[bodyRow + i, bodyCol + j];
				if (!CanEditColumn(cell.Column) || !CanEditRow(cell.Row))
				{
					continue;
				}
				...
				Leqisoft.Model.CellStyle style = ((srcCell.Style == null) ? null : Table.CellStyles.MutateAndGet(null, delegate(Leqisoft.Model.CellStyle cs)
				{
	...
	public void PasteFormula()
	{
		...
		try
		{
			...
			Leqisoft.Model.Cell item = Clipboard.Item1;
	...
	public void PasteHeaderCellFormula()
	{
		...
		{
			Leqisoft.Model.Cell cell = Table[_grid.BodyRow, i];
			FormulaReferenceModelResolver resolver = new FormulaReferenceModelResolver(Leqisoft.Model.Project.Current);
			try
			{
	...
	{
		CopiedColumns.Clear();
		CopiedColumns.AddRange(Table.Columns.Where((Leqisoft.Model.Column c) => c.Index >= _grid.BodySelection.LeftCol && c.Index <= _grid.BodySelection.RightCol));
	}
	...
	public void SubtotalImpl()
	{
		...
		SortAscending();
		Leqisoft.Model.CellStyle style = Table.CellStyles.MutateAndGet(Table.DefaultStyle, delegate(Leqisoft.Model.CellStyle cs)
		{
			cs.Align = CellTextAlign.MiddleCenter;
		});
		List<Leqisoft.Model.Column> list = Table.Columns.Where((Leqisoft.Model.Column c) => c.SubtotalAttributes == ColumnSubtotal.GroupBy).ToList();
		List<int> groupByIndexes = list.Select((Leqisoft.Model.Column gb) => gb.Index).ToList();
		List<IGrouping<List<Leqisoft.Model.Cell>, Leqisoft.Model.Row>> list2 = Table.Rows.GroupBy((Leqisoft.Model.Row row) => groupByIndexes.Select((int i) => Table[row.Index, i]).ToList(), CellListByValueEqualsComparer.Instance).ToList();
		IEnumerable<int> enumerable = from c in Table.Columns
			...
			select c.Index;
		Leqisoft.Model.Cell cell;
		foreach (IGrouping<List<Leqisoft.Model.Cell>, Leqisoft.Model.Row> item in list2)
		{
			Leqisoft.Model.Row row2 = item.Last();
			...
			cell.UpdateFormula("\"" + string.Join(" ", item.Key.Select((Leqisoft.Model.Cell c) => c.Value?.ToString() ?? string.Empty)) + " 小计\"");
			cell.UpdateStyle(style);
		}
		Leqisoft.Model.Row row3 = Table.Rows.LastOrDefault((Leqisoft.Model.Row r) => r.Role == RowRole.Total);
		if (row3 == null)
		{
			Table.Rows.Append(1);
		...
		foreach (IGrouping<List<Leqisoft.Model.Cell>, Leqisoft.Model.Row> item3 in list2)
		{
			Leqisoft.Model.Row firstRow = item3.First();
			Leqisoft.Model.Row row4 = item3.Last();
			int row5 = row4.Index + 1;
			foreach (int item4 in enumerable)
			{
				string arg = string.Join("AND", list.Select(delegate(Leqisoft.Model.Column gb)
				{
					string text3 = null;
	...
	public void SumColumns()
	{
		...
		try
		{
			pnlGrid.SuspendDrawing();
			_grid.BeginUpdate();
			Leqisoft.Model.Column column = Table.Columns[0];
			if (column.Style?.DataType == typeof(double))
			{
				column = Table.Columns.FirstOrDefault((Leqisoft.Model.Column c) => c.Visible && (c.Style == null || c.Style.DataType == null || c.Style.DataType == typeof(string))) ?? column;
			}
	...
	public void SumColumnsHeaderCell()
	{
		...
		Leqisoft.Model.Row row = Table.Rows.Skip(bodyRow + 1).Take(headerLastRow - bodyRow).LastOrDefault((Leqisoft.Model.Row r) => r.Role == RowRole.Total);
		if (row == null)
		{
		...
		int bodyCol = _grid.BodyCol;
		int bodyColSel = _grid.BodyColSel;
		Leqisoft.Model.Cell cell = Table[row.Index, 0];
		cell.UpdateFormula("\"合计\"");
		cell.UpdateStyle(Table.CellStyles.MutateAndGet(Table.DefaultStyle, delegate(Leqisoft.Model.CellStyle cs)
		{
			cs.Align = CellTextAlign.MiddleCenter;
		}));
		row.UpdateRole(RowRole.Total);
		Leqisoft.Model.Row row2 = Table.Rows.Skip(bodyRow + 1).Take(headerLastRow - bodyRow).LastOrDefault((Leqisoft.Model.Row r) => r.Role == RowRole.Normal || r.Role == RowRole.Among || r.Role == RowRole.Minus);
		if (row2 != null)
		{
			for (int i = 0; i < Table.Columns.Count; i++)
			{
	...
	public void CancelSubtotal()
	{
		...
		_grid.BeginUpdate();
		foreach (Leqisoft.Model.Row item in Table.Rows.ToList())
		{
	...
	public void Indent()
	{
		if (!IsTableLocked && _grid.BodyRow >= 0 && _grid.BodyCol >= 0)
		{
			...
			MutateCellStyle((Leqisoft.Model.CellStyle cs) => cs.Margin, displayMargin + 10);
		}
	}
	...
	{
		_grid.BeginUpdate();
		foreach (Leqisoft.Model.Row item in Table.Rows.Where((Leqisoft.Model.Row r) => r.Role == RowRole.Subtotal || r.Role == RowRole.Total))
		{
			foreach (Leqisoft.Model.Cell cell in item.GetCells())
			{
	...
	public void CalcCurrentTable()
	{
		...
		_isUpdatingView = true;
		List<Leqisoft.Model.Row> list = Table.TryApplyFormula(evalLqDistinct: true);
		PopulateRows();
		if (list.Count <= UserConfig.RowsApplyFormulaAuto)
		{
			Application.DoEvents();
			foreach (Leqisoft.Model.Row item in list)
			{
				if (item.IsExisting)
				{
	...
	public void LockRows()
	{
		...
		for (int i = _grid.BodyRow; i <= _grid.BodyRowSel; i++)
		{
			...
			{
				Table.Rows[i].UpdateLocker(Leqisoft.Model.User.Current.Id);
			}
			else
			{
	...
	public void LockColumns()
	{
		...
		for (int i = bodyCol; i <= bodyColSel; i++)
		{
			Table.Columns[i].UpdateStyle(Table.CellStyles.MutateAndGet(Table.Columns[i].Style, delegate(Leqisoft.Model.CellStyle s)
			{
				s.Locker = ((Program.MainForm.CurrentProject.Kind == ProjectType.Project) ? Leqisoft.Model.User.Current.Id : (-1));
			}));
		}
		foreach (Leqisoft.Model.Cell item in Table.EnumerateCellRange(0, bodyCol, Table.Rows.Count - 1, bodyColSel))
		{
			item.UpdateStyle(Table.CellStyles.MutateAndGet(item.Style, delegate(Leqisoft.Model.CellStyle s)
			{
				s.Locker = null;
			}));
		}
	...
	public void UnlockCells()
	{
		...
		foreach (Leqisoft.Model.Cell item in Table.EnumerateCellRange(bodySelection.TopRow, bodySelection.LeftCol, bodySelection.BottomRow, bodySelection.RightCol))
		{
			item.UpdateStyle(Table.CellStyles.MutateAndGet(item.Style, delegate(Leqisoft.Model.CellStyle cs)
			{
				cs.Locker = null;
			}));
		}
	...
	public void MakerSign()
	{
		...
		{
			if (Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "当前表未留有默认的签名位置，是否要在当前选定单元格中签名？", MessageBoxButtons.OKCancel) == DialogResult.OK && _grid.BodyRow >= 0 && _grid.BodyCol >= 0)
			{
				Leqisoft.Model.Cell cell = Table[_grid.BodyRow, _grid.BodyCol];
				if (CanEditCell(cell, IsTableExistFillFormula()))
				{
	...
	public void LockTable()
	{
		if (Program.MainForm.CurrentProject.Kind == ProjectType.Project)
		{
			Table.UpdateLocker(Leqisoft.Model.User.Current.Id);
		}
		else
		{
	...
	private bool CanSetRowOwnerExclusive()
	{
		...
		if (Leqisoft.Model.Project.Current.Users.First((KeyValuePair<Leqisoft.DTO.User, UserRole> kv) => kv.Key.Id == Leqisoft.Model.User.Current.Id).Value == UserRole.Manager)
		{
			return true;
		}
		return false;
	...
	private void TraceTableCell(Leqisoft.Model.Table table, int row, int col)
	{
		try
		{
	...
	protected async Task ExportTableAttachments(Leqisoft.Model.Table table, string finalDirName)
	{
		if (table.CellPropManager.DicCellAttachments.Count == 0)
		{
			return;
	...
	protected async Task ExportAttachmentsToFolder(List<CellAttachment> exportAttachmentsList, string finalDirName)
	{
		...
		else
		{
			...
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "以下附件导出失败: \r\n" + text);
		}
		await Task.Delay(1);
	}
	...
	public void PasteDistinct(Leqisoft.Model.Column srcCol, Leqisoft.Model.Column dstCol)
	{
		try
		{
	...
	public void PasteFilter(Leqisoft.Model.Column srcCol, Leqisoft.Model.Column dstCol)
	{
		...
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "粘贴为DistinctF公式过程中出现错误。");
		}
	}

	private string FilterToFormula(Leqisoft.Model.Table srcTable, FilterBase f)
	{
		Leqisoft.Model.Column byId = srcTable.Columns.GetById(Id64.Parse(f.ColumnId));
	...
	public void PasteAdvanced(string formTitle, string functionName, Leqisoft.Model.Column srcCol, Leqisoft.Model.Column dstCol)
	{
		try
		{
		...
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "目标列的当前列公式有错误，请修正目标列的列公式后再执行粘贴操作。");
			return;
		}
		...
		if (formAdvancedPaste.ShowDialog() == DialogResult.OK)
		{
			...
			{
				Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "列公式有错误，请修正列公式后再执行粘贴操作。");
				return;
			}
			PopulateTable();
		}
	...
	public void PasteSimpleList(Leqisoft.Model.Column srcCol, Leqisoft.Model.Column dstCol)
	{
		Leqisoft.Model.CellStyle style = dstCol.Table.CellStyles.MutateAndGet(dstCol.Style, delegate(Leqisoft.Model.CellStyle cs)
		{
			DataFormat value = cs.Format?.Clone() ?? default(DataFormat);
	...
	public void PasteTreeList(List<Leqisoft.Model.Column> srcCols, Leqisoft.Model.Column dstCol)
	{
		Leqisoft.Model.CellStyle style = dstCol.Table.CellStyles.MutateAndGet(dstCol.Style, delegate(Leqisoft.Model.CellStyle cs)
		{
			string arg = string.Join(",", srcCols.Select((Leqisoft.Model.Column c) => $"[2:{c.Table.Id}:{c.Id}]"));
			DataFormat value = cs.Format?.Clone() ?? default(DataFormat);
	...
	public void PasteTableList(List<Leqisoft.Model.Column> srcCols, Leqisoft.Model.Column dstCol)
	{
		Leqisoft.Model.CellStyle style = dstCol.Table.CellStyles.MutateAndGet(dstCol.Style, delegate(Leqisoft.Model.CellStyle cs)
		{
			string arg = string.Join(",", srcCols.Select((Leqisoft.Model.Column c) => $"[2:{c.Table.Id}:{c.Id}]"));
	...
	public void ControlFormulaCloseRestoreState()
	{
		...
		_grid.Styles.Highlight.ForeColor = Theme.SelectedLeqiTheme.GetC1Theme().GetColor("C1FlexGrid\\Styles\\Highlight\\ForeColor");
		_grid.Styles.Highlight.BackColor = Theme.SelectedLeqiTheme.GetBackgroundSolidColor("C1FlexGrid\\Styles\\Highlight\\Background");
		FormulaContext formulaContext = FormulaEditor.Context.Clone();
		FormulaEvaluator.ClearCache();
		_owner.ProjectHierarchy.FindAndSelectNode(formulaContext.Table.TreeNode);
		Table = formulaContext.Table;
	...
	static TableEditor()
	{
		CursorCross = new Cursor(new MemoryStream(Leqisoft.UI.Platform.Properties.Resources.Fill));
		CursorTable = new Cursor(new MemoryStream(Leqisoft.UI.Platform.Properties.Resources.table));
		CursorRowHeader = new Cursor(new MemoryStream(Leqisoft.UI.Platform.Properties.Resources.RowHeader));
		CursorColumnHeader = new Cursor(new MemoryStream(Leqisoft.UI.Platform.Properties.Resources.ColumnHeader));
		_curFormatBrush = new Cursor(new MemoryStream(Leqisoft.UI.Platform.Properties.Resources.cursortable));
	...
	public void SetTheme()
	{
		...
		if (selectedLeqiTheme != null)
		{
			...
			Color color = Theme.SelectedLeqiTheme.GetC1Theme().GetColor("C1FlexGrid\\Styles\\Normal\\Border\\Color");
			_penThick.Color = color;
			_penThin.Color = color;
			_navTreeTitleBackgroundColor = Theme.SelectedLeqiTheme.GetBackgroundSolidColor("C1Command\\C1OutBar\\Page\\Title\\Hot\\Background");
		...
		else
		{
			...
			case SelectionType.Table:
			{
				if (IsCellRangeHeaderCellEntireColumn(_grid.BodySelection))
				{
					...
					Leqisoft.Model.Cell cell2 = Table[_grid.BodyRow, _grid.BodyCol];
				...
				Leqisoft.Model.Column column = null;
	...
	private async Task KeyDown_Normal(KeyEventArgs e)
	{
		...
		case Keys.Delete:
		{
			...
			if (sel.IsSingleCell)
			{
				Leqisoft.Model.Cell cell = Table[sel.TopRow, sel.LeftCol];
				if (CanEditCell(cell, IsTableExistFillFormula()))
				{
			...
			_grid.BeginUpdate();
			List<Tuple<Leqisoft.Model.Cell, object>> list5 = new List<Tuple<Leqisoft.Model.Cell, object>>();
			bool isTableExistFillFormula = IsTableExistFillFormula();
			foreach (Leqisoft.Model.Cell item in Table.EnumerateCellRange(sel.TopRow, sel.LeftCol, sel.BottomRow, sel.RightCol))
			{
				if (flag2)
				{
					Leqisoft.Model.Row row = item.Row;
		...
		case Keys.Down | Keys.Alt:
		{
			...
			Leqisoft.Model.Operand list3 = GetList(cell.DisplayFormat.ComboList);
			if (!(list3 is ValueSetOperand valueSetOperand2))
			{
				break;
			}
			e.Handled = true;
			List<string> list4 = valueSetOperand2.Set.Select((Tuple<Leqisoft.Model.Row, ValueOperand> tup) => tup.Item2.ToString()).ToList();
		...
		case Keys.Up | Keys.Alt:
		{
			...
			Leqisoft.Model.Operand list = GetList(cell.DisplayFormat.ComboList);
			if (list is ValueSetOperand valueSetOperand)
			{
				e.Handled = true;
				List<string> list2 = valueSetOperand.Set.Select((Tuple<Leqisoft.Model.Row, ValueOperand> tup) => tup.Item2.ToString()).ToList();
	...
	private void KeyDown_Space()
	{
		...
		for (int i = _grid.BodyRow; i <= _grid.BodyRowSel; i++)
		{
			...
			for (int j = _grid.BodyCol; j <= _grid.BodyColSel; j++)
			{
				Leqisoft.Model.Cell cell2 = Table[i, j];
	...
	private void _grid_BodyAfterEdit(object sender, RowColEventArgs e)
	{
		...
		Leqisoft.Model.Cell cell = Table[e.Row, e.Col];
		if (cell == null) return;
		CellUpdateValueCommand cellUpdateValueCommand = new CellUpdateValueCommand(cell, obj);
		if (cell.HasCellFormulaOrColumnFormula)
		{
			bool isExistManualInputValue = true;
			if (!cell.IsExistManualInputValue && cell.GetDisplayValue() == Leqisoft.Model.Cell.GetDisplayValueImpl(obj, cell.DisplayFormat))
			{
	...
	private async Task MouseDoubleClick_Normal(MouseEventArgs e)
	{
		...
		try
		{
			...
			{
				Leqisoft.Model.CellAttachments attachments;
	...
	private void _grid_MouseUp(object sender, MouseEventArgs e)
	{
		if (_isRowResizingStartedDragging)
		{
			...
			if (_grid.Selection.TopRow <= _resizingRow && _resizingRow <= _grid.Selection.BottomRow)
			{
				...
				{
					ex.Log();
					Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.Hand, ex.ToString());
				}
			}
			else
			{
		...
		if (_isColumnResizingStartedDragging)
		{
			...
			if (_grid.Selection.LeftCol <= _resizingColumn && _resizingColumn <= _grid.Selection.RightCol)
			{
				...
				{
					ex2.Log();
					Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.Hand, ex2.ToString());
				}
			}
			else
			{
		...
		if (!_hasDoubleClicked_EditingFormula)
		{
			...
			try
			{
				...
				case HitTestTypeEnum.Cell:
				{
					...
					Leqisoft.Model.Column column2 = Table.Columns[_grid.BodyColSel];
					Leqisoft.Model.Row row2 = Table.Rows[_grid.BodyRowSel];
	...
	private void _grid_BeforeEdit(object sender, RowColEventArgs e)
	{
		...
		{
			Leqisoft.Model.Column column = Table.Columns[e.Col - _grid.Cols.Fixed];
	...
	private void AfterSelChange_ColHeaderFormula()
	{
		if (_isEditingHeaders)
		{
			...
			try
			{
				Leqisoft.Model.Column column = Table.Columns[_grid.Col - _grid.Cols.Fixed];
	...
	private async Task BodySelectionChanged_SignalR()
	{
		...
		try
		{
			Leqisoft.Model.Cell cell = Table[bodyRow, bodyCol];
			await SignalRClient.UpLoadTableCellId(Leqisoft.Model.User.Current.Id.ToString(), cell?.Id.ToString() ?? string.Empty);
		}
		catch
		{
		}
				...
				if (!(e is Leqisoft.Model.Cell cell))
				{
					if (!(e is TreeNodeBase node))
					{
						...
						if (consolidateAttributes == null)
						{
							...
							if (e is Tuple<Leqisoft.Model.Table, int, int> traceTarget)
							{
							...
							{
								int index2 = (int)e;
								Leqisoft.Model.Cell cell3 = Table[_grid.BodyRow, _grid.BodyCol];
							...
							{
								int i2 = (int)e;
								Leqisoft.Model.Cell cell4 = Table[_grid.BodyRow, _grid.BodyCol];
							...
							{
								int num = (int)e;
								Leqisoft.Model.Cell cell5 = Table[_grid.BodyRow, _grid.BodyCol];
						...
						Leqisoft.Model.Project project = await _owner.OpenOrSwitchToProject(consolidateAttributes.ProjectId);
						if (project != null)
						{
							TreeTableNode treeTableNode = project.GetAllTableNodes().FirstOrDefault((TreeTableNode t) => t.Id == consolidateAttributes.TableId);
							if (treeTableNode == null)
							{
								Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "数据源表格不存在");
							}
							else
							{
	...
	public static async Task OpenAttachment(CellAttachment ca)
	{
		...
		await Leqisoft.Model.Project.Current.FileCacheManager.DownloadIfNotExist(fileId);
		try
		{
			string fileName = Leqisoft.Model.Project.Current.FileCacheManager.DuplicateToTemp(fileId, ca.Name);
		...
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "打开附件失败。");
		}
	}
	...
	public void CellCollect(Ledger ledger)
	{
		if (_grid.BodyRow < 0 || _grid.BodyCol < 0)
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "请先选择采集单元格");
			return;
		}
		...
		if (Table == null)
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "请先选择采集表格");
			return;
		}
		if (!Table.TreeNode.HasReadPermission())
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "当前表格没有查看权限，不能执行采账处理");
			return;
		}
		if (!Table.TreeNode.HasWritePermission())
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "当前表格没有编辑权限，不能执行采账处理");
			return;
		}
		Leqisoft.Model.Cell cell = Table[_grid.BodyRow, _grid.BodyCol];
		...
		{
			cell.UpdateCollectSource(frmCellCollect.Formula);
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "审计年度设置不合理，未能采集数据，请重新设置审计年度");
		}
	...
	{
		Rectangle result = new Rectangle(cellRect.X + 2, cellRect.Y + 2, Leqisoft.UI.Platform.Properties.Resources.CancelManualInput.Width, Leqisoft.UI.Platform.Properties.Resources.CancelManualInput.Height);
		isIconOutOfRange = false;
		if (result.X - 2 + Leqisoft.UI.Platform.Properties.Resources.CancelManualInput.Width + 4 >= cellRect.Right || result.Y - 2 + Leqisoft.UI.Platform.Properties.Resources.CancelManualInput.Height + 4 >= cellRect.Bottom)
		{
	...
	private void _grid_MouseMove(object sender, MouseEventArgs e)
	{
		try
		{
			...
			case HitTestTypeEnum.Cell:
			{
				Leqisoft.Model.Cell cell = Table[hitTestInfo.Row - _grid.Rows.Fixed, hitTestInfo.Column - _grid.Cols.Fixed];
				if (cell == null) return;
			...
			case HitTestTypeEnum.ColumnHeader:
			{
				...
				Leqisoft.Model.Column column = Table.Columns[hitTestInfo.Column - _grid.Cols.Fixed];
	...
	private Pen GetPen(Leqisoft.Model.LineStyle ls)
	{
		return ls switch
		{
			Leqisoft.Model.LineStyle.Thick => _penThick, 
			Leqisoft.Model.LineStyle.Thin => _penThin, 
			Leqisoft.Model.LineStyle.None => Pens.Transparent, 
	...
	private void OwnerDrawCell_Shield(OwnerDrawCellEventArgs e)
	{
		...
		if (e.Col == 0)
		{
			...
			{
				Leqisoft.Model.Row row = Table.Rows[num];
				if (!CanEditRow(row) || row.IsLocked)
				{
					e.Image = Leqisoft.UI.Platform.Properties.Resources.TableLock;
		...
		{
			Leqisoft.Model.Column column = Table.Columns[num2];
			if (!CanEditColumn(column) || column.IsLocked)
			{
				e.Image = Leqisoft.UI.Platform.Properties.Resources.TableLock;
				e.Style.Display = DisplayEnum.Overlay;
	...
	private void OwnerDrawCell_ColHeaderFormula(OwnerDrawCellEventArgs e)
	{
		if (FormulaEditor.IsEditing && FormulaEditor.Context.Kind == FormulaContextKind.ColHeader && FormulaEditor.Context.Table == Table && e.Row < _grid.Rows.Fixed && e.Col - _grid.Cols.Fixed == FormulaEditor.Context.Column.Index)
		{
			...
			_penFormulaCell.Color = Theme.SelectedLeqiTheme.ThemeContext.DarkColor;
	...
	private void OwnerDrawCell_Border(OwnerDrawCellEventArgs e)
	{
		if (Table != null && e.Col == _grid.Cols.Count - 1 && e.Row >= 0 && e.Row < _grid.Rows.Fixed)
		{
			...
			if ((Table.BorderStyle ?? TableBorderStyles.Grid).LeftRightLine == Leqisoft.Model.LineStyle.None)
			{
	...
	private void OwnerDrawCell_Warning(OwnerDrawCellEventArgs e)
	{
		...
		if (Table.ControlWarningCells.Count > 0)
		{
			for (int i = 0; i < count; i++)
			{
				Leqisoft.Model.Cell item = Table[row.Index, i];
		...
		if (Table.ControlRemindCells.Count > 0)
		{
			...
			{
				Leqisoft.Model.Cell item2 = Table[row.Index, j];
		...
		if (_warningTextIsShown)
		{
			if (flag)
			{
				image = Leqisoft.UI.Platform.Properties.Resources.Warning16;
			}
			else if (flag2)
			{
				image = Leqisoft.UI.Platform.Properties.Resources.Remind16;
			}
		}
		e.Image = image;
	...
	private void BodyOwnerDrawCell_Style(OwnerDrawCellEventArgs e)
	{
		...
		try
		{
			...
			{
				e.Text = string.Empty;
				e.Image = (true.Equals(cell.Value) ? Leqisoft.UI.Platform.Properties.Resources.On : Leqisoft.UI.Platform.Properties.Resources.Off);
				styleNew.ImageAlign = C1FlexGridEx.ToImageAlign(cell.DisplayAlign);
			}
			else if (Table.CellPropManager.TryGetAttachments(cell, out attachments))
			{
				e.Image = Leqisoft.UI.Platform.Properties.Resources.CellAttachment;
			...
			else
			{
				...
				if (Leqisoft.UI.Controls.Util.RgbEquals(styleNew.BackColor, Color.White))
				{
					Leqisoft.Model.Cell headerCell;
					if (cell.HasFormula)
					{
						styleNew.BackColor = Leqisoft.UI.Controls.Util.DarkenColor(UserSet.Config.TableStyle.FormalaColor, 0.08);
					}
					...
					{
						IEnumerable<Tuple<Leqisoft.Model.Cell, ValidationResult>> source = ((IEnumerable<Tuple<Leqisoft.Model.Cell, ValidationResult>>)value.Cells).Where((Tuple<Leqisoft.Model.Cell, ValidationResult> t) => t.Item1 == Table[e.Row, e.Col]);
						...
						IEnumerable<Tuple<Leqisoft.Model.Column, ValidationResult>> source3 = ((IEnumerable<Tuple<Leqisoft.Model.Column, ValidationResult>>)value.Columns).Where((Tuple<Leqisoft.Model.Column, ValidationResult> t) => t.Item1.Index == e.Col);
						IEnumerable<Tuple<Leqisoft.Model.Cell, ValidationResult>> source4 = ((IEnumerable<Tuple<Leqisoft.Model.Cell, ValidationResult>>)value.HeaderCells).Where((Tuple<Leqisoft.Model.Cell, ValidationResult> t) => t.Item1.Column.Index == e.Col && t.Item1.Row.Index < e.Row && t.Item1.GetHeaderLastRow() >= e.Row);
			...
			styleNew.Border.Width = (((Table.BorderStyle ?? TableBorderStyles.Grid).BodyLine != Leqisoft.Model.LineStyle.None) ? 1 : 0);
			...
			if (!isIconOutOfRange)
			{
				...
				if (_isMouseOverCancelManualInputIcon)
				{
					...
					_cancelManualInputBackgroundBrush.Color = Leqisoft.UI.Controls.Util.DarkenColor(styleNew.BackColor, 0.1);
					e.Graphics.FillRectangle(_cancelManualInputBackgroundBrush, rect);
				}
				e.Graphics.DrawImage(Leqisoft.UI.Platform.Properties.Resources.CancelManualInput, cancelManualInputIconArea.X, cancelManualInputIconArea.Y);
				e.Handled = true;
			}
			Color GetBackColor()
			{
	...
	{
		return Table.Columns.Any((Leqisoft.Model.Column u) => u.IsExistFillFormula);
	}
	...
	private void _grid_BodyBeforeEdit(object sender, RowColEventArgs e)
	{
		...
		try
		{
			...
			Leqisoft.Model.Cell cell = Table[_isEditingHeaders ? (e.Row - 1) : e.Row, e.Col];
			if (cell == null)
			{
				return;
			}
	...
	private void _grid_BodyAfterRowColChange(object sender, RangeEventArgs e)
	{
		...
		try
		{
			...
			{
				Leqisoft.Model.Cell cell = Table[e.NewRange.r1, e.NewRange.c1];
	...
	private void _grid_BodyStartEdit(object sender, RowColEventArgs e)
	{
		...
		Leqisoft.Model.Operand list = GetList(dataFormat.ComboList);
		if (_grid.Editor == ListDropDown.DropDown)
		{
	...
	{
		if (Theme.SelectedLeqiTheme.ThemeFlags.HasFlag(ThemeEnum.Picture))
		{
	...
	private void _grid_Paint(object sender, PaintEventArgs e)
	{
		...
		void DrawBeginEditingColHeader(PaintEventArgs e)
		{
			...
			{
				Leqisoft.Model.Column column = Table.Columns[_mouseHeaderCol - _grid.Cols.Fixed];
				if (CanEditColumn(column) && !column.IsLocked && column.Width >= GetColumnMinWidthForShowColumnHeaderIcon())
				{
					...
					if (_isMouseInStartEditingColHeaderImageRect)
					{
						_brushStartEditingColHeaderBackground.Color = Leqisoft.UI.Controls.Util.DarkenColor(_grid.Styles.SelectedColumnHeader.BackColor, 0.1);
						e.Graphics.FillRectangle(_brushStartEditingColHeaderBackground, editColHeaderImageRectangle);
					}
					e.Graphics.DrawImage(Leqisoft.UI.Platform.Properties.Resources.EditColHeader, editColHeaderImageRectangle.Location);
				}
				if (column.Width >= GetColumnMinWidthForShowColumnHeaderIcon() && !_grid.FilterManager.IsColumnInFilting(_mouseHeaderCol))
				{
					...
					e.Graphics.DrawImage(Leqisoft.UI.Platform.Properties.Resources.menuMoreOperation, colHeaderShowMoreMenuImageRectangle.Location);
				}
			}
		}
		void DrawBorderStyle(PaintEventArgs e)
		{
			...
			if (borderStyle != null)
			{
				...
				{
					if (borderStyle.UpDownLine == Leqisoft.Model.LineStyle.Thick)
					{
			...
			if (FormulaEditor.IsEditing || ValidationEditor.IsEditing || AuxEditor.IsEditing || TitleEditor.AuxEditor.IsEditing || FootEditor.AuxEditor.IsEditing || LedgerCollectFormulaEditor.IsEditing)
			{
				...
				if (FormulaEditor.LastClickedComponent is TableEditor && _grid.Selection.TopRow >= 0 && _grid.Selection.LeftCol >= 0)
				{
					try
					{
						...
						_penAnimateDash.Color = ((Control.MouseButtons == MouseButtons.Left) ? Theme.SelectedLeqiTheme.ThemeContext.DarkColor : FormulaEditor.NextColor);
				...
				foreach (FormulaDisplayRef refInterval in FormulaEditor.RefIntervals)
				{
					...
					case FormulaDisplayRefKind.ColumnWildcard:
						if (ri.Table == Table)
						{
							...
							{
								Leqisoft.Model.Cell cell2 = FormulaEditor.Context.Cell;
					...
					case FormulaDisplayRefKind.HeaderCellWildcard:
						if (ri.Table == Table)
						{
							...
							{
								Leqisoft.Model.Cell cell = FormulaEditor.Context.Cell;
	...
	private Rectangle GetEditColHeaderImageRectangle(int col)
	{
		...
		int y = cellRect.Top + (cellRect.Height - Leqisoft.UI.Platform.Properties.Resources.EditColHeader.Height) / 2;
		return new Rectangle(new Point(x, y), Leqisoft.UI.Platform.Properties.Resources.EditColHeader.Size);
	}

	private Rectangle GetColHeaderShowMoreMenuImageRectangle(int col)
	{
		Rectangle cellRect = _grid.GetCellRect(_grid.Rows.Fixed - 1, col);
		int x = cellRect.Right - Leqisoft.UI.Platform.Properties.Resources.menuMoreOperation.Width - 8;
		int y = cellRect.Top + (cellRect.Height - Leqisoft.UI.Platform.Properties.Resources.menuMoreOperation.Height) / 2;
		return new Rectangle(new Point(x, y), Leqisoft.UI.Platform.Properties.Resources.menuMoreOperation.Size);
	}
	...
	private void Frf_FindNextHandler(object sender, FindNextEventArgs e)
	{
		...
		if (e.ScopeMode == ScopeMode.Current)
		{
			...
			{
				foreach (Leqisoft.Model.Cell item in from c in Table.Cells.Skip(bodySelection.TopRow * Table.Columns.Count + bodySelection.LeftCol + 1)
				...
				foreach (Leqisoft.Model.Cell item2 in from c in Table.Cells.Take(bodySelection.TopRow * Table.Columns.Count + bodySelection.LeftCol + 1)
					where _grid.BodyGetRow(c.Row.Index).Visible && _grid.BodyGetCol(c.Column.Index).Visible
					select c)
				{
				...
				Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "已查找当前表格，未找到字符串。");
				return;
			}
			for (int i = bodySelection.TopRow; i <= bodySelection.BottomRow; i++)
			{
			...
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "当前选区内无查找结果");
		}
		else
		{
			...
			foreach (Leqisoft.Model.Cell item3 in from c in Table.Cells.Skip(bodySelection.TopRow * Table.Columns.Count + bodySelection.LeftCol + 1)
				where _grid.BodyGetRow(c.Row.Index).Visible && _grid.BodyGetCol(c.Column.Index).Visible
				select c)
			{
			...
			foreach (Leqisoft.Model.Table item4 in from n in Leqisoft.Model.Project.Current.GetAllTableNodes().SkipWhile((TreeTableNode n) => n.Table != Table).Skip(1)
				select n.Table)
			{
				foreach (Leqisoft.Model.Cell cell in item4.LoadAndReturn().Cells)
				{
			...
			foreach (Leqisoft.Model.Table item5 in from n in Leqisoft.Model.Project.Current.GetAllTableNodes().TakeWhile((TreeTableNode n) => n.Table != Table)
				select n.Table)
			{
				foreach (Leqisoft.Model.Cell cell2 in item5.LoadAndReturn().Cells)
				{
			...
			foreach (Leqisoft.Model.Cell item6 in from c in Table.Cells.Take(bodySelection.TopRow * Table.Columns.Count + bodySelection.LeftCol + 1)
				where _grid.BodyGetRow(c.Row.Index).Visible && _grid.BodyGetCol(c.Column.Index).Visible
				select c)
			{
			...
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "已查找全部表格，未找到字符串。");
		}
	}
	...
	private void Frf_ReplaceHandler(object sender, ReplaceEventArgs e)
	{
		...
		if (e.IsReplaceAll)
		{
			...
			{
				foreach (TreeTableNode allTableNode in Leqisoft.Model.Project.Current.GetAllTableNodes())
				{
		...
		else
		{
			...
			Leqisoft.Model.Cell cell2 = Table[bodySelection2.TopRow, bodySelection2.LeftCol];
			ReplaceCell(cell2);
		...
		Invalidate();
		void ReplaceCell(Leqisoft.Model.Cell cell)
		{
		...
		void ReplaceTable(Leqisoft.Model.Table table)
		{
			table.BeginBatchUpdateValue();
			foreach (Leqisoft.Model.Cell cell3 in table.Cells)
			{
				ReplaceCell(cell3);
			}
			table.EndBatchUpdateValue();
	...
	private void Initialize(MainForm owner)
	{
		...
		cmdCollectFill2.Image = Leqisoft.UI.Platform.Properties.Resources.GenerateWorkingPaper;
		...
		cmdCalculateTable2.Image = Leqisoft.UI.Platform.Properties.Resources.CalculateTable;
		...
		cmdValidateTable2.Image = Leqisoft.UI.Platform.Properties.Resources.ValidateTable;
		...
		cmdLockTable.Image = Leqisoft.UI.Platform.Properties.Resources.ToggleLockTable;
		...
		cmdExportTable.Image = Leqisoft.UI.Platform.Properties.Resources.ExportExcel;
		...
		cmdFoot.Image = Leqisoft.UI.Platform.Properties.Resources.TableFoot;
		...
		cmdToolbarTables.Image = Leqisoft.UI.Platform.Properties.Resources.ToolbarTable;
		...
		cmdBack.Image = Leqisoft.UI.Platform.Properties.Resources.back32;
		cmdBack.Click += CmdBack_Click;
		lnkForward.Command = cmdForward;
		cmdForward.Image = Leqisoft.UI.Platform.Properties.Resources.forward32;
		cmdForward.Click += CmdForward_Click;
		lnkTicketInputMode.Command = cmdTicketInputMode;
		lnkTicketInputMode.Delimiter = true;
		cmdTicketInputMode.Image = Leqisoft.UI.Platform.Properties.Resources.TicketMode;
	...
	private bool _navTreeTitle_Paint(object sender, PaintEventArgs e)
	{
		...
		void Draw_MoreMenuIcon()
		{
			if (_isMouseOverNavTreeTitlePanel)
			{
				bool flag = false;
				Bitmap bitmap = null;
				if (Theme.SelectedLeqiTheme.ThemeContext.OutBarPageMoreMenuImageIndex == OutBarPageMoreMenuImageIndex.White)
				{
					if (_menuMoreOperationWhiteImage == null)
					{
						_menuMoreOperationWhiteImage = (Bitmap)new WhiteImageStrategy().ProcessImage(Leqisoft.UI.Platform.Properties.Resources.menuMoreOperation);
					}
				...
				{
					bitmap = Leqisoft.UI.Platform.Properties.Resources.menuMoreOperation;
				}
				if (_isMouseOverNavTreePanelMoreMenuIcon)
				{
					if (flag)
					{
						_brushMoreMenuIconBackground.Color = Leqisoft.UI.Controls.Util.LightColor(_navTreeTitleBackgroundColor, 0.2);
					}
					else
					{
						_brushMoreMenuIconBackground.Color = Leqisoft.UI.Controls.Util.DarkenColor(_navTreeTitleBackgroundColor, 0.1);
					}
		...
		{
			e.Graphics.DrawImage(Leqisoft.UI.Platform.Properties.Resources.tableNavTreeIcon16, new Point(10, 7));
		}
	}
	...
	private Point GetNavTreeTitleMoreMenuIconLeftTopPosition()
	{
		...
		int y = clientRectangle.Y + (clientRectangle.Height - Leqisoft.UI.Platform.Properties.Resources.menuMoreOperation.Height) / 2;
	...
	private Rectangle GetNavTreeTitleMoreMenuIconBackgroundRectangle()
	{
		...
		int num = 3;
		int num2 = 3;
		return new Rectangle(navTreeTitleMoreMenuIconLeftTopPosition.X - num, navTreeTitleMoreMenuIconLeftTopPosition.Y - num2, Leqisoft.UI.Platform.Properties.Resources.menuMoreOperation.Width + num * 2, Leqisoft.UI.Platform.Properties.Resources.menuMoreOperation.Height + num2 * 2);
	}
	...
	private void SetNavTreeTitlePanelBackgroundBrush()
	{
		...
		C1Theme c1Theme = Theme.SelectedLeqiTheme.GetC1Theme();
		ThemeBackground background = Theme.SelectedLeqiTheme.GetBackground("C1Command\\C1OutBar\\Page\\Title\\Default\\Background");
	...
	private void PopulateColumn(Leqisoft.Model.Column model, C1.Win.C1FlexGrid.Column view)
	{
	...
	public void PopulateColumns()
	{
		...
		{
			foreach (Leqisoft.Model.Column column2 in _table.Columns)
			{
			...
			List<Leqisoft.DTO.CellRange> mergeInfo = _table.GetMergeInfo(visibleOnly: false);
			foreach (Leqisoft.DTO.CellRange item in mergeInfo)
			{
		...
		{
			Leqisoft.Model.Column column = Table.Columns[n];
	...
	private void PopulateRow(Leqisoft.Model.Row model, C1.Win.C1FlexGrid.Row view)
	{
	...
	private void MutateTableColumnCellStyle<T>(Expression<Func<Leqisoft.Model.CellStyle, T>> propertySelector, T value, Leqisoft.Model.Column column)
	{
		...
		if (CanEditColumn(column))
		{
			...
			column.UpdateStyle(Table.CellStyles.MutateAndGet(column.Style, delegate(Leqisoft.Model.CellStyle s)
			{
				pi.SetValue(s, value);
				SetStyleFormatIsAllowManualInput(s, oldIsAllowManualInput);
			}));
		}
		static void SetStyleFormatIsAllowManualInput(Leqisoft.Model.CellStyle cellStyle, bool? isAllowManualInput)
		{
	...
	private void MutateCellStyle<T>(Expression<Func<Leqisoft.Model.CellStyle, T>> propertySelector, T value, bool updateTableDefaultIfSelectAll = true, bool clearCellStyleIfEntireColumn = true)
	{
		PropertyInfo pi = (PropertyInfo)((MemberExpression)propertySelector.Body).Member;
		bool flag = false;
		bool flag2 = true;
		...
		else if (_grid.IsEntireColumnSelected && flag2)
		{
			...
			{
				Table.UpdateDefaultStyle(Table.CellStyles.MutateAndGet(Table.DefaultStyle, delegate(Leqisoft.Model.CellStyle s)
				{
					pi.SetValue(s, value);
				}));
				foreach (Leqisoft.Model.Column column2 in Table.Columns)
				{
					...
					column2.UpdateStyle(Table.CellStyles.MutateAndGet(column2.Style, delegate(Leqisoft.Model.CellStyle s)
					{
						pi.SetValue(s, null);
				...
				foreach (Leqisoft.Model.Cell cell in Table.Cells)
				{
					...
					{
						cell.UpdateStyle(Table.CellStyles.MutateAndGet(cell.Style, delegate(Leqisoft.Model.CellStyle s)
						{
							pi.SetValue(s, null);
						}));
					}
			...
			else
			{
				...
				{
					foreach (Leqisoft.Model.Cell item2 in Table.EnumerateCellRange(0, bodySelection.LeftCol, Table.Rows.Count - 1, bodySelection.RightCol))
					{
						if (item2.ShouldApplyColumnFormula())
						{
							item2.UpdateStyle(Table.CellStyles.MutateAndGet(item2.Style, delegate(Leqisoft.Model.CellStyle s)
							{
								pi.SetValue(s, null);
							}));
						}
		...
		{
			foreach (Leqisoft.Model.Cell item3 in Table.EnumerateCellRange(bodySelection.TopRow, bodySelection.LeftCol, bodySelection.BottomRow, bodySelection.RightCol))
			{
				if ((!flag || IsGridRowVisible(item3)) && CanEditRow(item3.Row) && CanEditColumn(item3.Column) && item3.ShouldApplyColumnFormula())
				{
					item3.UpdateStyle(Table.CellStyles.MutateAndGet(item3.Style, delegate(Leqisoft.Model.CellStyle s)
					{
						pi.SetValue(s, value);
					}));
				}
	...
	private bool IsGridRowVisible(Leqisoft.Model.Cell cell)
	{
	...
	private void TableStyleBrushImpl(Leqisoft.Model.Table source, Leqisoft.Model.Table dst)
	{
		List<Leqisoft.Model.Cell> list = null;
		Leqisoft.Model.CellStyle cellStyle = null;
		foreach (Leqisoft.Model.Row item in dst.Rows.Where((Leqisoft.Model.Row r) => r.Role == RowRole.Total))
		{
			if (cellStyle == null)
			{
				cellStyle = dst.CellStyles.Get(dst.DefaultStyle, delegate(Leqisoft.Model.CellStyle cs)
				{
					cs.Align = CellTextAlign.MiddleCenter;
				});
				if (cellStyle == null)
			...
			for (int i = 0; i < count; i++)
			{
				Leqisoft.Model.Cell cell = dst[item.Index, i];
				if (!(cell.Formula != "\"合计\"") && cell.Style != null && cell.Style.Equals(cellStyle))
				{
					if (list == null)
					{
						list = new List<Leqisoft.Model.Cell>();
					}
					list.Add(cell);
				}
			}
		...
		dst.TagTitleDirty();
		foreach (Leqisoft.Model.Column column in dst.Columns)
		{
		...
		{
			flag = true;
			dst.UpdateDefaultStyle(dst.CellStyles.MutateAndGet(dst.DefaultStyle, delegate(Leqisoft.Model.CellStyle cs)
			{
		...
		foreach (Leqisoft.Model.Cell item2 in list)
		{
			Leqisoft.Model.CellStyle style = dst.CellStyles.MutateAndGet(item2.Style, delegate(Leqisoft.Model.CellStyle cs)
			{
				cs.FontFamily = source.DefaultStyle.FontFamily;
				cs.FontSize = source.DefaultStyle.FontSize;
			});
	...
	private async Task<bool> ValidateConsolidateSettings()
	{
		...
		foreach (ConsolidateEntry s in Table.ConsolidateSettings.Sources)
		{
			try
			{
				Leqisoft.DTO.Project project = (await StorageRouter.GetProjects()).FirstOrDefault(p => p.Id == s.ProjectId);
				s.Project = new Leqisoft.Model.Project
				{
					Id = s.ProjectId,
					Name = project.Name
				};
			...
			catch (HttpRequestException ex)
			{
				...
				{
					Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "被合并表格所在的" + StringConstBase.Current.Project + "已被删除，请重新设置合并参数。");
				}
				else
				{
			...
			if (s.Table == null)
			{
				Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "表格不存在，可能是表格已在云端删除，请重新设置");
				return false;
			}
			try
			{
			...
			s.GroupSrc = new List<Leqisoft.Model.Column>();
			...
			s.DataSrc = new List<Leqisoft.Model.Column>();
			foreach (Id64 item2 in s.DataSrcId)
			{
				Leqisoft.Model.Column byId2 = s.Table.Columns.GetById(item2);
				if (byId2 == null)
				{
					Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "数据列不存在，可能是列已在云端删除，请重新设置");
					return false;
				}
				s.DataSrc.Add(byId2);
			}
		}
		Table.ConsolidateSettings.GroupDest = new List<Leqisoft.Model.Column>();
		...
		Table.ConsolidateSettings.AggregateDest = new List<Leqisoft.Model.Column>();
	...
	private void MergeRowCellsImpl(int rowIndex, int leftCol, int rightCol)
	{
		...
		Leqisoft.Model.Cell cell = Table[rowIndex, leftCol];
	...
	public bool CanEditRow(Leqisoft.Model.Row row)
	{
		if (_owner.CurrentProject.Creator.Id == Leqisoft.Model.User.Current.Id)
		{
			return true;
		}
		if (!_owner.CurrentProject.Users.Any((KeyValuePair<Leqisoft.DTO.User, UserRole> kv) => kv.Key.Id == Leqisoft.Model.User.Current.Id))
		{
			return false;
		}
		if (_owner.CurrentProject.Users.First((KeyValuePair<Leqisoft.DTO.User, UserRole> kv) => kv.Key.Id == Leqisoft.Model.User.Current.Id).Value == UserRole.Manager)
		{
			return true;
		}
		if (Table.RowOwnerExclusive)
		{
			if (row.Creator != Leqisoft.Model.User.Current.Id)
			{
				return Table.RowOwnerLoadShare.Exists(row.Creator, Leqisoft.Model.User.Current.Id);
			}
			return true;
		}
	...
	private bool CanEditCell(Leqisoft.Model.Cell cell, bool isTableExistFillFormula, bool ignoreProp = false)
	{
		if (!cell.IsEditable)
		{
			return false;
	...
	public static bool IsCurrentUserCanEditColumn(Leqisoft.Model.Column column)
	{
		if (Leqisoft.Model.Project.Current.Creator.Id == Leqisoft.Model.User.Current.Id)
		{
			return true;
		}
		if (!Leqisoft.Model.Project.Current.Users.Any((KeyValuePair<Leqisoft.DTO.User, UserRole> kv) => kv.Key.Id == Leqisoft.Model.User.Current.Id))
	...
	public bool CanEditColumn(Leqisoft.Model.Column column)
	{
		if (Leqisoft.Model.Project.Current.Creator.Id == Leqisoft.Model.User.Current.Id)
		{
			return true;
	...
	private void ExecuteConsolidateImpl(bool showDataCols)
	{
		...
		List<Leqisoft.Model.Column> list5 = new List<Leqisoft.Model.Column>();
		for (int l = 0; l < list3.Count; l++)
		{
			...
			for (j = 0; j < count; j++)
			{
				Leqisoft.Model.Column column2 = Table.Columns.FirstOrDefault((Leqisoft.Model.Column c) => c.ConsolidateAttributes != null && c.ConsolidateAttributes.Role == ConsolidateRole.Data && c.ConsolidateAttributes.ProjectId == s2.ProjectId && c.ConsolidateAttributes.ColumnId == s2.DataSrcId[j]);
				Leqisoft.Model.Column column3 = Table.ConsolidateSettings.AggregateDest[j];
		...
		{
			Leqisoft.Model.Column column4 = Table.ConsolidateSettings.AggregateDest[m];
		...
		{
			Leqisoft.Model.Column column5 = Table.ConsolidateSettings.GroupDest[n];
		...
		List<Tuple<Leqisoft.Model.Row, List<object>>> list7 = (from r in Table.Rows
			...
			select Tuple.Create(r, Table.ConsolidateSettings.GroupDest.Select((Leqisoft.Model.Column c) => Table[r.Index, c.Index].Value).ToList())).ToList();
		list7.Reverse();
		foreach (Tuple<Leqisoft.Model.Row, List<object>> row2 in list7)
		{
		...
		Leqisoft.Model.Row row3 = Table.Rows.FirstOrDefault((Leqisoft.Model.Row r) => r.Role == RowRole.Subtotal || r.Role == RowRole.Total);
		if (row3 != null && row3.Index == 0 && row3.IsLocked)
		{
			row3 = null;
		}
		int num2 = row3?.Index ?? Table.Rows.Count;
		Leqisoft.Model.Row[] array = new Leqisoft.Model.Row[keys.Count];
		int i;
		for (i = 0; i < keys.Count; i++)
		{
			Tuple<Leqisoft.Model.Row, List<object>> tuple = list7.FirstOrDefault((Tuple<Leqisoft.Model.Row, List<object>> tup) => tup.Item2.SequenceEqual(keys[i].Key));
			if (tuple == null)
			{
	...
	private void ExecuteConsolidateAppend(bool showDataCols)
	{
		...
		var allRows = new List<Tuple<ConsolidateEntry, List<Leqisoft.Model.Row>>>();
	...
	private void ShowCommentTooltipImpl()
	{
		...
		Leqisoft.Model.Cell cell = Table[_grid.BodyRow, _grid.BodyCol];
		if (cell == null) return;
		Leqisoft.Model.Column column = cell.Column;
		...
		if (!CanEditCell(cell, IsTableExistFillFormula()))
		{
			...
			Leqisoft.Model.Row row = cell.Row;
			if (!CanEditRow(row))
			{
				flag = true;
				if (Table.RowOwnerExclusive)
		...
		if (flag5)
		{
			try
			{
				...
				else
				{
					...
					linkDic.Add("refresh", new Tuple<Leqisoft.Model.Table, int, int>(Table, _grid.BodyRow, _grid.BodyCol));
					opRow1.Add(new XElement("td", new XElement("a", new XAttribute("href", $"tableTrace_{_grid.BodyRow}_{_grid.BodyCol}"), "追踪数据")));
					linkDic.Add($"tableTrace_{_grid.BodyRow}_{_grid.BodyCol}", new Tuple<Leqisoft.Model.Table, int, int>(Table, _grid.BodyRow, _grid.BodyCol));
					opTable.Add(opRow1);
					xBody.Add(opTable);
					xBody.Add(new XElement("hr"));
				}
		...
		if (flag6)
		{
			IEnumerable<Tuple<Leqisoft.Model.Cell, ValidationResult>> enumerable = ((IEnumerable<Tuple<Leqisoft.Model.Cell, ValidationResult>>)value.Cells).Where((Tuple<Leqisoft.Model.Cell, ValidationResult> t) => t.Item1 == Table[_grid.BodyRow, _grid.BodyCol]).Distinct(new ValidateResultEqualityComparer());
			...
			IEnumerable<Tuple<Leqisoft.Model.Column, ValidationResult>> enumerable3 = ((IEnumerable<Tuple<Leqisoft.Model.Column, ValidationResult>>)value.Columns).Where((Tuple<Leqisoft.Model.Column, ValidationResult> t) => t.Item1.Index == _grid.BodyCol);
			IEnumerable<Tuple<Leqisoft.Model.Cell, ValidationResult>> enumerable4 = ((IEnumerable<Tuple<Leqisoft.Model.Cell, ValidationResult>>)value.HeaderCells).Where((Tuple<Leqisoft.Model.Cell, ValidationResult> t) => t.Item1.Column.Index == _grid.BodyCol && t.Item1.Row.Index < _grid.BodyRow && t.Item1.GetHeaderLastRow() >= _grid.BodyRow);
			...
			if (flag6)
			{
				...
				{
					i = 0;
					foreach (Tuple<Leqisoft.Model.Cell, ValidationResult> item5 in enumerable)
					{
					...
					i = 0;
					foreach (Tuple<Leqisoft.Model.Column, ValidationResult> item7 in enumerable3)
					{
						AddValidationResult(item7.Item2, "l");
						i++;
					}
					i = 0;
					foreach (Tuple<Leqisoft.Model.Cell, ValidationResult> item8 in enumerable4)
					{
	...
	private void PopulateTopLeftCell()
	{
		if (IsTableLocked)
		{
			_grid.SetCellImage(0, 0, Leqisoft.UI.Platform.Properties.Resources.TableLock);
		}
		else
		{
	...
	public Leqisoft.Model.Operand GetList(string comboList)
	{
		...
		try
		{
			...
			Leqisoft.Model.Operand operand = formulaEvaluator.EvaluateToOperand();
	...
	public void ScrollToAvailableRow(List<Leqisoft.Model.Row> rowsList)
	{
		if (rowsList == null || rowsList.Count == 0)
		{
			return;
		}
		foreach (Leqisoft.Model.Row rows in rowsList)
		{
