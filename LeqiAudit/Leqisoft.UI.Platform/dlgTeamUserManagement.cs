...
public class dlgTeamUserManagement : C1RibbonForm
{
	...
	private ListTileViewMode _Mode
	{
		get
		{
			return (Leqisoft.UI.Platform.ListTileViewMode)UserSet.Config.TeamUsersViewMode;
		}
		set
		{
	...
	internal dlgTeamUserManagement()
	{
		...
		_isAdmin = Leqisoft.Model.User.Current.IsTeamAdmin;
		...
		_listDropDown.SimpleCheckedList.Op = new ValueSetOperand(from v in UserTeamPermissions.GetValues()
			select Tuple.Create<Leqisoft.Model.Row, ValueOperand>(null, ValueOperand.FromObject(v)));
		...
		Leqisoft.UI.Controls.Theme.SetCurrentObject(_listDropDown.DropDown);
		SetTheme();
	...
	{
		base.Icon = Leqisoft.UI.Controls.Theme.SelectedLeqiTheme.GetThemedIcon(Leqisoft.UI.Platform.Properties.Resources.Users);
		Text = (Leqisoft.Model.User.Current.IsTeamAdmin ? "同事管理" : "我的同事") + " 【" + UserTeam.Current.Name + "】";
		toolLnkDisplayMode.Delimiter = Leqisoft.Model.User.Current.IsTeamAdmin;
		await Populate();
	}
	...
	private void CmdMoveUserGroup1_CommandStateQuery(object sender, CommandStateQueryEventArgs e)
	{
		...
		Leqisoft.DTO.User user = row?.UserData as Leqisoft.DTO.User;
	...
	internal async Task<bool> PopulateUserList()
	{
		...
		try
		{
			if (StorageRouter.IsLocalMode)
			{
				...
				foreach (Leqisoft.DTO.User user in users.OrderByDescending((Leqisoft.DTO.User u) => u.Id == managerId))
				{
			...
			else
			{
				...
				foreach (Leqisoft.DTO.User item2 in tuple.Item1.OrderByDescending((Leqisoft.DTO.User u) => u.Id == managerId))
				{
		...
		void AppendGroupNode(UserGroup userGroup, Node parentNode)
		{
			...
			_grid.SetCellImage(row2.Index, "CN_USERNAME", Leqisoft.UI.Platform.Properties.Resources.imgUserGroup);
			row2.AllowEditing = false;
			foreach (UserGroup child in userGroup.Children)
			{
				AppendGroupNode(child, row2.Node);
			}
			foreach (Leqisoft.DTO.User user in userGroup.Users)
			{
				AppendUserNode(user, row2.Node);
			}
		}
		void AppendUserNode(Leqisoft.DTO.User user, Node parentNode)
		{
			...
			System.Drawing.Image image = Leqisoft.UI.Controls.Util.GetHeadPic(user, 32, withManagerMark: true);
			if (!flag)
			{
	...
	internal async Task<bool> PopulateUserTile()
	{
		...
		try
		{
			...
			IEnumerable<Leqisoft.DTO.User> users = StorageRouter.IsLocalMode 
				? await StorageRouter.GetTeamUsersWithPic() 
				: await WebApiClient.GetTeamUsersWithPic();
			foreach (Leqisoft.DTO.User item in users.OrderByDescending((Leqisoft.DTO.User u) => u.IsTeamAdmin))
			{
		...
		return success;
		Tile createUserTile(Leqisoft.DTO.User user)
		{
			bool flag = members.FirstOrDefault((Member m) => m.Id == user.Id.ToString())?.IsOnline ?? false;
			System.Drawing.Image image = Leqisoft.UI.Controls.Util.GetHeadPic(user, 32, withManagerMark: true);
			if (!flag)
	...
	private void _grid_MouseClick(object sender, MouseEventArgs e)
	{
		...
		if (e.Button == MouseButtons.Right)
		{
			...
			{
				if (row.UserData is Leqisoft.DTO.User)
				{
	...
	{
		if (!_isAdmin || e.Row < _grid.Rows.Fixed || !(_grid.Rows[e.Row].UserData is Leqisoft.DTO.User user))
		{
			return;
		}
		if (e.Col == _grid.Cols.IndexOf("CN_JOBTITLE"))
		{
			...
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "职务名称不能超过20个字符");
		}
		else if (e.Col == _grid.Cols.IndexOf("CN_PERMISSIONS"))
		{
			string text2 = ((string)_grid[e.Row, e.Col]) ?? "";
			user.Permissions = UserTeamPermissions.Parse(text2);
			await WebApiClient.SetUserTeamPermissions(new Leqisoft.DTO.User
			{
				Id = user.Id,
				TeamId = Leqisoft.Model.User.Current.TeamId,
	...
	{
		if (_grid.Row >= _grid.Rows.Fixed && _grid.Row < _grid.Rows.Count && _grid.Rows[_grid.Row].UserData is Leqisoft.DTO.User user)
		{
			_ = user.Id;
			_ = Leqisoft.Model.User.Current.Id;
		}
	}
	...
	private void UpdateJobTitleComboList()
	{
		...
		{
			if (item.UserData is Leqisoft.DTO.User user && !string.IsNullOrWhiteSpace(user.JobTitle))
			{
				hashSet.Add(user.JobTitle);
			}
		}
		ValueSetOperand op = new ValueSetOperand(hashSet.Select((string j) => Tuple.Create<Leqisoft.Model.Row, ValueOperand>(null, ValueOperand.FromObject(j))));
	...
	private async void CmdAddChildGroup2_Click(object sender, ClickEventArgs e)
	{
		...
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "当前选中节点不是用户组！无法进行该操作");
		}
	}
	...
	private async Task AddUserToTeamImpl()
	{
		...
		try
		{
			Leqisoft.DTO.User ret = await WebApiClient.AddUserToTeam(text);
			ret.TeamId = Leqisoft.Model.User.Current.TeamId;
			await Populate();
			if (ret != null)
			{
				await SignalRClient.ChangeTeamMember(ret.Id.ToString(), Guid.Empty.ToString(), Leqisoft.Model.User.Current.TeamId.ToString());
			}
			SetViewState();
		}
		catch (HttpRequestException ex)
	...
	private async Task RemoveUserFromTeamImpl()
	{
		...
		try
		{
			...
			if (!(((_Mode != ListTileViewMode.Tile) ? currentRow()?.UserData : _tileControl.SelectedTile?.Tag) is Leqisoft.DTO.User user))
			{
				Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "请选择移除的同事");
			}
			else if (user.Id == current.Id)
			{
				Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "不能移除自己。");
			}
	...
	private async Task AppendGroupImpl()
	{
		...
		try
		{
			...
			{
				Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "分组名称不能超过20个字符");
				return;
			}
			...
			_grid.SetCellImage(node.Row.Index, "CN_USERNAME", Leqisoft.UI.Platform.Properties.Resources.imgUserGroup);
	...
	private async Task DeleteUserGroup()
	{
		...
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "分组中有人员，无法删除");
			return;
		}
		_grid.BeginUpdate();
		try
	...
	private async Task LeaveTeamImpl()
	{
		if (Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.Question, "确定要退出组织吗？", MessageBoxButtons.OKCancel) == DialogResult.OK)
		{
			...
			{
				Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "成功退出组织");
				Close();
			}
		}
	}
	...
	{
		if (Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.Exclamation, "警告：此次操作将会解散组织，组织的所有" + StringConstBase.Current.Project + "和模板将会无法使用，请谨慎确认后再操作！\n\n若确认要解散组织，点击“确定”。", MessageBoxButtons.OKCancel) == DialogResult.OK)
		{
			string text = InputForm.Text("解散验证", "请输入您的用户名以防止是在您的误操作下解散组织：", null, 256);
			if (text != Leqisoft.Model.User.Current.UserName)
			{
				Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "输入用户名不正确，终止解散组织！");
			}
			else if (await Program.DismissTeam())
			{
				await Program.GetUserTeams(withNotice: false);
				Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.Asterisk, "组织已被解散。");
				Close();
			}
		}
	}
	...
	private async Task MergeTeamRequest()
	{
		...
		if (formMergeTeam.ShowDialog() == DialogResult.OK)
		{
			try
			{
				...
				Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "合并组织成功。");
				await Populate();
			}
			catch (HttpRequestException ex)
			{
	...
	private void cmdToolDisplayMode_CommandStateQuery(object sender, CommandStateQueryEventArgs e)
	{
		...
		case ListTileViewMode.List:
			...
			toolCmdDisplayMode.Image = Leqisoft.UI.Controls.Theme.SelectedLeqiTheme.GetThemedBitmap(Leqisoft.UI.Platform.Properties.Resources.tileMode);
			break;
		case ListTileViewMode.Tile:
			toolCmdDisplayMode.Text = "列表模式";
			toolCmdDisplayMode.Image = Leqisoft.UI.Controls.Theme.SelectedLeqiTheme.GetThemedBitmap(Leqisoft.UI.Platform.Properties.Resources.listMode);
			break;
		}
	}
	...
	private void cmdToolRemoveTeamUser_CommandStateQuery(object sender, CommandStateQueryEventArgs e)
	{
		...
		{
		case ListTileViewMode.List:
			e.Enabled = currentRow()?.UserData is Leqisoft.DTO.User;
			break;
		case ListTileViewMode.Tile:
	...
	private async void toolCmdRenameTeam_Click(object sender, ClickEventArgs e)
	{
		...
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "组织名称不能超过50个字符");
			return;
		}
		try
		{
			...
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "组织名称修改成功！");
		}
		catch (HttpRequestException ex)
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, ex.InnerException.Message);
	...
	private void InitializeComponent()
	{
		...
		this.toolCmdAddTeamUser.Image = Leqisoft.UI.Platform.Properties.Resources.toolAddUserToTeam;
		...
		this.toolCmdRemoveTeamUser.Image = Leqisoft.UI.Platform.Properties.Resources.toolRemoveUserFromTeam;
		...
		this.toolCmdAddUserGroup.Image = Leqisoft.UI.Platform.Properties.Resources.addUserGroup;
		...
		this.toolCmdAddChildGroup.Image = Leqisoft.UI.Platform.Properties.Resources.addChildGroup;
		...
		this.toolCmdRemoveUserGroup.Image = Leqisoft.UI.Platform.Properties.Resources.toolDeleteGroup;
		...
		this.toolCmdRenameUserGroup.Image = Leqisoft.UI.Platform.Properties.Resources.toolRenameGroup;
		...
		this.toolCmdDisplayMode.Image = Leqisoft.UI.Platform.Properties.Resources.listMode;
		...
		this.toolCmdRenameTeam.Image = Leqisoft.UI.Platform.Properties.Resources.toolRenameGroup;
		...
		this.toolCmdMergeTeam.Image = Leqisoft.UI.Platform.Properties.Resources.toolMergeTeam;
		...
		this.toolCmdDismissTeam.Image = Leqisoft.UI.Platform.Properties.Resources.toolDismissTeam;
		...
		this.toolCmdLeaveTeam.Image = Leqisoft.UI.Platform.Properties.Resources.toolQuitTeam;
