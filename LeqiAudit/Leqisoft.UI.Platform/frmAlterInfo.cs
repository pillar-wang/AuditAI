...
public class frmAlterInfo : C1RibbonForm
{
	...
	private Leqisoft.DTO.User user;
	...
	{
		base.Icon = Theme.SelectedLeqiTheme.GetThemedIcon(Resources.SwitchUser);
	}
	...
	private async void Initialize()
	{
		...
		try
		{
			if (StorageRouter.IsLocalMode)
			{
				user = await Leqisoft.LocalDataStore.LocalDataStore.GetUserById(Leqisoft.Model.User.Current.Id);
			}
			else
			{
				user = await WebApiClient.GetUserById(Leqisoft.Model.User.Current.Id);
			}
	...
	private async void btnConfirm_Click(object sender, EventArgs e)
	{
		...
		try
		{
			...
			{
				Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "用户名不为空,且必须为2-20个字符不区分大小写");
				return;
			}
			if (!Regex.IsMatch(txtName.Text.Trim(), "^.{2,20}$"))
			{
				Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "姓名不为空,且必须为2-20个字符不区分大小写");
				return;
			}
			Leqisoft.DTO.User user = new Leqisoft.DTO.User
			{
			...
			{
				// 本地模式：直接保存，无需短信验证
				await Leqisoft.LocalDataStore.LocalDataStore.UpdateUserInfo(user);
			}
			...
			else
			{
				if (!_hadRequestSMS)
				{
					Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "修改手机号必须先请求短信验证码！");
					return;
				}
			...
			Leqisoft.Model.User.Current.Name = user.Name;
			Leqisoft.Model.User.Current.UserName = user.UserName;
	...
	private async void btnGetSMS_Click(object sender, EventArgs e)
	{
		...
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "手机号格式不正确！请检查是否为11位数字");
			return;
		}
		try
		{
			...
			{
				Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "手机号已注册！一个手机号只能绑定一个账号");
				return;
			}
			btnSMS.Format = "(0s)";
			btnSMS.Start(120);
	...
	private void pictureHead_Click(object sender, EventArgs e)
	{
		...
		if (openFileDialog.ShowDialog() == DialogResult.OK)
		{
			...
			{
				Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "未能打开此图片，请重试或者更改图片。");
			}
		}
	}
	...
	private void InitializeComponent()
	{
		...
		this.btnSMS = new Leqisoft.UI.Controls.TimerButton();
		this.txtSMS = new Leqisoft.UI.Controls.C1TextBoxEx();
