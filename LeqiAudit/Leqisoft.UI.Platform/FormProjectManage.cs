...
public class FormProjectManage : ISetTheme
{
	...
	protected class TilePayStatusIconRender : C1TileCustomButtonRender
	{
		...
		public bool OnTileSingleClicked(TileMouseEventArgs e)
		{
			...
			if (e.MouseEA.Button == MouseButtons.Left && _isMouseOverPayStatusIcon && _mouseOverTile != null && !_isMouseOverCreateProjectTile && _mouseOverTilePayStatusImage != null && _mouseOverTile.Tag is Leqisoft.DTO.Project project)
			{
	...
	private readonly List<Leqisoft.DTO.Project> _projects = new List<Leqisoft.DTO.Project>();
	...
	public Leqisoft.DTO.Project SelectedProject
	{
		get
		{
			if (Style.ViewMode == ListTileViewMode.List)
			{
				...
				{
					return _grid.BodyGetRow(_grid.BodyRow).UserData as Leqisoft.DTO.Project;
				}
				return null;
			}
			return _tile.SelectedTile?.Tag as Leqisoft.DTO.Project;
		}
	}

	public List<Leqisoft.DTO.Project> SelectedProjects
	{
		get
		{
			List<Leqisoft.DTO.Project> list = new List<Leqisoft.DTO.Project>();
			if (Style.ViewMode == ListTileViewMode.List)
			{
				for (int i = _grid.Rows.Fixed; i < _grid.Rows.Count; i++)
				{
					...
					{
						list.Add(_grid.Rows[i].UserData as Leqisoft.DTO.Project);
					}
				}
			}
			else
			{
				list.AddRange(_tile.SelectedTiles.Select((Tile t) => t.Tag as Leqisoft.DTO.Project));
			}
			return list;
		}
	}
	...
	public FormProjectManage()
	{
		...
		_form.Icon = Theme.SelectedLeqiTheme.GetThemedIcon(Resources.Projects16, Resources.Projects24);
		...
		AddRibbonButton(group2, "RB_USERMANAGE", Leqisoft.Model.User.Current.IsTeamAdmin ? "同事管理" : "我的同事");
		...
		group2 = AddRibbonGroup(tab2, "RG_RECYCLE", Leqisoft.Model.User.Current.IsTeamAdmin ? "恢复及删除" : "恢复");
		...
		ribbonButton.Visible = Leqisoft.Model.User.Current.IsTeamAdmin;
	...
	public void SetTheme()
	{
		...
		_tile.CustomBorderColor = Theme.SelectedLeqiTheme.ThemeContext.DarkColor;
	}
	...
	{
		List<Leqisoft.DTO.Project> projects = SortProjectImpl(_isSearch ? SearchProjects(_projects) : FilterProjects(_projects), Style.SortKind).ToList();
	...
	private async Task FetchModel()
	{
		...
		try
		{
			IEnumerable<Leqisoft.DTO.Project> projectList = new List<Leqisoft.DTO.Project>();
			...
			progressForm.ShowDialog(progressRuntimeData, async delegate
			{
				...
				if (State == ViewState.Project)
					{
						...
						{
							projectList = await Leqisoft.LocalDataStore.StorageRouter.GetProjects();
						}
						if (UserTeam.CurrentTeamIsPayByProject && projectList != null)
					{
						if (!StorageRouter.IsLocalMode)
					{
						IEnumerable<Leqisoft.DTO.Project> enumerable = await WebApiClient.GetTeamPayedProjects(UserTeam.Current.Id);
						Dictionary<Guid, Leqisoft.DTO.Project> dictionary = new Dictionary<Guid, Leqisoft.DTO.Project>();
						if (enumerable != null)
						{
							...
							foreach (Leqisoft.DTO.Project item2 in projectList)
							{
				...
				{
					projectList = await Leqisoft.LocalDataStore.StorageRouter.GetTemplates();
				}
				}
				else if (State == ViewState.RecycleProject)
				{
					...
					progressRuntimeData.UpdateProgress(0.8f);
					var recycleProjects = await Leqisoft.LocalDataStore.StorageRouter.GetRecycleProjects();
					projectList = recycleProjects.Where((Leqisoft.DTO.Project p) => p.Type == ProjectType.Project);
				}
				else
				{
					progressForm.SetProgressDisplayValueConverter(new ProgressDisplayValueConverter_SmoothByTime(0.2f));
					...
					progressRuntimeData.UpdateProgress(0.8f);
					var recycleProjects = await Leqisoft.LocalDataStore.StorageRouter.GetRecycleProjects();
					projectList = recycleProjects.Where((Leqisoft.DTO.Project p) => p.Type == ProjectType.Template);
				}
			});
			_projects.Clear();
	...
	private void PopulateForm()
	{
		if (State == ViewState.Project)
		{
			_form.Icon = Theme.SelectedLeqiTheme.GetThemedIcon(Resources.Projects24, Resources.Projects16);
			_form.Text = StringConstBase.Current.Project + "管理";
		}
		else if (State == ViewState.Template)
		{
			_form.Icon = Theme.SelectedLeqiTheme.GetThemedIcon(Resources.Templates16, Resources.Templates24);
			_form.Text = StringConstBase.Current.Template + "管理";
		}
		else if (State == ViewState.RecycleProject)
		{
			_form.Icon = Theme.SelectedLeqiTheme.GetThemedIcon(Resources.RemoveProject16, Resources.RemoveProject24);
	...
	private void PopulateGrid(List<Leqisoft.DTO.Project> projects)
	{
		_grid.BeginUpdate();
		_grid.Rows.Count = 1;
		_grid.Rows.Fixed = 1;
		...
		column.AllowEditing = false;
		Dictionary<Leqisoft.DTO.Project, bool> marked;
		if (State == ViewState.Project || State == ViewState.RecycleProject)
		{
			...
			if (State == ViewState.Project)
			{
				...
				marked = projects.ToDictionary((Leqisoft.DTO.Project p) => p, (Leqisoft.DTO.Project p) => false);
				for (int i = 0; i < projects.Count; i++)
				{
					Leqisoft.DTO.Project p2 = projects[i];
					AddProject(p2);
				}
			}
			else
			{
				_grid.Tree.Clear();
				foreach (Leqisoft.DTO.Project project3 in projects)
				{
		...
		else if (State == ViewState.Template || State == ViewState.RecycleTemplate)
		{
			...
			column.AllowEditing = false;
			_grid.Tree.Clear();
			foreach (Leqisoft.DTO.Project project4 in projects)
			{
		...
		void AddProject(Leqisoft.DTO.Project p)
		{
			if (!marked[p])
			{
				...
				{
					Leqisoft.DTO.Project project2 = projects.FirstOrDefault(delegate(Leqisoft.DTO.Project pr)
					{
						Guid id = pr.Id;
		...
		void PopulateProject(Leqisoft.DTO.Project project, C1.Win.C1FlexGrid.Row r)
		{
			...
			if (State == ViewState.Project || State == ViewState.RecycleProject)
			{
				...
				var users = project.Users ?? Enumerable.Empty<Leqisoft.DTO.User>();
				r["CN_PROJLEADER"] = users.Where((Leqisoft.DTO.User u) => u.Role == UserRole.Manager);
				r["CN_PROJASSIST"] = users.Where((Leqisoft.DTO.User u) => u.Role == UserRole.Assistant);
				r["CN_PROJCHECK"] = users.Where((Leqisoft.DTO.User u) => u.Role == UserRole.Checker);
			...
			else if (State == ViewState.Template || State == ViewState.RecycleTemplate)
			{
				...
				if (project.SystemBuild)
				{
					if (Leqisoft.Model.User.Current.IsSystemSupporter)
					{
						...
						value2 = users.Where((Leqisoft.DTO.User u) => u.Role == UserRole.Editor);
					}
					else
					{
						value = string.Empty;
				...
				r["CN_TMPLUSER"] = users.Where((Leqisoft.DTO.User u) => u.Role == UserRole.User);
			}
		}
	}

	private void PopulateTile(List<Leqisoft.DTO.Project> projects)
	{
		_tile.ClearSelected();
		_tile.BeginUpdate();
		_tile.Groups.Clear();
		...
		if (!(Program.MainForm.CurrentEdition is AppEditionGeneral) && (State == ViewState.Project || State == ViewState.Template))
		{
			...
			foreach (Leqisoft.DTO.Project item2 in from i in recent
	...
	private IEnumerable<Leqisoft.DTO.Project> SearchProjects(IEnumerable<Leqisoft.DTO.Project> projects)
	{
		string keyword = _txbSearch.Text;
		if (_isSearch && keyword != "")
		{
			Dictionary<Leqisoft.DTO.Project, int> source = projects.ToDictionary((Leqisoft.DTO.Project p) => p, (Leqisoft.DTO.Project p) => FuzzySearch.Filter(p.Name, keyword) + FuzzySearch.Filter(p.Number, keyword) + FuzzySearch.Filter(p.Category.Replace("|", ","), keyword) + FuzzySearch.Filter(p.Auditee, keyword));
	...
	{
		if (Leqisoft.Model.User.Current.IsTeamAdmin)
		{
	...
	private void FindAndSelectRow(Leqisoft.DTO.Project p)
	{
		...
		{
			if ((_grid.BodyGetRow(i).UserData as Leqisoft.DTO.Project)?.Id == p.Id)
			{
	...
	private async Task OpenProject()
	{
		...
		try
		{
			if (!_isInOpenProject)
			{
				...
				Task<Leqisoft.Model.Project> task = Program.MainForm.OpenOrSwitchToProject(sp.Id, willOpenProjectTypeName);
				if (task == null)
				{
					return;
				}
				Leqisoft.Model.Project project = await task;
				if (project == null)
				{
					return;
				}
				...
				if (!project.IsNeedSyncDataOnOpen)
				{
					...
					if (!Leqisoft.LocalDataStore.StorageRouter.IsLocalMode && UserTeam.CurrentTeamIsPayByProject && project != null)
					{
	...
	private async Task ModifyProject()
	{
		...
		try
		{
			ProgressForm<object> progressForm = new ProgressForm<object>(async delegate(IProgress<ProgressInfo> iProg)
			{
				...
				await Leqisoft.LocalDataStore.StorageRouter.UpdateProject(clone);
				if (!Leqisoft.LocalDataStore.StorageRouter.IsLocalMode)
				{
	...
	private async Task DeleteProject()
	{
		...
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "不能删除当前正在编辑的" + text);
		}
		else
		{
			...
			{
				if (Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.Exclamation, "警告：此操作将彻底删除所选" + text + "的数据，无法恢复，请谨慎确认此次操作！\n\n若确认要删除【" + SelectedProject.Name + "】，点击“确定”。", MessageBoxButtons.OKCancel) != DialogResult.OK)
				{
					return;
				}
				if (TokenTimer.LoginInfo != null && TokenTimer.LoginInfo.LoginMode == LoginMode.SMS)
				{
					try
					{
						if (!StorageRouter.IsLocalMode)
						{
							...
							if (text3.Trim() != text2)
							{
								Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "输入的验证码不正确，终止删除操作！");
								return;
							}
						}
					}
				...
				else
				{
					...
					{
						Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "输入的密码不正确，终止删除操作！");
						return;
					}
				}
				try
			...
			if (Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.Question, "确定要删除" + text + "吗？", MessageBoxButtons.OKCancel) == DialogResult.OK)
			{
				try
				{
				...
				{
					Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, ex4.InnerException?.Message ?? ex4.Message);
				}
				catch (Exception ex5)
				{
					Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "删除失败：" + ex5.Message);
				}
			}
		}
	}
	...
	private async Task DuplicateProject()
	{
		...
		try
		{
			...
			progressForm.ShowDialog(progressRuntimeData, async delegate
			{
				...
				{
					Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "本地模式不支持该操作");
				}
			});
	...
	private async Task ShareProject()
	{
		...
		try
		{
			...
			if (Leqisoft.LocalDataStore.StorageRouter.IsLocalMode)
			{
				Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "本地模式不支持跨组织分享。");
				return;
			}
			JObject jObj = new JObject();
			jObj["OldProject"] = id;
			...
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "跨组织分享成功。");
			await Task.Delay(1);
		}
		catch (HttpRequestException ex)
		{
	...
	private async Task SaveAsTemplate()
	{
		...
		try
		{
			...
			progressForm.ShowDialog(progressRuntimeData, async delegate
			{
				...
				await Leqisoft.LocalDataStore.StorageRouter.SaveProjectAsTemplate(id, newTemplate);
			});
	...
	private async Task ModifyTemplate()
	{
		...
		try
		{
			...
			await Leqisoft.LocalDataStore.StorageRouter.UpdateTemplate(clone);
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "修改" + StringConstBase.Current.Template + "成功");
	...
	private async Task DuplicateTemplate()
	{
		...
		try
		{
			...
			progressForm.ShowDialog(progressRuntimeData, async delegate
			{
				...
				// 远程模式：调用 WebApiClient.DuplicateProject
				await Leqisoft.LocalDataStore.StorageRouter.DuplicateTemplate(id, newTemplate);
			});
			ProjectInfoManager.GetInstance().UpdateOpenTime(newTemplate.Id.ToString(), DateTime.Now);
			await Populate();
			FindAndSelectRow(newTemplate);
	...
	private async Task RestoreProjects()
	{
		...
		List<Guid> o = SelectedProjects.Select((Leqisoft.DTO.Project p) => p.Id).ToList();
		try
		{
			JObject jObject = new JObject();
			jObject["Ids"] = JToken.FromObject(o);
	...
	{
		if (Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.Exclamation, "此操作将彻底删除所选数据，无法恢复，请谨慎确认此次操作！", MessageBoxButtons.OKCancel) != DialogResult.OK)
		{
			return;
		}
		List<Guid> o = SelectedProjects.Select((Leqisoft.DTO.Project p) => p.Id).ToList();
	...
	{
		if (Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.Exclamation, "此操作将彻底删除回收站数据，无法恢复，请谨慎确认此次操作！", MessageBoxButtons.OKCancel) != DialogResult.OK)
		{
			return;
		}
		List<Guid> o = _projects.Select((Leqisoft.DTO.Project p) => p.Id).ToList();
		try
		{
			JObject jobj = new JObject();
			jobj["Ids"] = JToken.FromObject(o);
	...
	private void _grid_BodyOwnerDrawCell(object sender, OwnerDrawCellEventArgs e)
	{
		try
		{
			if (State == ViewState.Project || State == ViewState.RecycleProject)
			{
				...
				{
					IEnumerable<Leqisoft.DTO.User> source = (IEnumerable<Leqisoft.DTO.User>)_grid.BodyGetData(e.Row, e.Col);
			...
			else
			{
				...
				{
					IEnumerable<Leqisoft.DTO.User> enumerable = (IEnumerable<Leqisoft.DTO.User>)_grid.BodyGetData(e.Row, e.Col);
					if (enumerable == null)
					{
						e.Text = string.Empty;
						return;
				...
				else
				{
					...
					Leqisoft.DTO.Project project = _grid.BodyGetRow(e.Row).UserData as Leqisoft.DTO.Project;
					if (project.TeamVisible)
					{
						e.Text = "(所有同事)";
						return;
					}
					IEnumerable<Leqisoft.DTO.User> source2 = (IEnumerable<Leqisoft.DTO.User>)_grid.BodyGetData(e.Row, e.Col);
