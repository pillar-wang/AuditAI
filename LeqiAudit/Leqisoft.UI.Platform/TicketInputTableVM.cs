...
public class TicketInputTableVM
{
	...
	protected class TicketInputDataResolver : FormulaReferenceTicketInputDataResolver
	{
		...
		public override Leqisoft.Model.Cell GetTicketTitleCell(int row, int col)
		{
			...
			{
				return new Leqisoft.Model.Cell
				{
					Value = cellVM.Value,
					Row = _parent.DummyMR
				};
		...
		public override Leqisoft.Model.Cell GetTicketFooterCell(int row, int col)
		{
			TicketInputCellVM cellVM;
			try
			{
	...
	private readonly Leqisoft.Model.Row DummyMR = new Leqisoft.Model.Row
	{
		Creator = Leqisoft.Model.User.Current.Id
	};

	private readonly Leqisoft.Model.Column DummyColumn;

	private readonly bool _hasFillingFormula;

	protected HashSet<Leqisoft.Model.Row> DynamicRowRefTableDataRows = new HashSet<Leqisoft.Model.Row>();
	...
	protected Dictionary<int, List<Leqisoft.Model.Row>> MixRangeDataRowGroupDic;

	protected Dictionary<int, TicketInputRowVM> MixRangeTemplateRowPreFixedRowDic;

	protected HashSet<Leqisoft.Model.Row> MixTicketDataRowsRefTableRows = new HashSet<Leqisoft.Model.Row>();
	...
	protected List<Leqisoft.Model.Row> _newAddedTableRowsList;
	...
	public HashSet<Leqisoft.Model.Row> RemovedRows { get; } = new HashSet<Leqisoft.Model.Row>();
	...
	public Leqisoft.Model.Table Table => _ticket.Table;
	...
	protected Leqisoft.Model.Row DynamicRowKeyCellsRefEmptyTableRow { get; private set; }
	...
	protected Leqisoft.Model.Row MixTicketKeyCellsRefEmptyTableRow { get; private set; }
	...
	public TicketInputTableVM(TicketTable ticket, TicketRecord record)
		: this(ticket, record, ticket.Table.Columns.Where((Leqisoft.Model.Column c) => c.HasFormula).Any(delegate(Leqisoft.Model.Column c)
		{
	...
	public TicketInputTableVM(TicketTable ticket, TicketRecord record, bool isExistFillingColumnFormula, bool isCalculateTicket = true)
	{
		...
		DummyColumn = Leqisoft.Model.Column.CreateDummyColumn(ticket.Table);
			...
			for (int l = 0; l < ticket.Columns.Count; l++)
			{
				...
				if (cell.HasField())
				{
					...
					if (ticketInputCellVM.Column != null)
					{
						...
						if (record == null)
						{
							ticketInputCellVM.TempCell = new Leqisoft.Model.Cell
							{
						...
						{
							Leqisoft.Model.Cell cell4 = (ticketInputCellVM.TableCell = (ticketInputCellVM.TempCell = Table[record.Rows[0].Index, ticketInputCellVM.Column.Index]));
		...
		{
			HashSet<Leqisoft.Model.Column> _keyColumns = new HashSet<Leqisoft.Model.Column>(from c in ticket.Cells
				where c.HasText() && c.HasField()
				select ticket.GetFieldColumn(c.Field) into c
				where c != null
				select c);
			HashSet<Leqisoft.Model.Column> source = new HashSet<Leqisoft.Model.Column>(from c in ticket.Cells
				where c.HasField()
			...
			FixedCells.AddRange(from c in _cells
				...
				where g.Count() == 1
				select g.Single());
			keyCellsList = _keyColumns.Select((Leqisoft.Model.Column kc) => new
			{
				Column = kc,
			...
			var list = source.Select((Leqisoft.Model.Column vc) => new
			{
				Column = vc,
			...
			for (i = 0; i < num; i++)
			{
				...
				Leqisoft.Model.Row row3 = (ticketFixedMultiRowVM.Row = record?.Rows.FirstOrDefault((Leqisoft.Model.Row r) => RowMatches(r)));
				foreach (var item4 in keyCellsList)
				{
					...
					if (row3 == null)
					{
						ticketInputCellVM2.TempCell = new Leqisoft.Model.Cell
						{
							Column = item4.Column,
							Row = DummyMR,
					...
					{
						Leqisoft.Model.Cell tableCell = (ticketInputCellVM2.TempCell = Table[row3.Index, item4.Column.Index]);
				...
				foreach (var item5 in list)
				{
					...
					if (row3 == null)
					{
						ticketInputCellVM3.TempCell = new Leqisoft.Model.Cell
						{
							Column = item5.Column,
							Row = DummyMR,
							Value = ""
					...
					{
						Leqisoft.Model.Cell cell6 = Table[row3.Index, item5.Column.Index];
			...
			if (ticket.Table.RowOwnerExclusive && _record != null && FixedMultiRowVMs.Where((TicketFixedMultiRowVM r) => r.Row != null).All((TicketFixedMultiRowVM r) => r.Row.Creator != Leqisoft.Model.User.Current.Id))
			{
				DummyMR.Creator = 0L;
			}
		}
		else if (ticket.Kind == TicketKind.DynamicRow)
		{
			...
			else
			{
				...
				{
					Leqisoft.Model.Row row4 = record.Rows[m];
					...
					for (int n = 0; n < ticket.Columns.Count; n++)
					{
						...
						if (ticketColumn2.HasField())
						{
							...
							{
								Leqisoft.Model.Cell cell9 = (ticketInputCellVM4.TableCell = (ticketInputCellVM4.TempCell = Table[row4.Index, ticketInputCellVM4.Column.Index]));
						...
						{
							ticketInputCellVM4.TempCell = new Leqisoft.Model.Cell
							{
			...
			for (int num2 = 0; num2 < ((record == null) ? DataRowsCount : (DataRowsCount - record.Rows.Count)); num2++)
			{
				...
				for (int num3 = 0; num3 < ticket.Columns.Count; num3++)
				{
					...
					if (ticketColumn3.HasField())
					{
						...
						{
							ticketInputCellVM5.TempCell = new Leqisoft.Model.Cell
							{
		...
		bool RowMatches(Leqisoft.Model.Row row)
		{
	...
	private void InitFixedDataRowMixDynamicDataRowTicket(bool isCalculateTicket)
	{
		...
		{
			foreach (Leqisoft.Model.Row row3 in _record.Rows)
			{
			...
			for (int k = 0; k < _ticket.Columns.Count; k++)
			{
				...
				if (cell.HasField())
				{
					...
					if (ticketInputCellVM.Column != null)
					{
						...
						{
							Leqisoft.Model.Cell cell4 = (ticketInputCellVM.TableCell = (ticketInputCellVM.TempCell = Table[row.Index, ticketInputCellVM.Column.Index]));
							if (cell4 == null) continue;
							ticketInputCellVM.Value = cell4.Value;
							if (Table.CellPropManager.TryGetAttachments(ticketInputCellVM.TableCell, out var attachments))
							{
		...
		foreach (TicketTableMixRangeTemplateRow dynamicDataRowTemplateRow2 in _ticket.FixedAndDynamicMixRange.DynamicDataRowTemplateRows)
		{
			...
			{
				value2 = new List<Leqisoft.Model.Row>();
			}
			...
			{
				Leqisoft.Model.Row row2 = value2[l];
				...
				for (int m = 0; m < _ticket.Columns.Count; m++)
				{
					...
					if (cell5.HasField())
					{
						...
						if (ticketInputCellVM2.Column != null)
						{
							...
							Leqisoft.Model.Cell cell8 = (ticketInputCellVM2.TableCell = (ticketInputCellVM2.TempCell = Table[row2.Index, ticketInputCellVM2.Column.Index]));
	...
	private void RecordTableRowsToNewAddRow(int startIndex, int count)
	{
		...
		{
			_newAddedTableRowsList = new List<Leqisoft.Model.Row>();
		}
		if (Table == null)
		{
			return;
	...
	private void RecordTableRowsToNewAddRow(Leqisoft.Model.Row row)
	{
		if (_isNeedRecordNewAddedTableRow)
		{
			if (_newAddedTableRowsList == null)
	...
	{
		MixRangeDataRowGroupDic = new Dictionary<int, List<Leqisoft.Model.Row>>();
		...
		List<Leqisoft.Model.Row> list = new List<Leqisoft.Model.Row>(_record.Rows);
		if (_ticket.FixedAndDynamicMixRange.DataGroupKeyListForFixedDataRow != null)
		{
			...
			{
				List<Leqisoft.Model.Row> list2 = GetDataKeyMatchedRows(item, list, isOnlyGetFirstRow: true);
				if (list2.Count > 0)
				{
		...
		foreach (TicketTableMixRangeDataGroupKey item2 in _ticket.FixedAndDynamicMixRange.DataGroupKeyListForDynamicDataRow)
		{
			...
			{
				MixRangeDataRowGroupDic[item2.KeyId] = new List<Leqisoft.Model.Row>(list.Where((Leqisoft.Model.Row u) => u != null));
				list = new List<Leqisoft.Model.Row>();
				break;
			}
			List<Leqisoft.Model.Row> list3 = GetDataKeyMatchedRows(item2, list, isOnlyGetFirstRow: false);
			if (list3.Count > 0)
			{
				MixRangeDataRowGroupDic[item2.KeyId] = list3;
			}
		}
		List<Leqisoft.Model.Row> GetDataKeyMatchedRows(TicketTableMixRangeDataGroupKey dataGroupKey, List<Leqisoft.Model.Row> rowsList, bool isOnlyGetFirstRow)
		{
			List<Tuple<Leqisoft.Model.Column, ValueOperand>> list4 = new List<Tuple<Leqisoft.Model.Column, ValueOperand>>();
			foreach (TicketTableMixRangeDataGroupKeyItem keyItem in dataGroupKey.KeyItems)
			{
				Leqisoft.Model.Column byId = _ticket.Table.Columns.GetById(keyItem.TableColumnId);
				if (byId == null)
				{
					return new List<Leqisoft.Model.Row>();
				}
				list4.Add(Tuple.Create(byId, Leqisoft.Model.Cell.ChangeToValueOperand(keyItem.TableColumnValue, byId.GetDataType())));
			}
			if (list4.Count == 0)
			{
				return new List<Leqisoft.Model.Row>();
			}
			List<Leqisoft.Model.Row> list5 = new List<Leqisoft.Model.Row>();
			for (int i = 0; i < rowsList.Count; i++)
			{
				Leqisoft.Model.Row row = rowsList[i];
				if (row != null)
				{
					bool flag = true;
					foreach (Tuple<Leqisoft.Model.Column, ValueOperand> item3 in list4)
					{
	...
	public void InitCombolistForNewRecord(Dictionary<Id64, string> initValue = null)
	{
		...
		{
			UpdateTicketCellValue(newValue: (item5.Item1.TableCell == null) ? item5.Item4 : Leqisoft.Model.Cell.ChangeDataTypeImpl(item5.Item4, item5.Item1.TableCell.DisplayDataType), ticketCell: item5.Item1, isFormulaExistManualInputValue: true);
		}
		foreach (Tuple<TicketInputCellVM, int, int, string> item6 in list2)
		{
			UpdateTicketCellValue(newValue: (item6.Item1.TableCell == null) ? item6.Item4 : Leqisoft.Model.Cell.ChangeDataTypeImpl(item6.Item4, item6.Item1.TableCell.DisplayDataType), ticketCell: item6.Item1, isFormulaExistManualInputValue: true);
		}
		EndBatchUpdateValue();
					...
					for (int l = 0; l < _columns.Count; l++)
					{
						...
						if (cellVM2.Column != null && cellVM2.IsFormula && !cellVM2.IsFormulaFromTicket && !(cellVM2.Formula != cellVM2.Column.Formula) && !cellVM2.Column.IsFormulaRefSelfTableColumnOrCell)
						{
							try
							{
								...
								object obj3 = Leqisoft.Model.Cell.ChangeDataTypeImpl(value2, cellVM2.Column.GetDataType());
								string displayValueImpl2 = Leqisoft.Model.Cell.GetDisplayValueImpl(value2, cellVM2.Column.GetFormat());
					...
					for (int j = 0; j < target.Columns.Count; j++)
					{
						...
						if (cellVM.Column != null && cellVM.IsFormula && !cellVM.IsFormulaFromTicket && !(cellVM.Formula != cellVM.Column.Formula) && !cellVM.Column.IsFormulaRefSelfTableColumnOrCell)
						{
							try
							{
								...
								object obj = Leqisoft.Model.Cell.ChangeDataTypeImpl(value, cellVM.Column.GetDataType());
								string displayValueImpl = Leqisoft.Model.Cell.GetDisplayValueImpl(value, cellVM.Column.GetFormat());
	...
	public int GetMixTicketColumnWhichRefTableColumn(int rowIndex, Leqisoft.Model.Column tableColumn)
	{
		if (tableColumn == null)
		{
			return -1;
	...
	private void BindTableRowsToTicketDataRow_DynamicRow(int ticketStartIndex, int tableRowStartIndex, int count)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			Leqisoft.Model.Row row = Table.Rows[index];
			foreach (TicketInputCellVM dynamicRowKeyCell in DynamicRowKeyCells)
			{
				Leqisoft.Model.Cell tableCell = Table[row.Index, dynamicRowKeyCell.Column.Index];
	...
	{
		Leqisoft.Model.Row row = Table.Rows[tableRowIndex];
		foreach (TicketInputCellVM mixTicketKeyCell in MixTicketKeyCells)
		{
			Leqisoft.Model.Cell tableCell = Table[row.Index, mixTicketKeyCell.Column.Index];
			bool isFormulaExistManualInputValue = false;
		...
		for (int i = 0; i < _ticket.Columns.Count; i++)
		{
			...
			if (cellVM.IsMixTicketExistDesignInputValue)
			{
				...
				if (column != null)
				{
					Leqisoft.Model.Cell tableCell2 = Table[row.Index, column.Index];
					string inputValue = cellVM.TicketCell.GetInputValue();
					object value = Leqisoft.Model.Cell.ChangeDataTypeImpl(inputValue, column.GetDataType());
	...
	private void BuildTableCellForTicketKeyCell_DynamicRow(TicketInputCellVM cell)
	{
		...
		Table.Rows.Append(1);
		Leqisoft.Model.Row row = Table.Rows[Table.Rows.Count - 1];
	...
	private void BuildTableCellForTicketCell_DynamicRow(int cellRowIndex, int cellColIndex)
	{
		...
		else
		{
			...
			{
				Leqisoft.Model.Cell tableCell = Table[row.Index, dynamicRowKeyCell2.Column.Index];
				bool isFormulaExistManualInputValue = false;
	...
	private void BuildTableCellForTicketCell_MixTicket(int cellRowIndex, int cellColIndex)
	{
		...
		else
		{
			...
			foreach (TicketInputCellVM mixTicketKeyCell2 in MixTicketKeyCells)
			{
				...
				Leqisoft.Model.Cell tableCell = Table[row.Index, mixTicketKeyCell2.Column.Index];
				if (tableCell == null) continue;
				bool isFormulaExistManualInputValue = false;
			...
			if (ticketRow.IsMixTicketFixedDataRow)
			{
				for (int i = 0; i < _ticket.Columns.Count; i++)
				{
					...
					{
						Leqisoft.Model.Column column = cellVM2.Column;
						if (column != null)
						{
							Leqisoft.Model.Cell tableCell2 = Table[row.Index, column.Index];
			...
			else if (ticketRow.IsMixTicketDynamicDataRow)
			{
				...
				if (ticketTableMixRangeTemplateRow != null)
				{
					for (int j = 0; j < _ticket.Columns.Count; j++)
					{
						...
						{
							Leqisoft.Model.Column column2 = cellVM3.Column;
							if (column2 != null)
							{
								Leqisoft.Model.Cell tableCell3 = Table[row.Index, column2.Index];
								string inputValue2 = cellVM3.TicketCell.GetInputValue();
								object value2 = Leqisoft.Model.Cell.ChangeDataTypeImpl(inputValue2, column2.GetDataType());
	...
	private Leqisoft.Model.Cell BindTableNewCreatedCellToTicketCell(TicketInputCellVM ticketCell, Leqisoft.Model.Row tableRow)
	{
	...
	private void BuildTableCellForAllTicketCell_FixedMultiRow()
	{
		...
		Queue<Leqisoft.Model.Row> queue = new Queue<Leqisoft.Model.Row>();
		...
		foreach (TicketFixedMultiRowVM fixedMultiRowVM in FixedMultiRowVMs)
		{
			...
			Leqisoft.Model.Row row3 = (fixedMultiRowVM.Row = queue.Dequeue());
			if (row == null)
			{
				row = row3;
			}
			...
			{
				Leqisoft.Model.Cell tableCell = BindTableNewCreatedCellToTicketCell(keyCell, row3);
			...
			{
				Leqisoft.Model.Cell tableCell2 = Table[row3.Index, item.Column.Index];
				bool isFormulaExistManualInputValue = false;
		...
		Leqisoft.Model.Row row4 = FixedMultiRowVMs[0].Row;
		foreach (TicketInputCellVM item2 in list)
		{
			if (item2.TableRow == null)
			{
				Leqisoft.Model.Cell tableCell3 = (item2.TempCell = Table[row4.Index, item2.Column.Index]);
	...
	private void UpdateTableCellValue(Leqisoft.Model.Cell tableCell, object value, bool isFormulaExistManualInputValue)
	{
	...
	private void UpdateTicketCellValue_FixedMultiRow(TicketInputCellVM ticketCell, object newValue, bool isFormulaExistManualInputValue)
	{
		...
		if (ticketCell.IsFixedMultiFixedCell)
		{
			...
			foreach (TicketFixedMultiRowVM fixedMultiRowVM in FixedMultiRowVMs)
			{
				if (fixedMultiRowVM.Row != null)
				{
					Leqisoft.Model.Cell tableCell = Table[fixedMultiRowVM.Row.Index, ticketCell.Column.Index];
	...
	protected void UpdateTicketCellValue_DynamicRow(TicketInputCellVM ticketCell, object value, bool isFormulaExistManualInputValue)
	{
		...
		if (ticketCell.IsDynamicRowKeyCell)
		{
			BeginBatchUpdateValue();
			bool flag = false;
			foreach (Leqisoft.Model.Row dynamicRowRefTableDataRow in DynamicRowRefTableDataRows)
			{
				Leqisoft.Model.Cell cell = Table[dynamicRowRefTableDataRow.Index, ticketCell.Column.Index];
			...
			{
				Leqisoft.Model.Cell tableCell = Table[DynamicRowKeyCellsRefEmptyTableRow.Index, ticketCell.Column.Index];
	...
	protected void UpdateTicketCellValue_MixTicket(TicketInputCellVM ticketCell, object value, bool isFormulaExistManualInputValue)
	{
		...
		if (ticketCell.IsMixTicketKeyCell)
		{
			BeginBatchUpdateValue();
			bool flag = false;
			foreach (Leqisoft.Model.Row mixTicketDataRowsRefTableRow in MixTicketDataRowsRefTableRows)
			{
				Leqisoft.Model.Cell cell = Table[mixTicketDataRowsRefTableRow.Index, ticketCell.Column.Index];
				UpdateTableCellValue(cell, value, isFormulaExistManualInputValue);
				if (cell == ticketCell.TableCell)
				{
					flag = true;
			...
			{
				Leqisoft.Model.Cell tableCell = Table[MixTicketKeyCellsRefEmptyTableRow.Index, ticketCell.Column.Index];
				UpdateTableCellValue(tableCell, value, isFormulaExistManualInputValue);
			}
			if (!flag)
			{
	...
	private void ExecuteNewAddTableRowsFormula()
	{
		...
		try
		{
			List<Leqisoft.Model.Column> list = null;
			foreach (Leqisoft.Model.Column column in Table.Columns)
			{
				if (column.HasFormula && !column.IsFormulaRefSelfTableColumnOrCell)
				{
					if (list == null)
					{
						list = new List<Leqisoft.Model.Column>();
					}
					list.Add(column);
				}
			}
			...
			foreach (Leqisoft.Model.Row newAddedTableRows in _newAddedTableRowsList)
			{
			...
			foreach (Leqisoft.Model.Column item in list)
			{
				try
				{
	...
	protected TicketInputRowVM InsertDataRow_DynamicRowTicket(int index)
	{
		TicketInputRowVM ticketInputRowVM = new TicketInputRowVM
		{
			TempRow = new Leqisoft.Model.Row
			{
				Creator = Leqisoft.Model.User.Current.Id
			},
			IsNew = true,
	...
	public int MoveUpDataRow_DynamicRowTicket(int vmStartRowIndex, int rowsCount, out int afterMoveFistRowIndex)
	{
		...
		{
			Leqisoft.Model.Row tableRow = _rows[num5].TableRow;
			if (tableRow != null)
			{
				row2 = tableRow;
				break;
	...
	public int MoveDownDataRow_DynamicRowTicket(int vmStartRowIndex, int rowsCount, out int afterMoveFistRowIndex)
	{
		...
		{
			Leqisoft.Model.Row tableRow = _rows[j].TableRow;
			if (tableRow != null)
			{
				row2 = tableRow;
				break;
	...
	public int MoveDataRowToTop_DynamicRowTicket(int vmStartRowIndex, int rowsCount, out int afterMoveFistRowIndex)
	{
		...
		List<Leqisoft.Model.Row> list = new List<Leqisoft.Model.Row>();
		for (int k = 0; k < rowsCount; k++)
		{
			Leqisoft.Model.Row tableRow2 = _rows[num3 + k].TableRow;
			if (tableRow2 != null)
			{
				list.Add(tableRow2);
			}
		...
		if (row != null)
		{
			...
			{
				Leqisoft.Model.Row row2 = list[num6];
				if (row2 != row)
				{
	...
	public int MoveDataRowToBottom_DynamicRowTicket(int vmStartRowIndex, int rowsCount, out int afterMoveFistRowIndex)
	{
		...
		{
			Leqisoft.Model.Row tableRow = _rows[num6].TableRow;
			if (tableRow != null)
			{
				row = tableRow;
				break;
		...
		{
			Leqisoft.Model.Row tableRow2 = _rows[num4 + j].TableRow;
			if (tableRow2 != null)
			{
				list.Add(tableRow2);
			}
		...
		if (flag)
		{
			...
			{
				Leqisoft.Model.Row row2 = list[n];
				if (row2 != row)
				{
		...
		else if (row != null)
		{
			...
			{
				Leqisoft.Model.Row row3 = list[num9];
				if (row3 != row)
				{
	...
	public int MoveDataRowToTop_MixTicket(int vmStartRowIndex, int rowsCount, out int afterMoveFistRowIndex)
	{
		...
		for (int k = 0; k < rowsCount; k++)
		{
			Leqisoft.Model.Row tableRow2 = _rows[num4 + k].TableRow;
			if (tableRow2 != null)
			{
				list.Add(tableRow2);
			}
		...
		if (row != null)
		{
			...
			{
				Leqisoft.Model.Row row2 = list[num7];
				if (row2 != row)
				{
					Table.Rows.Move(row2.Index, 1, row.Index);
				}
	...
	public int MoveDataRowToBottom_MixTicket(int vmStartRowIndex, int rowsCount, out int afterMoveFistRowIndex)
	{
		...
		{
			Leqisoft.Model.Row tableRow = _rows[num7].TableRow;
			if (tableRow != null)
			{
				row = tableRow;
				break;
		...
		for (int j = 0; j < rowsCount; j++)
		{
			Leqisoft.Model.Row tableRow2 = _rows[num5 + j].TableRow;
			if (tableRow2 != null)
			{
				list.Add(tableRow2);
			}
		...
		else if (row != null)
		{
			...
			{
				Leqisoft.Model.Row row3 = list[num10];
				if (row3 != row)
				{
					Table.Rows.Move(row3.Index, 1, row.Index);
				}
	...
	public HashSet<Leqisoft.Model.Column> GetNavColumns()
	{
		HashSet<Leqisoft.Model.Column> hashSet = new HashSet<Leqisoft.Model.Column>();
		foreach (TicketInputCellVM cell in _cells)
		{
	...
	private FormulaEvaluationEnvironment GetEvalEnv(int row, TicketInputCellVM cell)
	{
		...
		FormulaEvaluationEnvironment formulaEvaluationEnvironment = new FormulaEvaluationEnvironment
		{
			...
			CurrentUserId = Leqisoft.Model.User.Current.Id,
			RefEvalContext = new DataReferenceEvaluationContext
			{
				Project = Table.Project,
				CurrentTreeNode = Table.TreeNode
	...
	private void FindOutTicketFormulaShouldUpdateTableCell(TicketInputCellVM ticketCell, Dictionary<Leqisoft.Model.Cell, TicketInputCellVM> tableNeedUpdateCellDic)
	{
	...
	public void CalculateTicket(bool forceCalculate = false)
	{
		...
		Dictionary<Leqisoft.Model.Cell, TicketInputCellVM> tableNeedUpdateCellDic = new Dictionary<Leqisoft.Model.Cell, TicketInputCellVM>();
		int num = 0;
		bool anyChanged;
		do
		{
			...
			{
				BeginBatchUpdateValue();
				foreach (Leqisoft.Model.Cell key in tableNeedUpdateCellDic.Keys)
				{
		...
		return new TicketEvalContext
		{
			ResolveColumnWildcard = delegate(Id64 colId, int row, int ticketdatarow)
			{
				...
				if (ticketInputCellVM == null)
				{
					...
					return (Leqisoft.Model.Cell)null;
				}
			...
			ResolveTicketCell = delegate(int row, int col)
			{
				...
				return (cellVM4.Column == null || cellVM4.TempCell == null) ? new Leqisoft.Model.Cell
				{
					Value = cellVM4.Value,
					Row = DummyMR,
					Column = DummyColumn
			...
			{
				List<Leqisoft.Model.Cell> list2 = new List<Leqisoft.Model.Cell>();
				int num = 0;
				if (_ticket.Kind == TicketKind.DynamicRow)
				{
				...
				for (int k = r1; k <= r2; k++)
				{
					for (int l = c1; l <= c2; l++)
					{
						...
						{
							list2.Add(new Leqisoft.Model.Cell
							{
								Value = cellVM3.Value,
								Row = DummyMR,
								Column = DummyColumn
			...
			{
				List<Leqisoft.Model.Cell> list = new List<Leqisoft.Model.Cell>();
				int value;
				if (_ticket.Kind == TicketKind.DynamicRow)
				{
	...
	public bool ChangeEmptyTableRowToAvailableDynamicDataRow_MixTicket(Leqisoft.Model.Row tableRow)
	{
		...
		for (int i = 0; i < _ticket.Columns.Count; i++)
		{
			...
			{
				Leqisoft.Model.Column byId = Table.Columns.GetById(cell.Field);
				if (byId != null)
				{
					result = true;
					Leqisoft.Model.Cell tableCell = Table[tableRow.Index, byId.Index];
					object value = Leqisoft.Model.Cell.ChangeDataTypeImpl(inputValue, byId.GetDataType());
	...
	public Leqisoft.Model.Row GetKeyCellsRefTableRow_MixTicket()
	{
	...
	public bool IsTableRowDataColumnEmpty_MixTicket(Leqisoft.Model.Row mr)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			if (byId != null)
			{
				Leqisoft.Model.Cell cell2 = Table[mr.Index, byId.Index];
				if (!cell2.IsEmpty)
				{
					return false;
				}
	...
	public bool IsTableRowDataColumnEmpty_OnlyCheckAutoFillColumn_MixTicket(Leqisoft.Model.Row mr)
	{
		if (Table.Ticket.FixedAndDynamicMixRange.DynamicDataRowTemplateRows.Count == 0)
		{
			return true;
	...
	public bool IsTableRowKeyColumnEmpty_MixTicket(Leqisoft.Model.Row mr)
	{
		foreach (TicketInputCellVM mixTicketKeyCell in MixTicketKeyCells)
		{
			Leqisoft.Model.Column column = mixTicketKeyCell.Column;
			if (column != null)
			{
				Leqisoft.Model.Cell cell = Table[mr.Index, column.Index];
				if (!cell.IsEmpty)
				{
					return false;
				}
	...
	public Leqisoft.Model.Row GetKeyCellsRefTableRow_DynamicRow()
	{
	...
	public bool IsTableRowDataColumnEmpty_DynamicRowTicket(Leqisoft.Model.Row mr)
	{
		int count = _columns.Count;
		for (int i = 0; i < count; i++)
		{
			...
			{
				Leqisoft.Model.Cell cell = Table[mr.Index, ticketInputColumnVM.TableColumn.Index];
				if (!cell.IsEmpty)
				{
					return false;
				}
	...
	public bool IsTableRowKeyColumnEmpty_DynamicRowTicket(Leqisoft.Model.Row mr)
	{
		foreach (TicketInputCellVM dynamicRowKeyCell in DynamicRowKeyCells)
		{
			Leqisoft.Model.Column column = dynamicRowKeyCell.Column;
			if (column != null)
			{
				Leqisoft.Model.Cell cell = Table[mr.Index, column.Index];
				if (!cell.IsEmpty)
	...
	public Leqisoft.Model.Row GetAnyOrDefaultTableRow()
	{
		for (int i = 0; i < _rows.Count; i++)
		{
			for (int j = 0; j < _columns.Count; j++)
