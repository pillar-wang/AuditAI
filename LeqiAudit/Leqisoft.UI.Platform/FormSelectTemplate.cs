...
public class FormSelectTemplate
{
	...
	private readonly List<Leqisoft.DTO.Project> _templates = new List<Leqisoft.DTO.Project>();

	public Leqisoft.DTO.Project ResultTemplate { get; private set; }
	...
	private async Task<IEnumerable<Leqisoft.DTO.Project>> GetCurrentPlatformTemplates()
	{
		List<Leqisoft.DTO.Project> list = (await GetCurrentPlatformTemplatesImpl()).ToList();
		list.Sort(delegate(Leqisoft.DTO.Project left, Leqisoft.DTO.Project right)
		{
	...
	private async Task<IEnumerable<Leqisoft.DTO.Project>> GetCurrentPlatformTemplatesImpl()
	{
		if (!StorageRouter.IsLocalMode)
		{
			IEnumerable<Leqisoft.DTO.Project> enumerable = await WebApiClient.GetTemplates();
			...
			{
				return enumerable.Where((Leqisoft.DTO.Project u) => u.SystemBuild);
			}
	...
	private async Task Populate()
	{
		...
		_templates.Clear();
		List<Leqisoft.DTO.Project> templates = _templates;
		...
		foreach (Leqisoft.DTO.Project template in _templates)
		{
			Tile tile = new Tile
			{
				HorizontalSize = 1,
	...
	private async void _form_Load(object sender, EventArgs e)
	{
		...
		_form.Icon = Theme.SelectedLeqiTheme.GetThemedIcon(Resources.Templates);
