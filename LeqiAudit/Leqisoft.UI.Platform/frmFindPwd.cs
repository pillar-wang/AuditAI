...
public class frmFindPwd : Form
{
	...
	{
		_leqiMainColor = PlatformColorManager.GetMainColorButton(Program.ClientPlatformType);
		btnGetVerification.BackColor = _leqiMainColor;
		btnFindPwd.BackColor = _leqiMainColor;
	}
	...
	private async void btnFindPwd_Click(object sender, EventArgs e)
	{
		try
		{
			...
			{
				Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "新密码长度为6-20位字符！");
				return;
			}
	...
	private async Task<bool> UserNameValidate()
	{
		if (txtUserName.Text.Length == 0)
		{
			...
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "用户名不能为空");
			return false;
		}
		try
		{
			...
			catch (HttpRequestException ex)
			{
				...
				Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, message.Contains("用户不存在") ? "该账号未捆绑手机号，无法重置密码。" : message);
				return false;
			}
			if (!Regex.IsMatch(text, "^[0-9]{11,11}$"))
			{
				txtPhone.Text = string.Empty;
				Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "您未绑定手机号，无法重置密码。请联系客服人员");
				return false;
			}
	...
	private void InitializeComponent()
	{
		...
		this.txtPhoneValidate = new Leqisoft.UI.Controls.C1TextBoxEx();
		...
		this.txtNewPassword = new Leqisoft.UI.Controls.C1TextBoxEx();
		this.txtUserName = new Leqisoft.UI.Controls.C1TextBoxEx();
		this.c1Label1 = new C1.Win.C1Input.C1Label();
		this.txtPhone = new Leqisoft.UI.Controls.C1TextBoxEx();
		this.btnGetVerification = new Leqisoft.UI.Controls.TimerButton();
