...
public class ProjectUsersTileSelector
{
	...
	{
		public Leqisoft.DTO.User User { get; set; }

		public UserRole? UserRole { get; set; }

		public TileTag(Leqisoft.DTO.User user)
		{
			User = user;
		}
	}
	...
	private bool IsUserVisible(Leqisoft.DTO.User u)
	{
		return Context.UserViewStates.Find((Tuple<Leqisoft.DTO.User, bool> tup) => tup.Item1.Id == u.Id).Item2;
	}
	...
	public void PopulateUsers()
	{
		...
		foreach (Leqisoft.DTO.User rootUser in Context.RootUsers)
		{
		...
		void AppendGroup(UserGroup ug)
		{
			...
			foreach (Leqisoft.DTO.User user3 in ug.Users)
			{
		...
		Tile createUserTile(Leqisoft.DTO.User user)
		{
			...
			Tile tile = new Tile
			{
				...
				Image1 = Leqisoft.UI.Controls.Util.GetHeadPic(user, 32, withManagerMark: false),
				Text1 = user.Name,
				Text2 = null,
				Tag = tileTag,
				HorizontalSize = 1,
			...
			Leqisoft.DTO.User user2 = Context.Project.Users.FirstOrDefault((Leqisoft.DTO.User u) => u.Id == user.Id);
			if (user2 != null)
			{
	...
	public void SetTheme()
	{
		...
		_tileControl.CustomBorderColor = Theme.SelectedLeqiTheme.ThemeContext.DarkColor;
		_tileControl.HotBorderColor = Theme.SelectedLeqiTheme.ThemeContext.DarkColor;
	}

	public IEnumerable<Leqisoft.DTO.User> ValidateAndGetUsers()
	{
		List<Leqisoft.DTO.User> list = new List<Leqisoft.DTO.User>();
		foreach (Tile selectedTile in _tileControl.SelectedTiles)
		{
			if (selectedTile.Tag is TileTag { UserRole: var userRole } tileTag)
			{
				...
				{
					Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "用户角色不能为空");
					return null;
				}
		...
		if (list.Select((Leqisoft.DTO.User u) => u.UserName).Distinct().Count() != list.Count)
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "用户名不允许重复");
			return null;
		}
		if (!list.Any((Leqisoft.DTO.User u) => u.Role == UserRole.Manager))
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "成员至少要包含一名" + StringConstBase.Current.Manager);
			return null;
		}
		return list;
	}
