...
public class TicketInputEditor2 : ISetTheme
{
	...
	protected class CollectFormulaDependSettingData
	{
		...
		public Leqisoft.Model.Cell FormulaComeFromWhichTableCell;

		public Leqisoft.Model.Column FormulaComeFromWhichTableColumn;

		public HashSet<Leqisoft.Model.Column> DependTableColumnSet;

		public HashSet<Leqisoft.Model.Cell> DependTableCellSet;
	}
	...
	protected class TicketInputGridFilterContext : FilterContext
	{
		...
		public override FilterValue GetData(int row, int col)
		{
			...
			object value2 = Leqisoft.Model.Cell.ChangeDataTypeImpl(value, t);
		...
		public override FilterValue GetDataFilterOnColumnHeader(int col)
		{
			...
			object value = Leqisoft.Model.Cell.ChangeDataTypeImpl(obj, type);
			return FilterValue.FromObject(value, Leqisoft.Model.Cell.GetDisplayValueImpl(value, df));
		}
	...
	protected class ShowColumnsSelector
	{
		...
		public DialogResult ShowDialog(TicketTable ticket)
		{
			...
			for (int i = 0; i < ticket.Columns.Count; i++)
			{
				...
				{
					text = Leqisoft.Model.Column.GetExcelColumnName(i);
				}
	...
	protected class TicketColumnLedgerCollectFormulaSetting
	{
		...
		public Leqisoft.Model.Column TableColumn;
	...
	private Leqisoft.Model.Row _recordFirstTableRow;
	...
	private void SetCurrentRecord()
	{
		...
		{
			TicketRecord ticketRecord2 = Ticket.Records.Find((TicketRecord r) => r.Rows.Any((Leqisoft.Model.Row u) => u.Id.Value == _wantPopulatedRecordContainsRowId));
		...
		if (Record != null && Record.Rows != null && Record.Rows.Count > 0)
		{
			...
			_isTicketLocked = Record.Rows.Any((Leqisoft.Model.Row u) => !Table.IsControlFormulaAllowEditRow(u));
		}
		else
		{
	...
	public void SaveCurrentSelectdRecordByRowId()
	{
		...
		foreach (Leqisoft.Model.Row row in selectedRecord.Rows)
		{
	...
	private void _radioButtonOpenTicketView_CheckedChanged(object sender, EventArgs e)
	{
		if (!_isSuspendViewChangeModeCheckEvent && _radioButtonOpenTicketView.Checked)
		{
			if (Program.MainForm.CurrentView == MainFormView.TablePreview)
			{
				...
				Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "请退出预览模式后再进行切换！");
			}
			else
			{
	...
	private async void Cmd_Click_CollectFill(object sender, ClickEventArgs e)
	{
		if (Ticket.Kind != TicketKind.DynamicRow)
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "当前类型的表单不支持采账填充!");
			return;
		}
	...
	{
		if (Program.ClientPlatformType == PlatformType.AuditPlatform && Leqisoft.Model.User.Current != null && Leqisoft.Model.User.Current.IsSystemSupporter)
		{
			return true;
		}
	...
	private void _cmdCollectFill2_Click(object sender, ClickEventArgs e)
	{
		...
		try
		{
			...
			if (IsAllowExecuteCollectFormulaOnCurrentTicket())
			{
				...
				{
					ex.Log();
					Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "账套采集公式运算失败:\r\n" + ex.ToString());
				}
				finally
				{
			...
			{
				Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.Hand, "从账套的科目余额表中读取数据失败!");
			}
			else if (isVoucherVirtualTableBuildFailed)
			{
				Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.Hand, "从账套的会计凭证表中读取数据失败!");
			}
		}
		catch (Exception ex2)
		{
			ex2.Log();
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "采数公式运算失败:\r\n" + ex2.ToString());
		}
		finally
		{
			_isSuspendTicketSelectRangeAndScrollPositionCacheEvent = false;
		...
		void ClearTicketColumnCellValue(int vmStartRowIndex, int rowsCount, int colIndex)
		{
			...
			for (int n = 0; n < rowsCount; n++)
			{
				...
				{
					object newValue4 = Leqisoft.Model.Cell.ChangeDataTypeImpl(string.Empty, cellVM7.TableCell.DisplayDataType);
		...
		void ExcuteDataRowFormula_DynamicRowTicket()
		{
			...
			for (int num14 = 0; num14 < Ticket.Columns.Count; num14++)
			{
				...
				{
					Leqisoft.Model.Column byId = Table.Columns.GetById(ticketColumn.Field);
				...
				if (existTableCellTicketCell != null)
				{
					...
					if (ticketColumnLedgerCollectFormulaSetting2 != null && !_vm.IsHasFillingFormula)
					{
						try
						{
							...
							{
								List<object> list8 = valueSetOperand2.Set.Select((Tuple<Leqisoft.Model.Row, ValueOperand> u) => u.Item2.Object).ToList();
							...
							for (int num17 = 0; num17 < num16; num17++)
							{
								...
								{
									object newValue5 = Leqisoft.Model.Cell.ChangeDataTypeImpl(list7[num17], cellVM11.TableCell.DisplayDataType);
					...
					foreach (TicketColumnLedgerCollectFormulaSetting item3 in list6)
					{
						if (!item3.IsFill.IsFill)
						{
							CollectFormulaDependSettingData collectFormulaDependSettingData2 = new CollectFormulaDependSettingData
							{
								...
								DependTableCellSet = new HashSet<Leqisoft.Model.Cell>(),
								DependTableColumnSet = new HashSet<Leqisoft.Model.Column>(),
								userData = item3
							};
									...
									try
									{
										...
										if (operand4 is CellsOperand { IsCollectFill: not false } cellsOperand2)
										{
											if (!_vm.IsHasFillingFormula)
											{
												...
												List<object> list10 = cellsOperand2.Cells.Select((Leqisoft.Model.Cell u) => u.Value).ToList();
												...
												for (int num21 = 0; num21 < num20; num21++)
												{
													...
													{
														object newValue6 = Leqisoft.Model.Cell.ChangeDataTypeImpl(list10[num21], cellVM13.TableCell.DisplayDataType);
										...
										object newValue7 = Leqisoft.Model.Cell.ChangeDataTypeImpl(value3, cellVM12.TableCell.DisplayDataType);
		...
		void ExcuteDataRowRangeFormula_MixTicket(MixTicketDynamicDataRowRangeSetting dynamicRange)
		{
			if (dynamicRange.DataRowsCount != 0)
			{
				...
				for (int i = 0; i < columnsCount; i++)
				{
					...
					if (cellVM.Column != null)
					{
						Leqisoft.Model.Column column = cellVM.Column;
					...
					if (ticketInputCellVM != null)
					{
						...
						if (ticketColumnLedgerCollectFormulaSetting != null && !_vm.IsHasFillingFormula)
						{
							try
							{
								...
								{
									List<object> list3 = valueSetOperand.Set.Select((Tuple<Leqisoft.Model.Row, ValueOperand> u) => u.Item2.Object).ToList();
									...
									if (mixTicketMixRangeFixedRowsIndex.Count > 0)
									{
										...
										for (int num = list2.Count - 1; num >= 0; num--)
										{
											try
											{
												object value = Leqisoft.Model.Cell.ChangeDataTypeImpl(list2[num], dataType);
												string displayValueImpl = Leqisoft.Model.Cell.GetDisplayValueImpl(value, format2);
								...
								for (int k = 0; k < num3; k++)
								{
									...
									{
										object newValue = Leqisoft.Model.Cell.ChangeDataTypeImpl(list2[k], cellVM4.TableCell.DisplayDataType);
										...
										try
										{
											...
											if (operand2 is CellsOperand { IsCollectFill: not false } cellsOperand)
											{
												if (!_vm.IsHasFillingFormula)
												{
													...
													List<object> list5 = cellsOperand.Cells.Select((Leqisoft.Model.Cell u) => u.Value).ToList();
													...
													for (int m = 0; m < num5; m++)
													{
														...
														{
															object newValue2 = Leqisoft.Model.Cell.ChangeDataTypeImpl(list5[m], cellVM6.TableCell.DisplayDataType);
											...
											object newValue3 = Leqisoft.Model.Cell.ChangeDataTypeImpl(value2, cellVM5.TableCell.DisplayDataType);
		...
		void ExcuteFixedCellFormula()
		{
			...
			Dictionary<TicketInputCellVM, Leqisoft.Model.Cell> dictionary3 = new Dictionary<TicketInputCellVM, Leqisoft.Model.Cell>();
			_vm.StartRecordNewAddTableRows();
			...
			foreach (Tuple<TicketInputCellVM, string> item10 in list13)
			{
				...
				{
					Leqisoft.Model.Cell cell = null;
					Leqisoft.Model.Column column2 = null;
			...
			foreach (Tuple<TicketInputCellVM, string> item11 in list13)
			{
				try
				{
					if (item11.Item1.TableCell != null)
					{
						...
						object newValue8 = Leqisoft.Model.Cell.ChangeDataTypeImpl(value11, item11.Item1.TableCell.DisplayDataType);
		...
		void GetDependSameTableOtherColumnsAndCells(string formula, HashSet<Leqisoft.Model.Column> colSet, HashSet<Leqisoft.Model.Cell> cellSet)
		{
			...
			{
				foreach (Leqisoft.Model.Column item12 in referredColumns)
				{
					colSet.Add(item12);
				}
			}
			...
			{
				foreach (Leqisoft.Model.Cell item13 in referredCells)
				{
					cellSet.Add(item13);
				}
			}
		...
		Leqisoft.Model.Row GetFirstAnyTableRow()
		{
		...
		bool IsAllowExecuteCollectFormulaOnCurrentTicket()
		{
					...
					if (row3.IsDynamicRowTicketDataRow)
					{
						if (!flag4)
						{
							...
							for (int num36 = 0; num36 < Ticket.Columns.Count; num36++)
							{
								...
								{
									Leqisoft.Model.Column byId2 = Table.Columns.GetById(ticketColumn2.Field);
									if (byId2 != null)
									{
				...
				else if (Ticket.Kind == TicketKind.FixedDataRowMixDynamicDataRow && row3.IsMixTicketDynamicDataRow)
				{
					if (hashSet4 == null || !hashSet4.Contains(row3.TicketRow.MixRangeDynamicDataRowTemplateId))
					{
						...
						for (int num37 = 0; num37 < Ticket.Columns.Count; num37++)
						{
							...
							{
								Leqisoft.Model.Column column3 = cellVM18.Column;
								if (column3 != null)
								{
			...
			_vm.ExecuteNewAddedTableRowsWhichNotAbleAutoTriggerFormula();
			Leqisoft.Model.Row firstAnyTableRow = GetFirstAnyTableRow();
		...
		{
			Dictionary<Leqisoft.Model.Cell, HashSet<CollectFormulaDependSettingData>> dictionary = new Dictionary<Leqisoft.Model.Cell, HashSet<CollectFormulaDependSettingData>>();
			Dictionary<Leqisoft.Model.Column, HashSet<CollectFormulaDependSettingData>> dictionary2 = new Dictionary<Leqisoft.Model.Column, HashSet<CollectFormulaDependSettingData>>();
			...
			for (int num22 = 0; num22 < list11.Count; num22++)
			{
				...
				{
					bool flag2 = false;
					foreach (Leqisoft.Model.Cell item16 in collectFormulaDependSettingData3.DependTableCellSet)
					{
					...
					if (!flag2)
					{
						foreach (Leqisoft.Model.Column item18 in collectFormulaDependSettingData3.DependTableColumnSet)
						{
			...
			while (list11.Count > 0)
			{
				...
				{
					bool flag3 = false;
					foreach (Leqisoft.Model.Cell item22 in item21.DependTableCellSet)
					{
					...
					foreach (Leqisoft.Model.Column item24 in item21.DependTableColumnSet)
					{
	...
	protected void DebugOutputTicketTableRowsData(string outFilePath = null)
	{
		...
		HashSet<Leqisoft.Model.Row> hashSet = new HashSet<Leqisoft.Model.Row>();
		...
		List<Leqisoft.Model.Row> list = hashSet.ToList();
		list.Sort((Leqisoft.Model.Row left, Leqisoft.Model.Row right) => left.Index.CompareTo(right.Index));
	...
	private void RestorePreviousSelectedRecordInTableMode()
	{
		...
		if (isSelectByRowId)
		{
			TicketRecord ticketRecord = Ticket.Records.Find((TicketRecord r) => r.Rows.Any((Leqisoft.Model.Row u) => u.Id.Value == rowId));
	...
	public void ShowNavTreePanelForTableEditor(Leqisoft.Model.Table willToShowTable)
	{
	...
	public void RemoveRecordRows()
	{
		...
		for (int i = selection.TopRow; i <= selection.BottomRow; i++)
		{
			if (i >= 0 && i < _grid.Rows.Count && _grid.Rows[i].Visible)
			{
				...
				Leqisoft.Model.Row tempRow = _vm.GetRow(index).TempRow;
				if (tempRow != null && (!Program.MainForm.TableEditor.CanEditRow(tempRow) || tempRow.IsLocked || Table.ControlLockRows.Contains(tempRow)))
				{
	...
	protected void Populate(bool isOpenSelectedRecordOnLeave, bool isExandToCurrentSelecteRecord, bool isRestoreFilterSetting = true)
	{
		...
		try
		{
			...
			HasFillingFormula = Table.Columns.Where((Leqisoft.Model.Column c) => c.HasFormula).Any(delegate(Leqisoft.Model.Column c)
			{
	...
	public void DeleteRecord()
	{
		...
		if (Record.Rows.Any((Leqisoft.Model.Row r) => !Program.MainForm.TableEditor.CanEditRow(r) || r.IsLocked || Table.ControlLockRows.Contains(r)))
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "您无权删除当前" + Ticket.GetLevelString() + "。");
		}
		else
		{
			if (Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "确定要删除当前" + Ticket.GetLevelString() + "吗？", MessageBoxButtons.OKCancel) != DialogResult.OK)
			{
				return;
			}
			_isSuspendTicketSelectRangeAndScrollPositionCacheEvent = true;
			...
			try
			{
				...
				foreach (Leqisoft.Model.Row row in Record.Rows)
				{
					row.Remove();
				}
	...
	public void PrintTableRowsData(string msg)
	{
		...
		foreach (TicketFixedMultiRowVM fixedMultiRowVM in _vm.FixedMultiRowVMs)
		{
			if (fixedMultiRowVM.Row != null)
			{
				...
				{
					Leqisoft.Model.Cell cell = Table[fixedMultiRowVM.Row.Index, i];
	...
	private void SaveTable(bool isReCalculateTable)
	{
		...
		Table.Project.Dal.SaveTreeNodes(new Leqisoft.DTO.TreeNode[1] { Table.TreeNode.ToDto() });
	}
	...
	public void SaveRecord(bool isSaveReccordFilterSetting = true, bool isRePopulate = true)
	{
		...
		void DeleteTableRows(HashSet<Leqisoft.Model.Row> rowSet)
		{
			if (rowSet.Count != 0)
			{
				if (rowSet.Count == 1)
				{
					foreach (Leqisoft.Model.Row item in rowSet)
					{
						item.Remove();
					}
					return;
				}
				List<Leqisoft.Model.Row> list = rowSet.ToList();
				list.Sort((Leqisoft.Model.Row left, Leqisoft.Model.Row right) => left.Index.CompareTo(right.Index));
				List<List<Leqisoft.Model.Row>> list2 = new List<List<Leqisoft.Model.Row>>
				{
					new List<Leqisoft.Model.Row> { list[0] }
				};
				for (int j = 1; j < list.Count; j++)
				{
					if (list[j].Index != list[j - 1].Index + 1)
					{
						list2.Add(new List<Leqisoft.Model.Row> { list[j] });
					}
					else
					{
		...
		Leqisoft.Model.Row RemoveTableEmptyRow_DynamicRow()
		{
			bool flag2 = false;
			Leqisoft.Model.Row keyCellsRefTableRow_DynamicRow = _vm.GetKeyCellsRefTableRow_DynamicRow();
			Leqisoft.Model.Row row3 = null;
			HashSet<Leqisoft.Model.Row> hashSet2 = new HashSet<Leqisoft.Model.Row>();
			int rowsCount = _vm.GetRowsCount();
			int columnsCount = _vm.GetColumnsCount();
			for (int i = 0; i < rowsCount; i++)
			{
		...
		Leqisoft.Model.Row RemoveTableEmptyRow_FixedMultiRow()
		{
			Leqisoft.Model.Row row2 = null;
			HashSet<Leqisoft.Model.Row> hashSet = new HashSet<Leqisoft.Model.Row>();
			foreach (TicketFixedMultiRowVM fixedMultiRowVM in _vm.FixedMultiRowVMs)
			{
				if (fixedMultiRowVM.Row != null)
		...
		Leqisoft.Model.Row RemoveTableEmptyRow_FixedOneRow()
		{
		...
		Leqisoft.Model.Row RemoveTableEmptyRow_MixTicket()
		{
			Leqisoft.Model.Row row5 = null;
			HashSet<Leqisoft.Model.Row> hashSet3 = new HashSet<Leqisoft.Model.Row>();
			int rowsCount2 = _vm.GetRowsCount();
			int columnsCount2 = _vm.GetColumnsCount();
			...
			Leqisoft.Model.Row keyCellsRefTableRow_MixTicket = _vm.GetKeyCellsRefTableRow_MixTicket();
	...
	public void AddNav()
	{
		...
		try
		{
			...
			{
				Columns = ((System.Collections.IEnumerable)formTicketNav.Result).Cast<Leqisoft.Model.Column>().Select((Leqisoft.Model.Column c) => c.Id).ToList()
			});
	...
	private void CalculateTicketRecordValidationFailedCount()
	{
		...
		try
		{
			...
			foreach (TicketRecord record in Ticket.Records)
			{
				int cellCount = 0;
				int rpCount = 0;
				HashSet<Id64> refRowsId = new HashSet<Id64>(record.Rows.Select((Leqisoft.Model.Row r) => r.Id));
				int value2 = value.ErrorRefs.Sum(delegate(object u)
				{
					if (u is Leqisoft.Model.Cell cell)
					{
						cellCount++;
					...
					if (u is Leqisoft.Model.Column column)
					{
					...
					{
						rpCount++;
						return rangeOperand.Cells.Any((Leqisoft.Model.Cell c) => refRowsId.Contains(c.Row.Id) && refColsIdSet.Contains(c.Column.Id)) ? 1 : 0;
					}
					return 0;
				});
	...
	public void Cut()
	{
		...
		catch (Exception exception)
		{
			...
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.Hand, "无法向剪贴板上写入数据！", MessageBoxButtons.OK, "剪贴板错误");
		}
		_grid.Invalidate();
		TitleEditor.Invalidate();
		FooterEditor.Invalidate();
	...
	public static object ConvertCopyValueToCellValue(object value, Type dataType)
	{
		...
		return Leqisoft.Model.Cell.ChangeDataTypeImpl(value, dataType);
	}
	...
	public void ExportPdf()
	{
		...
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.Hand, "导出失败: " + ex.Message, MessageBoxButtons.OK, "错误!");
			ex.Log();
		}
	}
	...
	public static string TicketNavToString(List<Leqisoft.Model.Column> nav)
	{
		return string.Join("→", nav.Select((Leqisoft.Model.Column c) => c.GetUniqueFormulaName()));
	}
	...
	private void ShowTooltip()
	{
		...
		if (Program.MainForm.TableValidationResults.TryGetValue(Table.TreeNode, out var value) && cellVM.IsTableExistCell)
		{
			...
			IEnumerable<Tuple<Leqisoft.Model.Cell, ValidationResult>> enumerable = ((IEnumerable<Tuple<Leqisoft.Model.Cell, ValidationResult>>)value.Cells).Where((Tuple<Leqisoft.Model.Cell, ValidationResult> t) => t.Item1.Id == tableCell.Id);
			...
			IEnumerable<Tuple<Leqisoft.Model.Column, ValidationResult>> enumerable3 = ((IEnumerable<Tuple<Leqisoft.Model.Column, ValidationResult>>)value.Columns).Where((Tuple<Leqisoft.Model.Column, ValidationResult> t) => t.Item1.Index == cellColIndex);
			IEnumerable<Tuple<Leqisoft.Model.Cell, ValidationResult>> enumerable4 = ((IEnumerable<Tuple<Leqisoft.Model.Cell, ValidationResult>>)value.HeaderCells).Where((Tuple<Leqisoft.Model.Cell, ValidationResult> t) => t.Item1.Column.Index == cellColIndex && t.Item1.Row.Index < cellRowIndex && t.Item1.GetHeaderLastRow() >= cellRowIndex);
	...
	private bool IsGridColumnCanEdit(Leqisoft.Model.Column column)
	{
		if (column == null)
		{
			return false;
	...
	private void SetThemeNavTreePanel()
	{
		...
		TableTicketViewModeThemeContext tableTicketViewModePanelContext = Theme.SelectedLeqiTheme.ThemeContext.TableTicketViewModePanelContext;
	...
	private void SetThemeNavs()
	{
		...
		_pageTitleBackgroundColor = Theme.SelectedLeqiTheme.GetBackgroundSolidColor("C1Command\\C1OutBar\\Page\\Title\\Hot\\Background");
	}
	...
	private void _grid_BodyOwnerDrawCell(object sender, OwnerDrawCellEventArgs e)
	{
		try
		{
			...
			{
				Leqisoft.Model.Cell tableCell = gridBodyCellVM.TempCell;
				int cellRowIndex = tableCell.Row.Index;
				int cellColIndex = tableCell.Column.Index;
				IEnumerable<Tuple<Leqisoft.Model.Cell, ValidationResult>> source = ((IEnumerable<Tuple<Leqisoft.Model.Cell, ValidationResult>>)value.Cells).Where((Tuple<Leqisoft.Model.Cell, ValidationResult> t) => t.Item1.Id == tableCell.Id);
				...
				IEnumerable<Tuple<Leqisoft.Model.Column, ValidationResult>> source3 = ((IEnumerable<Tuple<Leqisoft.Model.Column, ValidationResult>>)value.Columns).Where((Tuple<Leqisoft.Model.Column, ValidationResult> t) => t.Item1.Index == cellColIndex);
				IEnumerable<Tuple<Leqisoft.Model.Cell, ValidationResult>> source4 = ((IEnumerable<Tuple<Leqisoft.Model.Cell, ValidationResult>>)value.HeaderCells).Where((Tuple<Leqisoft.Model.Cell, ValidationResult> t) => t.Item1.Column.Index == cellColIndex && t.Item1.Row.Index < cellRowIndex && t.Item1.GetHeaderLastRow() >= cellRowIndex);
	...
	private void SendBodySelectionChanged_SignalR(int rowIndex, int colIndex)
	{
		...
		async Task BodySelectionChanged_SignalR(int bodyRowIndex, int bodyColIndex)
		{
			try
			{
				...
				{
					Leqisoft.Model.Cell tableCell = gridBodyCellVM.TableCell;
					if (tableCell != null)
					{
						num = tableCell.Id.Value;
					}
				}
				await SignalRClient.UpLoadTableCellId(Leqisoft.Model.User.Current.Id.ToString(), num.ToString());
			}
			catch
			{
			}
	...
	private void _otbNavs_PageTitlePostPaint(C1OutPage page, PaintEventArgs e, Rectangle rect)
	{
		...
		try
		{
			...
			if (_isMouseOverMoreMenuIcon)
			{
				if (flag)
				{
					_brushMoreMenuIconBackground.Color = Leqisoft.UI.Controls.Util.LightColor(_pageTitleBackgroundColor, 0.2);
				}
				else
				{
					_brushMoreMenuIconBackground.Color = Leqisoft.UI.Controls.Util.DarkenColor(_pageTitleBackgroundColor, 0.1);
				}
	...
	private List<RowFillSetting> PrepareVMDataForFillCollectResult_Balance(TableCollectorBalance balanceCollector, TableCollectResult collectResult, TicketCollectFillTable ticketTable, RowIncreaseSetting rowIncreaseSetting)
	{
		...
		if (balanceCollector.Maps.Any((KeyValuePair<long, string> m) => m.Value.Equals("科目名称")))
		{
			...
			list2.Add(num);
			Leqisoft.Model.Column columnByIndex = ticketTable.GetColumnByIndex(num);
	...
	{
		List<RowFillSetting> list = null;
		List<Leqisoft.Model.Column> condtionCols = (from m in subsidiayCollector.Maps
			where new string[7] { "日期", "字号", "字", "号", "摘要", "借方金额", "贷方金额" }.Contains(m.Value)
			select m into kv
			select ticketTable.GetColumnByIndex((int)kv.Key)).ToList();
		List<Leqisoft.Model.Column> decimalValueCols = (from m in subsidiayCollector.Maps
		...
		return list;
		List<List<Tuple<string, TicketInputCellVM, int>>> GetAllRowsConditionData(List<Leqisoft.Model.Column> condtionColIList)
		{
			...
			for (int n = 0; n < dataRowsCount2; n++)
			{
				...
				bool flag2 = true;
				foreach (Leqisoft.Model.Column condtionColI in condtionColIList)
				{
		...
		int GetDataAppendStartRowIndex()
		{
			...
			{
				foreach (Leqisoft.Model.Column item3 in condtionCols)
				{
		...
		int GetGridMatchRowIndex(List<List<Tuple<string, TicketInputCellVM, int>>> rowsCondtionDataList, List<Leqisoft.Model.Column> condtionColIList, int checkRowIndex)
		{
			...
			for (int k = 0; k < count3; k++)
			{
				...
				{
					Leqisoft.Model.Column column = condtionColIList[l];
	...
	public void FillCollectResult(TableCollectResult collectResult, TicketCollectFillTable ticketTable)
	{
		...
		try
		{
			...
			if (!(tableCollector is TableCollectorBalance balanceCollector))
			{
				if (!(tableCollector is TableCollectorSubsidiary subsidiayCollector))
				{
					...
					{
						Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "当前版本暂时不支持该类型的采账填充!");
						return;
					}
		...
		try
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				array[i] = IsGridColumnCanEdit((Leqisoft.Model.Column)column.TableColumn);
			}
			foreach (Leqisoft.Model.Column key in collectResult.Values.Keys)
			{
				...
				for (int j = 0; j < count2; j++)
				{
					...
					if (IsGridCellCanEditWithoutColumnCheck(cellVM))
					{
						...
						_vm.UpdateTicketCellValue(cellVM, Leqisoft.Model.Cell.ConvertInputValueToCellValue(obj), isFormulaExistManualInputValue: true);
					}
				}
			}
			_vm.EndBatchUpdateValue();
