...
public class frmNodeSelector : C1RibbonForm
{
	...
	public DialogResult ShowBatchBrush()
	{
		Text = "样式刷";
		base.Icon = Leqisoft.UI.Controls.Theme.SelectedLeqiTheme.GetThemedIcon(Resources.TableStyleBrush);
		ckbExportExcel.Visible = false;
		cboShowHideNodes.Visible = false;
		Populate();
		PopulateBatchBrushCheck();
		_grid.MouseClick += _grid_MouseClick_Tooltip_Brush;
		return ShowDialog();
	}
	...
	{
		Text = "批量打印文件";
		base.Icon = Leqisoft.UI.Controls.Theme.SelectedLeqiTheme.GetThemedIcon(Resources.BatchPrint16);
		ckbExportExcel.Visible = false;
		cboShowHideNodes.Visible = false;
		Populate();
	...
	{
		Text = "批量导出文件";
		base.Icon = Leqisoft.UI.Controls.Theme.SelectedLeqiTheme.GetThemedIcon(Resources.BatchExport16);
	...
	public DialogResult ShowHideNode(int mode)
	{
		...
		cboShowHideNodes.Items.Clear();
		cboShowHideNodes.Items.AddRange(new string[3] { "列示全部文件", "仅列示隐藏文件", "不列示隐藏文件" });
		cboShowHideNodes.SelectedIndex = mode;
		cboShowHideNodes.SelectedIndexChanged += CboShowHideNodes_SelectedIndexChanged;
		Populate(mode);
	...
	{
		Text = "批量删除文件";
		base.Icon = Leqisoft.UI.Controls.Theme.SelectedLeqiTheme.GetThemedIcon(Resources.BatchRemoveNodes16);
		ckbExportExcel.Visible = false;
		cboShowHideNodes.Visible = false;
		_grid.Glyphs[GlyphEnum.Grayed] = Resources.NoPermission;
		_grid.MouseClick += _grid_MouseClick_Tooltip_RemoveNodes;
	...
	{
		Text = "跨表批量复制列";
		base.Icon = Leqisoft.UI.Controls.Theme.SelectedLeqiTheme.GetThemedIcon(Resources.BatchColumnDuplicate16);
		ckbExportExcel.Visible = false;
		cboShowHideNodes.Visible = false;
		Populate();
		return ShowDialog();
	...
	{
		Text = "跨表批量删除列";
		base.Icon = Leqisoft.UI.Controls.Theme.SelectedLeqiTheme.GetThemedIcon(Resources.BatchColumnRemove16);
		ckbExportExcel.Visible = false;
		cboShowHideNodes.Visible = false;
		Populate();
		return ShowDialog();
	...
	{
		Text = "跨表批量重命名列";
		base.Icon = Leqisoft.UI.Controls.Theme.SelectedLeqiTheme.GetThemedIcon(Resources.BatchColumnRename16);
		ckbExportExcel.Visible = false;
		cboShowHideNodes.Visible = false;
		Populate();
		return ShowDialog();
	...
	{
		Text = "智能扩充跨表公式";
		base.Icon = Leqisoft.UI.Controls.Theme.SelectedLeqiTheme.GetThemedIcon(Resources.GenerateBatchFormula16);
		ckbExportExcel.Visible = false;
		cboShowHideNodes.Visible = false;
		Populate();
		return ShowDialog();
	...
	protected void SwitchSelectRangeCellChecBox(int mouseClickGridRowIndex = -1)
	{
		...
		else
		{
			...
			{
				if (i >= _grid.Rows.Fixed && i < _grid.Rows.Count && _grid.Rows[i].Visible)
				{
					CheckEnum cellCheck = _grid.GetCellCheck(i, 1);
		...
		CheckEnum check;
		switch (_grid.GetCellCheck(num, 1))
		{
		case CheckEnum.None:
		...
		try
		{
			...
			for (int j = selection.TopRow; j <= selection.BottomRow; j++)
			{
				...
				if (row.Visible)
				{
					CheckEnum cellCheck2 = _grid.GetCellCheck(j, 1);
					if (cellCheck2 == CheckEnum.Grayed || cellCheck2 == CheckEnum.None)
					{
						num2 = -1;
						continue;
					}
					_grid.SetCellCheck(j, 1, check);
					num2 = j;
				}
			}
			if (num2 == -1)
			...
			Leqisoft.Model.TreeGroup treeGroup = userData as Leqisoft.Model.TreeGroup;
			if (treeGroup == null)
			{
			...
			checkChildren(_grid.Rows[num2].Node, check);
		}
	...
	private void CboShowHideNodes_SelectedIndexChanged(object sender, EventArgs e)
	{
		Populate(cboShowHideNodes.SelectedIndex);
		PopulateShowHideCheck();
	}

	private void Populate(int mode = 2)
	{
		_grid.BeginUpdate();
		try
		{
			...
			foreach (Leqisoft.Model.TreeGroup treeGroup in Project.TreeGroups)
			{
				...
				foreach (TreeNodeBase rootNode in treeGroup.RootNodes)
				{
					...
					if (mode == 2 && !rootNode.Visible)
					{
						node2.Row.Visible = false;
					}
					else
					{
						if (mode != 1 || !rootNode.Visible)
						{
							continue;
						}
						if (rootNode is TreeDirectoryNode treeDirectoryNode2)
						{
							if (treeDirectoryNode2.GetDescendants().All((TreeNodeBase n) => n.Visible))
							{
								node2.RemoveNode();
							}
						}
		...
		void AddDirectoryNode(TreeDirectoryNode subRoot, Node subRootView)
		{
			...
			{
				Node node3 = null;
				if (!(child is TreeDirectoryNode treeDirectoryNode3))
				{
				...
				if (mode == 2 && !child.Visible)
				{
					node3.Row.Visible = false;
				}
				else if (mode == 1 && child.Visible)
				{
					if (child is TreeDirectoryNode treeDirectoryNode4)
					{
						if (treeDirectoryNode4.GetDescendants().All((TreeNodeBase n) => n.Visible))
						{
							node3.RemoveNode();
						}
					}
	...
	private void PopulateShowHideCheck()
	{
		for (int i = _grid.Rows.Fixed; i < _grid.Rows.Count; i++)
		{
			if (_grid.Rows[i].UserData is TreeNodeBase treeNodeBase)
			{
				if (Program.MainForm.ProjectHierarchy.CanRemoveNode(treeNodeBase))
				{
					_grid.SetCellCheck(i, 1, treeNodeBase.Visible ? CheckEnum.Checked : CheckEnum.Unchecked);
				}
				else
				{
	...
	private void PopulateRemoveNodesCheck()
	{
		for (int i = _grid.Rows.Fixed; i < _grid.Rows.Count; i++)
		{
			if (_grid.Rows[i].UserData is TreeNodeBase node)
			{
				if (!Program.MainForm.ProjectHierarchy.CanRemoveNode(node))
				{
					_grid.SetCellCheck(i, 1, CheckEnum.Grayed);
				}
			}
	...
	private void PopulateBatchBrushCheck()
	{
		for (int i = _grid.Rows.Fixed; i < _grid.Rows.Count; i++)
		{
			if (_grid.Rows[i].UserData is TreeNodeBase treeNodeBase && (!treeNodeBase.HasWritePermission() || !treeNodeBase.HasSchemaPermission()))
			{
				_grid.SetCellCheck(i, 1, CheckEnum.Grayed);
			}
		}
	...
	private void GrdEditor_CellChecked_ShowHideNode(object sender, RowColEventArgs e)
	{
		try
		{
			_grid.CellChecked -= GrdEditor_CellChecked_ShowHideNode;
			...
			object userData = _grid.Rows[e.Row].UserData;
			if (!(userData is TreeDirectoryNode) && !(userData is TreeDocumentNode) && !(userData is TreeTableNode) && !(userData is TreeImageNode) && !(userData is TreePdfNode))
			{
				return;
			}
			_grid.BeginUpdate();
			for (Node node = _grid.Rows[e.Row].Node; node != null; node = node.Parent)
			{
				CheckEnum cellCheck2 = _grid.GetCellCheck(node.Row.Index, 1);
				if (cellCheck2 != CheckEnum.Grayed && cellCheck2 != 0)
				{
					_grid.SetCellCheck(node.Row.Index, 1, CheckEnum.Checked);
				}
			}
			_grid.EndUpdate();
		}
	...
	private void _grid_MouseClick_Tooltip_ShowHideNode(object sender, MouseEventArgs e)
	{
		_ttp.Hide();
		if (_grid.Col == 1 && _grid.GetCellCheck(_grid.Row, _grid.Col) == CheckEnum.Grayed && _grid.Rows[_grid.Row].UserData is TreeNodeBase treeNodeBase && !Program.MainForm.ProjectHierarchy.CanRemoveNode(treeNodeBase))
		{
			string content;
			if (treeNodeBase is TreeDirectoryNode treeDirectoryNode)
			{
				TreeNodeBase firstCantDeleteDescendant = treeDirectoryNode.GetFirstCantDeleteDescendant();
				content = "<p style=\"color:red\">因您没有该文件夹下【" + firstCantDeleteDescendant.Name + "】文件的【" + firstCantDeleteDescendant.GetDontHavePermissionString() + "】权限，因此，无法对该文件夹及其下所有文件执行显示/隐藏操作。</p>";
			}
			else
			{
				content = "<p style=\"color:red\">因您没有该文件的【" + treeNodeBase.GetDontHavePermissionString() + "】权限，因此，无法对该文件执行显示/隐藏操作。</p>";
			}
	...
	private void _grid_MouseClick_Tooltip_RemoveNodes(object sender, MouseEventArgs e)
	{
		_ttp.Hide();
		if (_grid.Col == 1 && _grid.GetCellCheck(_grid.Row, _grid.Col) == CheckEnum.Grayed && _grid.Rows[_grid.Row].UserData is TreeNodeBase treeNodeBase && !Program.MainForm.ProjectHierarchy.CanRemoveNode(treeNodeBase))
		{
			...
			{
				TreeNodeBase firstCantDeleteDescendant = treeDirectoryNode.GetFirstCantDeleteDescendant();
				content = "<p style=\"color:red\">因您没有该文件夹下【" + firstCantDeleteDescendant.Name + "】文件的【" + firstCantDeleteDescendant.GetDontHavePermissionString() + "】权限，因此，无法对该文件夹及其下所有文件执行删除操作。</p>";
			}
			else
			{
				content = "<p style=\"color:red\">因您没有该文件的【" + treeNodeBase.GetDontHavePermissionString() + "】权限，因此，无法对该文件执行删除操作。</p>";
			}
			_ttp.SetText("操作提示", content);
			Rectangle cellRect = _grid.GetCellRect(_grid.Row, _grid.Col);
			_ttp.Show(_grid, new Point(cellRect.Right, cellRect.Top + cellRect.Height / 2));
	...
	private void _grid_MouseClick_Tooltip_Brush(object sender, MouseEventArgs e)
	{
		_ttp.Hide();
		if (_grid.Col == 1 && _grid.GetCellCheck(_grid.Row, _grid.Col) == CheckEnum.Grayed && _grid.Rows[_grid.Row].UserData is TreeNodeBase treeNodeBase && (!treeNodeBase.HasWritePermission() || !treeNodeBase.HasSchemaPermission()))
		{
			string content = "<p style=\"color:red\">因您没有该文件的【" + treeNodeBase.GetDontHavePermissionString() + "】权限，因此，无法对该文件执行样式刷操作。</p>";
			_ttp.SetText("操作提示", content);
			Rectangle cellRect = _grid.GetCellRect(_grid.Row, _grid.Col);
			_ttp.Show(_grid, new Point(cellRect.Right, cellRect.Top + cellRect.Height / 2));
		}
	...
	private void SearchImpl()
	{
		...
		HashSet<Leqisoft.Model.TreeGroup> hashSet2 = new HashSet<Leqisoft.Model.TreeGroup>();
		...
		for (int i = _grid.Rows.Fixed; i < _grid.Rows.Count; i++)
		{
			...
			if (row.UserData is Leqisoft.Model.TreeGroup item)
			{
	...
	private void InitializeComponent()
	{
		...
		this._grid = new Leqisoft.UI.Controls.C1FlexGridEx();
		...
		this.pnlSearchTxt.KeepRelativeSize = false;
		this.pnlSearchTxt.Location = new System.Drawing.Point(0, 0);
		this.pnlSearchTxt.MinHeight = 0;
		...
		this.btnSearch.Image = Leqisoft.UI.Platform.Properties.Resources.btnSearch;
