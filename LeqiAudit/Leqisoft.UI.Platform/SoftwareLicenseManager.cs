...
public class SoftwareLicenseManager
{
	...
	public static string GetCurrentLicenseDisplayName()
	{
		if (Program.ClientPlatformType == PlatformType.Custom)
		{
			if (IsPayByProject())
			{
				if (Leqisoft.Model.Project.Current != null && Leqisoft.Model.Project.Current.ProjectChargeType == ChargeType.Pay)
				{
					return "正式版";
				}
				return "免费版";
	...
	{
		if (Leqisoft.Model.User.Current == null)
		{
			return false;
		}
		if (Leqisoft.Model.User.Current.IsSystemSupporter)
	...
	public static bool IsAllowAddTableRows(bool showDialog = true)
	{
		...
		if (IsPayByProjectReachExpireDate())
		{
			if (showDialog)
			{
				Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, $"尊敬的用户：\r\n您的产品已于{Leqisoft.Model.Project.Current.ProjectLicenseDate:yyyy年MM月dd日}到期，无增加表格行的权限, 建议您致电官方客服电话：400-690-6500，联系购买或续期！");
			}
			return false;
		}
		if (IsTeamReachExpireDate())
	...
	public static bool IsTableRowsCountOutOfLicenseLimit(Leqisoft.Model.Table table, int newRowsCount)
	{
		if (Leqisoft.Model.User.Current.IsSystemSupporter)
		{
			return false;
		...
		if (IsPayByProject())
		{
			if (Leqisoft.Model.Project.Current != null && Leqisoft.Model.Project.Current.ProjectChargeType == ChargeType.Pay)
			{
				...
				{
					Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "单表格的最大行数限制为" + 1000000 + "行！");
					return true;
				}
				return false;
			}
	...
	public static bool IsTableColsCountOutOfLicenseLimit(Leqisoft.Model.Table table, int newColsCount)
	{
	...
	public static bool IsTableAttachmentOutOfLicenseLimit(long fileLength)
	{
		...
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "附件大小不能超过" + 50 + "MB");
			return true;
		}
		return false;
	}
	...
	public static bool IsImportPDFFileOutOfLicenseLimit(double fileLength)
	{
		...
		if (fileLength > 50.0)
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "您导入的PDF文件过大，无法为您导入！");
			return true;
		}
		return false;
	}
	...
	public static bool IsImportImageFileOutOfLicenseLimit(double fileLength)
	{
		...
		if (fileLength > 50.0)
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "您导入的图片文件过大，无法为您导入！");
			return true;
		}
		return false;
	}
	...
	public static bool IsPayByProjectReachExpireDate(Leqisoft.Model.Project project)
	{
	...
	protected static bool IsPayByProjectReachExpireDate()
	{
		...
		if (UserTeam.CurrentTeamIsPayByProject && Leqisoft.Model.Project.Current != null)
		{
			double totalDays = (Leqisoft.Model.Project.Current.ProjectLicenseDate - DateTime.Now).TotalDays;
			if (totalDays < 0.0)
			{
				return true;
			}
	...
	public static bool IsPayByProjectWillReachExpireDate(Leqisoft.Model.Project project)
	{
		if (Leqisoft.LocalDataStore.StorageRouter.IsLocalMode)
		{
			return false;
