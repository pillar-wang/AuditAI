...
public static class ProjectArchive
{
    ...
    public static void Export(Leqisoft.DTO.Project projectInfo, string outputPath)
    {
        ...
        var metadata = new ProjectArchiveMetadata
        {
            ...
            ExportedBy = Leqisoft.Model.User.Current?.Name ?? Leqisoft.Model.User.Current?.UserName ?? "",
    ...
    public static Leqisoft.DTO.Project Import(string archivePath)
    {
        if (!File.Exists(archivePath))
            throw new FileNotFoundException("归档文件不存在", archivePath);
        ...
        try
        {
            ...
            // 8. 在主数据库注册新项目
            var newProject = new Leqisoft.DTO.Project
            {
                Id = newProjectId,
            ...
            Leqisoft.LocalDataStore.LocalDataStore.RegisterImportedProject(newProject);
