...
public class TableTitleEditor
{
	...
	public void ShowCommentTooltip(int row, int col, bool forceShow)
	{
		...
		if (flag2)
		{
			IEnumerable<Tuple<Leqisoft.Model.Table, int, int, ValidationResult>> enumerable = value.Titles.Cast<Tuple<Leqisoft.Model.Table, int, int, ValidationResult>>().Where((Tuple<Leqisoft.Model.Table, int, int, ValidationResult> t) => t.Item2 == row + 1 && t.Item3 == col + 1);
			...
			if (flag2)
			{
				...
				try
				{
					...
					i = 0;
					foreach (Tuple<Leqisoft.Model.Table, int, int, ValidationResult> item in enumerable)
					{
	...
	public void UpdateCellValue(TableTitleCell cell, object value)
	{
		...
		object obj = Leqisoft.Model.Cell.ChangeDataTypeImpl(value, cell.DataFormat.GetDataType());
		if (cell.Value != obj)
		{
			cell.Value = obj;
			Title.Table.TagTitleDirty();
	...
	private void _grid_BodyOwnerDrawCell(object sender, OwnerDrawCellEventArgs e)
	{
		try
		{
			...
			{
				e.Text = string.Empty;
				e.Image = (cell.Value.Equals(true) ? Leqisoft.UI.Platform.Properties.Resources.On : Leqisoft.UI.Platform.Properties.Resources.Off);
				styleNew.ImageAlign = C1FlexGridEx.ToImageAlign(cell.Align);
			}
			else
			{
			...
			{
				Tuple<Leqisoft.Model.Table, int, int, ValidationResult> tuple = value.Titles.Cast<Tuple<Leqisoft.Model.Table, int, int, ValidationResult>>().FirstOrDefault((Tuple<Leqisoft.Model.Table, int, int, ValidationResult> t) => t.Item2 == e.Row + 1 && t.Item3 == e.Col + 1);
				if (tuple != null)
				{
	...
	private void Paint_Formula(PaintEventArgs e)
	{
		...
		{
			TableEditor._penFormulaCell.Color = Theme.SelectedLeqiTheme.ThemeContext.DarkColor;
		...
		if (_owner.FormulaEditor.LastClickedComponent is TableTitleEditor && _lastMouseUpCell.HasValue)
		{
			...
			TableEditor._penAnimateDash.Color = ((Control.MouseButtons == MouseButtons.Left) ? Theme.SelectedLeqiTheme.ThemeContext.DarkColor : _owner.FormulaEditor.NextColor);
		...
		if (!(e is Leqisoft.Model.Column column))
		{
			if (!(e is Leqisoft.Model.Cell cell))
			{
				if (!(e is TreeNodeBase node))
				{
					...
					Leqisoft.Model.Project project = await Program.MainForm.OpenOrSwitchToProject(consolidateAttributes.ProjectId);
					if (project != null)
					{
