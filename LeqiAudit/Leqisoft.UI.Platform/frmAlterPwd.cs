...
{
	private readonly Color _leqiBlue = Color.FromArgb(0, 195, 245);
	...
	private async void Initialize()
	{
		...
		try
		{
			result = await WebApiClient.GetFuzzyPhone(Leqisoft.Model.User.Current.UserName);
		...
		if (!_bindPhone && !Program.IsOnPremise && Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "您的账号未捆绑手机号，出于保护账号安全和遗忘密码后可安全找回的需要，建议您捆绑手机号，需要马上捆绑手机号吗？", MessageBoxButtons.OKCancel) == DialogResult.OK)
		{
			Close();
	...
	private async void btnGetValidateCode_Click(object sender, EventArgs e)
	{
		...
		try
		{
			...
			{
				Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "请先输入原密码！");
			}
			else if (ValidateAllText())
			{
				_ = string.Empty;
				string phone = (await WebApiClient.GetUserById(Leqisoft.Model.User.Current.Id)).Phone;
	...
	private void InitializeComponent()
	{
		...
		this.txtNewPassword2 = new Leqisoft.UI.Controls.C1TextBoxEx();
