...
public class FormulaEditor : ISetTheme
{
	public class LedgerVirtualTableSettingProvider : ILegderVirtualTableSetting
	{
		...
		public Leqisoft.Model.Column GetBalanceEmptyVirtualTableColumn(string columnName)
		{
		...
		public Leqisoft.Model.Column GetVoucherEmptyVirtualTableColumn(string columnName)
		{
	...
	public void RemoveRefAtPos()
	{
		...
		catch (Leqisoft.Model.FormulaException)
		{
		}
	}
	...
	{
		if (Theme.SelectedLeqiTheme == null)
		{
			return Color.Gray;
		}
		return Theme.SelectedLeqiTheme.ThemeContext.FormulaEditorBorderColor;
	}
	...
	{
		Color formulaEditorBorderColor = Theme.SelectedLeqiTheme.ThemeContext.FormulaEditorBorderColor;
	...
	private void CommitInputForColHeader()
	{
		...
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, $"公式长度不得超过{10000}字符");
			return;
		}
		try
		{
			...
			else
			{
				...
				if (AF == null)
				{
					ApplicationField applicationField = new ApplicationField(ApplicationFieldFormat.MSWord, "MERGEFIELD", text3, new string[3]
					{
						"Formula",
						text2,
						Leqisoft.Model.Project.Current.GetNextId().ToString()
					})
					{
						Deleteable = true,
	...
	public static string EvaluateDocumentFormula(string formula, bool calculateTable)
	{
		...
		if (calculateTable)
		{
			...
			{
				Leqisoft.Model.Project.Current?.GetTableById(item)?.CalculateRecursive();
			}
		}
		var currentProject = Leqisoft.Model.Project.Current;
