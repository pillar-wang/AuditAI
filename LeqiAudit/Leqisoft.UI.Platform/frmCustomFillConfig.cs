...
public class frmCustomFillConfig : C1RibbonForm
{
	...
	private static readonly Dictionary<string, Leqisoft.Model.ConditionOperator> _opMap = new()
	{
		{ "等于", Leqisoft.Model.ConditionOperator.Equals },
		{ "不等于", Leqisoft.Model.ConditionOperator.NotEquals },
		{ "包含", Leqisoft.Model.ConditionOperator.Contains },
		{ "不包含", Leqisoft.Model.ConditionOperator.NotContains },
		{ "大于", Leqisoft.Model.ConditionOperator.GreaterThan },
		{ "小于", Leqisoft.Model.ConditionOperator.LessThan },
		{ "为空", Leqisoft.Model.ConditionOperator.IsEmpty },
		{ "非空", Leqisoft.Model.ConditionOperator.IsNotEmpty },
	};

	private static readonly Dictionary<Leqisoft.Model.ConditionOperator, string> _opReverseMap;
			...
			for (int i = 0; i < config.Rules.Count; i++)
			{
				...
				if (rule.Condition != null && !string.IsNullOrWhiteSpace(rule.Condition.SourcePosition))
				{
					...
					_grid[row, ColCondOp] = _opReverseMap.TryGetValue(rule.Condition.Operator, out var display)
						? display : "";
					if (rule.Condition.Operator != Leqisoft.Model.ConditionOperator.IsEmpty
						&& rule.Condition.Operator != Leqisoft.Model.ConditionOperator.IsNotEmpty)
					{
	...
	private CustomFillConfig BuildConfigFromGrid()
	{
		...
		for (int i = 0; i < dataRowCount; i++)
		{
			...
			{
				string condValue = (condOp == Leqisoft.Model.ConditionOperator.IsEmpty || condOp == Leqisoft.Model.ConditionOperator.IsNotEmpty)
					? null
	...
	private void SaveConfigToProject()
	{
		...
		if (project?.Dal == null)
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "无法保存：项目未加载");
			return;
		}
	...
	private void BtnImport_Click(object sender, EventArgs e)
	{
		...
		try
		{
			...
			if (config?.Rules == null || config.Rules.Count == 0)
			{
				Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "文件中没有有效的填充规则");
				return;
			}
			...
			_hasUnsavedChanges = true;
			UpdateStatus();
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, $"已导入 {config.Rules.Count} 条规则");
		}
		catch (Exception ex)
		{
			ex.Log();
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "导入失败: " + ex.Message);
		}
	}
	...
	private void BtnExport_Click(object sender, EventArgs e)
	{
		...
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "没有规则可导出");
			return;
		}
		...
		try
		{
			...
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, $"已导出 {config.Rules.Count} 条规则");
		}
		catch (Exception ex)
		{
			ex.Log();
...
■ 示例：
  ...
  填充值: 已复核             位置: B1:B10  条件: A1 不等于 空";
		Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, help);
	}
	...
	private void BtnTargetPicker_Click(object sender, EventArgs e)
	{
		...
		if (row <= 0)
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "请先选中一个数据行");
			return;
		}
	...
	{
		SaveConfigToProject();
		Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "填充配置已保存");
	}
