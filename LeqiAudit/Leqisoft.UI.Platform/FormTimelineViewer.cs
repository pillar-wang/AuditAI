...
public class FormTimelineViewer
{
	...
	public Leqisoft.Model.Table TemporaryTable { get; set; }

	public Leqisoft.Model.Document TemporaryDocument { get; set; }

	public int LatestVersion { get; set; }

	public Leqisoft.Model.Table RevertedTable { get; set; }

	public Leqisoft.Model.Document RevertedDocument { get; set; }
	...
	public async Task PopulateTableTimeline()
	{
		...
		request.Add("ProjectId", Leqisoft.Model.Project.Current.Id);
		JArray jarray = null;
		...
		for (int i = 0; i < jarray.Count; i++)
		{
			...
			Tile item = new Tile
			{
				...
				Text3 = $"{pushEntityMeta.Time:G}  {Leqisoft.Model.Util.GetReadableFileSize(pushEntityMeta.Length)}",
				IntValue1 = i
			};
		...
		if (Metas.Count == 0)
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "该表格没有历史版本。");
	...
	public async Task PopulateDocumentTimeline()
	{
		...
		if (Metas.Count == 0)
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "该文档没有历史版本。");
			_form.DialogResult = DialogResult.Cancel;
		}
		else
		{
	...
	public async Task<object> PopulateDocument()
	{
		...
		{
			exception.Log();
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "因内存加载资源过多，历史文档未能成功打开，可尝试重新登录再打开历史文档！");
			return null;
		}
	}
	...
	public void PopulateColumns()
	{
		...
		foreach (Leqisoft.Model.Column column2 in RevertedTable.Columns)
		{
		...
		List<Leqisoft.DTO.CellRange> mergeInfo = RevertedTable.GetMergeInfo(visibleOnly: false);
		...
		{
			Leqisoft.Model.Column column = RevertedTable.Columns[m];
	...
	private async void _form_Shown(object sender, EventArgs e)
	{
		...
		_cmdPrevious.Image = Theme.SelectedLeqiTheme.GetThemedBitmap(Resources.PreviousError);
		_cmdNext.Image = Theme.SelectedLeqiTheme.GetThemedBitmap(Resources.NextError);
		_cmdRevert.Image = Theme.SelectedLeqiTheme.GetThemedBitmap(Resources.RevertTable);
	...
	{
		Leqisoft.Model.Cell cell = RevertedTable[e.Row, e.Col];
	...
	private async Task DoRevertTableByIndex(int index)
	{
		...
		try
		{
			...
			Leqisoft.Model.Table table = RevertedTable;
			progressForm.ShowDialog(progressRuntimeData, async delegate
			{
				await Task.Delay(1).ConfigureAwait(continueOnCapturedContext: false);
			...
			Leqisoft.Model.Project.Current.Dal.DeleteTable(table.Id);
			table.NeedSave = true;
			...
			if (!StorageRouter.IsLocalMode)
			{
				await SignalRClient.SyncProject(Leqisoft.Model.Project.Current.Id.ToString());
			}
		}
		catch (HttpRequestException ex)
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, string.Format(ex.InnerException.Message, StringConstBase.Current.Manager));
		}
	}
	...
	{
		PushEntityMeta meta = Metas[index];
		Leqisoft.Model.Document document = Program.MainForm.CurrentDocumentEditor.Document;
		if (document.Version != LatestVersion)
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "文档数据不是最新版本，需要同步后才可执行。");
			return;
		}
		try
		{
