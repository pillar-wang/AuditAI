...
public class TableNavGrid : ISetTheme
{
	protected class FindRefTitleCellResolver : FormulaReferenceTableTitleCellResolver
	{
		...
		public override TableTitleCell GetTableTitleCell(Leqisoft.Model.Table table, int row, int col)
		{
	...
	public void SelectNavNodeByTableInputValue()
	{
		...
		for (int i = _grid.Rows.Fixed; i < count; i++)
		{
			...
			{
				navNode.RealValueOnTitleCellSameType = Leqisoft.Model.Cell.ChangeToValueOperand(navNode.DisplayStringRealValue, navNode.TitleCell.DataFormat.GetDataType());
			}
		}
		int num = -1;
	...
	public void SetTheme()
	{
		...
		_brushHoverBackground.Color = Color.FromArgb(100, Theme.SelectedLeqiTheme.GetBackgroundSolidColor("C1FlexGrid\\Styles\\Highlight\\Background"));
		_colorPositionLine = Theme.SelectedLeqiTheme.GetC1Theme().GetColor("C1FlexGrid\\Styles\\Normal\\Border\\Color");
		_contentTextDefaultColor = Theme.SelectedLeqiTheme.GetC1Theme().GetColor("BaseThemeProperties\\Styles\\Content\\ForeColor", Color.Black);
		_gridMouseOverMoreMenuIconBrush.Color = Leqisoft.UI.Controls.Util.DarkenColor(_brushHoverBackground.Color, 0.1);
		if (Theme.SelectedLeqiTheme.ThemeFlags.HasFlag(ThemeEnum.Picture))
		{
	...
	private void _grid_BodyOwnerDrawCell(object sender, OwnerDrawCellEventArgs e)
	{
		try
		{
			...
			if (_isMouseOverMoreMenuIcon)
			{
				if (isUseWhiteImage2)
				{
					_gridMouseOverMoreMenuIconBrush.Color = Leqisoft.UI.Controls.Util.LightColor(e.Style.BackColor, 0.2);
				}
				else
				{
					_gridMouseOverMoreMenuIconBrush.Color = Leqisoft.UI.Controls.Util.DarkenColor(_grid.Styles.SelectedColumnHeader.BackColor, 0.1);
				}
		...
		Bitmap GetGridRowMoreMenuImage(out bool isUseWhiteImage)
		{
			...
			if (Theme.SelectedLeqiTheme.ThemeContext.GridMoreMenuImageIndexOnHighLightRow == GridMoreMenuImageIndex.White)
			{
	...
	private void FillNavNodeValue(NavNode rootNode, int fillLevel, FormulaEvaluator formulaEvaluator, VirtualTable virtualTable, TableTitleCell cell)
	{
		...
		void UpdateVirtualTableCellValue(NavNode navNode, object cellValue)
		{
			if (navNode != null && navNode.TitleCell != null && Table.Title.GetCellIndex(navNode.TitleCell, out var rowIndex, out var colIndex))
			{
				...
				object value = Leqisoft.Model.Cell.ChangeDataTypeImpl(cellValue, navNode.TitleCell.DataFormat.GetDataType());
	...
	private List<Tuple<string, string>> ConvertOperandToNodeDisplayValueList(Operand op, bool isNavCellCountMoreThanOne, out TreeListOperand treeListData)
	{
		...
		return ((!(op is InputListOperand inputListOperand)) ? op.ToValueSetOrderByRowIndex() : inputListOperand)?.Set.Select((Tuple<Leqisoft.Model.Row, ValueOperand> tup) => Tuple.Create(tup.Item2.ToString(), tup.Item2.ToString())).ToList();
