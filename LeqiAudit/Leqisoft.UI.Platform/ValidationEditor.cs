...
public class ValidationEditor
{
	...
	private void CmdRemove_Click(object sender, ClickEventArgs e)
	{
		...
		{
			if (Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.Question, "确定删除本条校验公式吗?", MessageBoxButtons.OKCancel) == DialogResult.OK)
			{
		...
		if (Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.Question, $"确定删除选中的 {bottomRow - topRow + 1} 条校验公式吗?", MessageBoxButtons.OKCancel) == DialogResult.OK)
		{
