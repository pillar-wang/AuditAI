...
internal static class Program
{
	...
	static Program()
	{
		try
		{
			// CefSharp.Core assembly not available in the decompiled project
		...
		catch (Exception ex)
		{
			...
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "初始化Cef控件失败: \r\n" + ex.ToString());
			throw ex;
		}
	}
	...
	private static void Main()
	{
		...
		// ★ 新增：初始化本地存储（必须在 StartLeqiPlatform 之前）
		Leqisoft.LocalDataStore.StorageRouter.Initialize();
	...
	private static void StartLeqiPlatform()
	{
		...
		static void startLeqiPlatform()
		{
			...
			AppCommands.TeamUsers.Button.Text = (Leqisoft.Model.User.Current.IsTeamAdmin ? "同事管理" : "我的同事");
	...
	{
		string text = Path.Combine(Application.StartupPath, "乐其采数器", "乐其采数器.exe");
		if (!File.Exists(text))
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "找不到程序文件");
			return;
		}
		Process process = new Process();
		process.StartInfo = new ProcessStartInfo(text)
		{
			WorkingDirectory = Path.Combine(Application.StartupPath, "乐其采数器"),
	...
	public static bool CheckUpdaterExeExist(bool isExistApplication)
	{
		...
		string path = Path.Combine(Path.GetDirectoryName(location), "leqiupdater.exe");
		if (!File.Exists(path))
		{
	...
	private static string GetOpeningProjectDebugInfo()
	{
		...
		try
		{
			...
			{
				stringBuilder.Append("当前打开的项目: " + Leqisoft.Model.Project.Current.Name + ", ID=" + Leqisoft.Model.Project.Current.Id.ToString("D") + "\n");
			}
			if (MainForm != null)
			{
	...
	private static async void CurrentDomain_UnhandledException(object sender, UnhandledExceptionEventArgs e)
	{
		...
		try
		{
			...
			string exceptStr = GetOpeningProjectDebugInfo() + $"UserId:\t{Leqisoft.Model.User.Current?.Id}\n{GetAppVersionInfo()}{GetAppServerInfo()}\n{ex}";
			...
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "程序因发生异常将强行关闭，您可稍候再次重启！\r\n" + exceptStr + rootStatckTrace);
		}
	...
	public static async Task<List<UserTeam>> GetUserTeams(bool withNotice = true)
	{
		...
		catch (ServerException ex3)
		{
			if (withNotice)
			{
				Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, ex3.ToString());
			}
			return null;
		}
	...
	public static async Task<bool> OpenTeam(Guid teamId, bool withNotice = true)
	{
		...
		if (userTeam == null)
		{
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "打开失败，您不在该组织中，请刷新组织列表");
			return false;
		}
	...
	private static void ShowAbleOpenTeamClientPlatformMessage(UserTeam userTeam)
	{
		...
		else
		{
			...
			{
				Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "当前客户端已不支持该类型的组织，请致电官方客服电话：400-690-6500 进行处理。");
				return;
			}
		}
		string text2 = ((UserTeam.Teams.Count > 1) ? ("当前客户端不支持该组织，请登录 " + text + " 进行操作。") : ("当前客户端不支持用户所在的组织，请登录 " + text + " 进行操作。"));
		Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, text2);
	}
	...
	public static bool IsClientPlatformMatchToTeamType(UserTeam userTeam, bool isAllowUseSystemSupporterRule = true)
	{
		...
		{
			if (isAllowUseSystemSupporterRule && Leqisoft.Model.User.Current.IsSystemSupporter)
			{
				return true;
			}
			if (userTeam.Type == AppEditions.Audit.Code || userTeam.Type == AppEditions.Tax.Code)
	...
	private static void InitPlatformData()
	{
		try
		{
			if (ClientPlatformType == PlatformType.EnterpriseManagerPlatform || ClientPlatformType == PlatformType.TableDevelopPlatform || ClientPlatformType == PlatformType.ProductionCostAccountingSystem || ClientPlatformType == PlatformType.ContractLedgerManagementSystem || ClientPlatformType == PlatformType.RDExpenseLedgerSystem || ClientPlatformType == PlatformType.SalesOrderManagementSystem || ClientPlatformType == PlatformType.PSIManagementSystem || ClientPlatformType == PlatformType.ProjectLedgerManagementSystem)
			{
				DeleteLeqiCaishuQiDir();
			}
			else if (ClientPlatformType == PlatformType.Custom)
			{
				InitCustomPlatformData();
				if (ClientCustomizeData.Current.GetOptionValueInSettingIniFile_Bool("delete_leqi_cai_shu_qi_folder", defaultValue: true))
				{
					DeleteLeqiCaishuQiDir();
				}
			}
	...
	private static void InitCustomPlatformData()
	{
		...
		{
			ex.Log("客户端配置数据初始化失败");
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.Hand, ex.ToString(), MessageBoxButtons.OK, "系统配置参数错误!");
			Application.Exit();
		}
	...
	private static void DeleteLeqiCaishuQiDir()
	{
	...
	private static async Task<Stream> HandleLocalApi(string url)
	{
		...
		{
			var result = await Leqisoft.LocalDataStore.StorageRouter.GetTableCollectDic();
			jsonBytes = Encoding.UTF8.GetBytes(result.ToString());
			return new MemoryStream(jsonBytes);
		}
		if (url.Contains("CellCollectDic"))
		{
			var result = await Leqisoft.LocalDataStore.StorageRouter.GetCellCollectDic();
			jsonBytes = Encoding.UTF8.GetBytes(result.ToString());
			return new MemoryStream(jsonBytes);
		}
		if (url.Contains("LedgerValidateDic"))
		{
			var result = await Leqisoft.LocalDataStore.StorageRouter.GetLedgerValidateDic();
			jsonBytes = Encoding.UTF8.GetBytes(result.ToString());
			return new MemoryStream(jsonBytes);
		}
		...
		{
			var templates = await Leqisoft.LocalDataStore.StorageRouter.GetTemplates();
		...
		{
			var result = await Leqisoft.LocalDataStore.StorageRouter.OpenProject(Guid.Empty); // projectId 从 URL 解析
		...
		{
			var result = await Leqisoft.LocalDataStore.StorageRouter.GetUserTeams();
			jsonBytes = Encoding.UTF8.GetBytes(result.ToString());
			return new MemoryStream(jsonBytes);
		}
		if (url.Contains("Project/GetTeamUsersWithPic"))
		{
			var users = await Leqisoft.LocalDataStore.StorageRouter.GetTeamUsersWithPic();
		...
		{
			var result = await Leqisoft.LocalDataStore.StorageRouter.PushTable(null);
