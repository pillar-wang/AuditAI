...
public class frmLedgerCollectFormulaEdit : Form
{
	...
	private Dictionary<Leqisoft.Model.Column, string> _changedFormula;

	private LedgerCollectFormulaEditor _owner;

	private Leqisoft.Model.Table _inEditingTable;

	private Leqisoft.Model.Column _inEditingColumn;

	private C1FlexGridEx _navGrid;

	private Dictionary<Leqisoft.Model.Column, string> _rawFormulaDic = new Dictionary<Leqisoft.Model.Column, string> ();

	private Dictionary<Leqisoft.Model.Column, string> _afterModifyFormulaTextDic = new Dictionary<Leqisoft.Model.Column, string> ();
	...
	public Dictionary<Leqisoft.Model.Column, string> ChangedFormula => _changedFormula;
	...
	private void _navGrid_BodySelectionChanged (object sender, EventArgs e)
	{
		...
		if (topRow >= _navGrid.Rows.Fixed && topRow < _navGrid.Rows.Count && leftCol >= _navGrid.Cols.Fixed && leftCol < _navGrid.Cols.Count && _navGrid.GetUserData (topRow, leftCol) is Leqisoft.Model.Column column && column != _inEditingColumn) {
			if (_inEditingColumn != null) {
				_afterModifyFormulaTextDic [_inEditingColumn] = rtbFormulaInput.Text;
			}
			string value = null;
	...
	public void PopulateNavGrid (Leqisoft.Model.Table table, Leqisoft.Model.Column column)
	{
		_inEditingTable = table;
		_inEditingColumn = column;
		List<Tuple<string, Leqisoft.Model.Column>> list = new List<Tuple<string, Leqisoft.Model.Column>> ();
		...
		Leqisoft.UI.Controls.Theme.SetCurrentTree (_navGrid);
		...
		for (int num3 = 0; num3 < list.Count; num3++) {
			...
			Tuple<string, Leqisoft.Model.Column> tuple = list [num3];
	...
	private void TryToSelectNavGridRow (Leqisoft.Model.Column tableColum, out string tableColumnName)
	{
		tableColumnName = string.Empty;
		int num = -1;
		for (int num2 = 0; num2 < _navGrid.Rows.Count; num2++) {
			Leqisoft.Model.Column column = _navGrid.GetUserData (num2, 0) as Leqisoft.Model.Column;
	...
	private void Form_Shown (object sender, EventArgs e)
	{
		...
		_gridHilightForeColor = Leqisoft.UI.Controls.Theme.SelectedLeqiTheme.GetC1Theme ().GetColor ("C1FlexGrid\\Styles\\Highlight\\ForeColor");
		_gridHilightBackColor = Leqisoft.UI.Controls.Theme.SelectedLeqiTheme.GetBackgroundSolidColor ("C1FlexGrid\\Styles\\Highlight\\Background");
		_gridHightBorderColor = Leqisoft.UI.Controls.Theme.SelectedLeqiTheme.GetC1Theme ().GetColor ("C1FlexGrid\\Styles\\Highlight\\Border\\Color");
		_gridNormalBorderColor = Leqisoft.UI.Controls.Theme.SelectedLeqiTheme.GetC1Theme ().GetColor ("C1FlexGrid\\Styles\\Normal\\Border\\Color");
		...
		void DrawSelectionRangeBorder (PaintEventArgs e2)
		{
			...
			try {
				if (IsGridSelectColumnInFormula (_balanceGrid)) {
					...
					_penAnimateDash.Color = ((Control.MouseButtons == MouseButtons.Left) ? Leqisoft.UI.Controls.Theme.SelectedLeqiTheme.ThemeContext.DarkColor : Program.MainForm.FormulaEditor.NextColor);
	...
	private void _balanceGrid_BodyOwnerDrawCell (object sender, OwnerDrawCellEventArgs e)
	{
		...
		Leqisoft.Model.Cell cell = _balanceVirtualTable [e.Row, virtualTableColumnIndex];
	...
	private bool IsGridSelectColumnInFormula (C1FlexGridEx grid)
	{
		...
		Leqisoft.Model.Column col = virtualTable.Columns [num2];
	...
	private void _voucherGrid_BodyOwnerDrawCell (object sender, OwnerDrawCellEventArgs e)
	{
		...
		Leqisoft.Model.Cell cell = _voucherVirtualTable [e.Row, e.Col];
		if (cell.Value is DateYearMonth dateYearMonth) {
			e.Text = dateYearMonth.Date.ToString ("yyyy-MM");
	...
	private void CmdInsertColumn1_Click (object sender, ClickEventArgs e)
	{
		...
		form.AfterSelected += delegate(object obj, Leqisoft.Model.Column column) {
	...
	private void CmdInsertVariable_Click (object sender, ClickEventArgs e)
	{
		...
		if (referenceEditor.ShowSelect () == DialogResult.OK) {
			Leqisoft.Model.DataReference selectedReference = referenceEditor.SelectedReference;
	...
	private bool ValidteAllColumnFormula (out Dictionary<Leqisoft.Model.Column, string> changedFormulaDic)
	{
		changedFormulaDic = null;
		Dictionary<Leqisoft.Model.Column, string> dictionary = new Dictionary<Leqisoft.Model.Column, string> ();
		foreach (Leqisoft.Model.Column key in _afterModifyFormulaTextDic.Keys) {
			...
			try {
				...
				Leqisoft.UI.Controls.MessageBox.Show (MessageBoxIcon.None, GetErrorMessagePrefix (tableColumnName) + "函数参数数量错误");
				return false;
			} catch (FormulaFunctionNotExistException) {
				TryToSelectNavGridRow (key, out var tableColumnName2);
				Leqisoft.UI.Controls.MessageBox.Show (MessageBoxIcon.None, GetErrorMessagePrefix (tableColumnName2) + "函数不存在");
				return false;
			} catch (FormulaSyntaxException ex3) {
				TryToSelectNavGridRow (key, out var tableColumnName3);
				Leqisoft.UI.Controls.MessageBox.Show (MessageBoxIcon.None, GetErrorMessagePrefix (tableColumnName3) + "语法错误: " + ex3.Message);
				try {
				...
				Leqisoft.UI.Controls.MessageBox.Show (MessageBoxIcon.None, GetErrorMessagePrefix (tableColumnName4) + "引用不存在");
				return false;
			} catch (FormulaNotApplicableException ex5) {
				TryToSelectNavGridRow (key, out var tableColumnName5);
				Leqisoft.UI.Controls.MessageBox.Show (MessageBoxIcon.None, GetErrorMessagePrefix (tableColumnName5) + ex5.Message);
				return false;
			} catch (FormulaBadValueException ex6) {
				TryToSelectNavGridRow (key, out var tableColumnName6);
				Leqisoft.UI.Controls.MessageBox.Show (MessageBoxIcon.None, GetErrorMessagePrefix (tableColumnName6) + "值范围错误: " + ex6.Message);
				return false;
			} catch (FormulaTypeMismatchException) {
				TryToSelectNavGridRow (key, out var tableColumnName7);
				Leqisoft.UI.Controls.MessageBox.Show (MessageBoxIcon.None, GetErrorMessagePrefix (tableColumnName7) + "参数类型错误");
				return false;
			} catch (FormulaColumnWildcardNoRowException) {
				TryToSelectNavGridRow (key, out var tableColumnName8);
				Leqisoft.UI.Controls.MessageBox.Show (MessageBoxIcon.None, GetErrorMessagePrefix (tableColumnName8) + "公式引用的列没有本表对应的行");
				return false;
			} catch (Exception ex9) {
				ex9.Log ();
				TryToSelectNavGridRow (key, out var tableColumnName9);
				Leqisoft.UI.Controls.MessageBox.Show (MessageBoxIcon.None, GetErrorMessagePrefix (tableColumnName9) + ex9.Message);
				return false;
			}
		}
		changedFormulaDic = new Dictionary<Leqisoft.Model.Column, string> ();
		foreach (Leqisoft.Model.Column key2 in dictionary.Keys) {
