...
public class ProjectImport
{
	...
	private void ImportFolderImpl(object parent, int index, string folder)
	{
		...
		foreach (KeyValuePair<string, FileKind> item in files)
		{
			...
			{
				ex.Log();
				Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, item.Key + "导入失败！失败原因：" + ex.Message);
			}
		}
	}
	...
	private TreeNodeBase ImportTableImpl(int index, string file, object parent, Func<int, TreeTableNode> genTableNode, Func<int, TreeDirectoryNode> genDirNode)
	{
		...
		{
			exception.Log();
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "导入文件时出现错误，无法导入。若文件是xls格式，建议另存为xlsx格式后再次尝试导入。");
			return null;
		}
	...
	private TreeImageNode ImportImage(TreeGroup parent, int index, string file)
	{
		...
		try
		{
			...
			using (System.Drawing.Image image = System.Drawing.Image.FromFile(file))
			{
				...
				{
					Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "您导入的图片文件过大，将自动缩小为适合尺寸！");
		...
		{
			exception.Log();
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "打开图片文件时发生错误。");
			return null;
		}
	...
	private TreePdfNode ImportPdf(TreeGroup parent, int index, string file)
	{
		...
		{
			exception.Log();
			Leqisoft.UI.Controls.MessageBox.Show(MessageBoxIcon.None, "打开 PDF 文件时发生错误。");
			return null;
		}
