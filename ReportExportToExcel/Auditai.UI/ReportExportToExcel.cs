...
public class ReportExportToExcel
{
	...
	public void SetFormula(ExcelContex context)
	{
		try
		{
			...
			foreach (Auditai.Model.Column column in Table.Columns)
			{
				...
				foreach (Auditai.Model.Row row2 in Table.Rows)
				{
					try
					{
						Auditai.Model.Cell cell = Table[row2.Index, column.Index];
						if (cell.IsAllowUseColumnFormulaResultUpdateCellValue)
						{
							...
							{
								CurrentTable = Table,
								CellOffset = (Auditai.Model.Cell c) => Tuple.Create(context[c.Row.Table].RowOffSet, context[c.Row.Table].ColumnOffSet),
								CurrentRowIndex = row2.Index,
								Resolver = new FormulaReferenceModelResolver(Table.Project),
								TablePathMapper = (Auditai.Model.Table t) => context[t].SheetName,
								CurrentCell = cell,
			...
			foreach (Auditai.Model.Row row3 in Table.Rows)
			{
				...
				foreach (Auditai.Model.Column column2 in Table.Columns)
				{
					try
					{
						Auditai.Model.Cell cell3 = Table[row3.Index, column2.Index];
						if (!cell3.HasFormula)
						{
							continue;
						}
						...
						if (!formulaEvaluator2.CannotExportExcel(cannotExportExcelContext))
						{
							...
							{
								CurrentTable = Table,
								CellOffset = (Auditai.Model.Cell c) => Tuple.Create(context[c.Row.Table].RowOffSet, context[c.Column.Table].ColumnOffSet),
	...
	public static void BatchExportToFile(string filename, List<Tuple<Auditai.Model.Table, PageSetup>> batchs)
	{
	...
	private void Build(XLSheet sheet, out int rowOffSet, out int columnOffSet)
	{
		...
		static object ConvertValue(Auditai.Model.Cell cell)
		{
			if (cell == null)
			{
				return string.Empty;
		...
		void setCaption()
		{
			...
			if (Table.HeaderMode == TableHeaderMode.Custom)
			{
				...
				for (int num10 = 0; num10 < captionLayers; num10++)
				{
					...
					{
						Auditai.Model.CellStyle captionStyle = Table.Columns[num11].CaptionStyle;
		...
		void setContent()
		{
			for (int num7 = 0; num7 < Table.Rows.Count; num7++)
			{
				...
				{
					Auditai.Model.Cell cell2 = Table[num7, num9];
					if (cell2 != null)
					{
	...
	private XLStyle GetCellStyle(Auditai.Model.Cell cell)
	{
		XLStyle xLStyle = new XLStyle(xlBook);
